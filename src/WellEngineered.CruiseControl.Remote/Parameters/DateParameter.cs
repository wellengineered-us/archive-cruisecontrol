...
namespace WellEngineered.CruiseControl.Remote.Parameters
{
        ...
        : ParameterBase
    {
        ...
        {
            get { return this.myMinValue; }
            set { this.myMinValue = value; }
        }
        #endregion
        ...
        {
            get { return this.myMaxValue; }
            set { this.myMaxValue = value; }
        }
        #endregion
        ...
        {
            get { return this.myIsRequired; }
            set { this.myIsRequired = value; }
        }
        #endregion
        ...
        {
            get { return this.myClientDefault; }
            set { this.myClientDefault = value; }
        }
        #endregion
        #endregion
        ...
        public override Exception[] Validate(string value)
        {
            ...
            {
                if (this.IsRequired) exceptions.Add(this.GenerateException("Value of '{name}' is required"));
            }
            else
            {
                if (DateTime.TryParse(value, out actualValue))
                {
                    if (actualValue < this.myMinValue)
                    {
                        exceptions.Add(
                            this.GenerateException("Value of '{name}' is less than the minimum allowed ({0})",
                                    this.myMinValue.ToString(CultureInfo.InvariantCulture.DateTimeFormat.ShortDatePattern, CultureInfo.InvariantCulture)));
                    }
                    if (actualValue > this.myMaxValue)
                    {
                        exceptions.Add(
                            this.GenerateException("Value of '{name}' is more than the maximum allowed ({0})",
                                    this.myMaxValue.ToString(CultureInfo.InvariantCulture.DateTimeFormat.ShortDatePattern, CultureInfo.InvariantCulture)));
                    }
                }
                else
                {
                    exceptions.Add(this.GenerateException("Value of '{name}' is not a date"));
                }
            }
        ...
        {
            this.myClientDefault = this.CalculateDate(this.DefaultValue).ToShortDateString();
        }
        #endregion
        ...
        private DateTime CalculateDate(string value)
        {
            ...
            {
                date = this.CalculateOperation(value.Substring(5), DateTime.Today);
            }
            else if (dayOfWeekRegex.IsMatch(value))
            {
                ...
                date = this.CalculateOperation(value.Substring(12), date);
            }
            else if (dayOfMonthRegex.IsMatch(value))
            {
                ...
                date = this.CalculateOperation(value.Substring(day.Value.Length + 12), date);
            }
            else
            {
