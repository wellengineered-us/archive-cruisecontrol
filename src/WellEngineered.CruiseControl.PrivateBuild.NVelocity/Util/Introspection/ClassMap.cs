...
namespace WellEngineered.CruiseControl.PrivateBuild.NVelocity.Util.Introspection
{
    ...
    public class ClassMap
    {
        ...
        public ClassMap(System.Type clazz, Log log)
        {
            ...
            this.CreateMethodCache();
        }

        private void CreatePropertyCache()
        {
            // Get all publicly accessible methods
            IList<PropertyInfo> properties = GetAccessibleProperties(this.clazz);
            ...
            foreach (PropertyInfo property in properties)
            {
                try
                {
                    this.propertyCache[property.Name] = new PropertyEntry(property);
                }
                catch
                {
        ...
        {
            // Get all publicly accessible methods
            IList<MethodInfo> methods = GetAccessibleMethods(this.clazz);
            ...
            foreach (MethodInfo method in methods)
            {
                if (!method.IsGenericMethod)
                {
                    ...
                    if (this.methodCache.ContainsKey(methodName))
                    {
                        Dictionary<string, MethodEntry> methodInofs = this.methodCache[methodName];
                    ...
                    else
                    {
                        try
                        {
                            ...
                            methodInofs[MakeMethodKey(method)] = new MethodEntry(method);
                            this.methodCache[methodName] = methodInofs;
                        }
                        catch
                        {
        ...
        public  MethodEntry FindMethod(string name, object[] parameters)
        {
            ...
            this.methodCache.TryGetValue(name, out methodEntry);
        ...
        public  PropertyEntry FindProperty(string name)
        {
            ...
            this.propertyCache.TryGetValue(name, out entry);
