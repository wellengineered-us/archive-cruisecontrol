...
namespace WellEngineered.CruiseControl.PrivateBuild.NVelocity.Util.Introspection
{
    ...
    public class SecureUberspector : UberspectImpl, IRuntimeServicesAware
    {
        ...
        {
            string[] badPackages = this.runtimeServices.Configuration.GetStringArray(RuntimeConstants.INTROSPECTOR_RESTRICT_PACKAGES);

            string[] badClasses = this.runtimeServices.Configuration.GetStringArray(RuntimeConstants.INTROSPECTOR_RESTRICT_CLASSES);
        ...
        {
            if ((obj != null) && !((ISecureIntrospectorControl)this.introspector).CheckObjectExecutePermission(obj.GetType(), null))
            {
                this.log.Warn("Cannot retrieve iterator from object of class " + obj.GetType().FullName + " due to security restrictions.");
                return null;
            }
            else
            {
