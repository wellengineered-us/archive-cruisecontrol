    ...
    class IncludeEventExecutor : IEventHandlerMethodExecutor
    {
        ...
        public virtual void Execute(IEventHandler handler)
        {
            ...
            if (eh is IContextAware)
                ((IContextAware)eh).Context = this.context;

            this.executed = true;
            this.includeResourcePath = ((IIncludeEventHandler)handler).IncludeEvent(this.includeResourcePath, this.currentResourcePath, this.directiveName);
        }
    }
