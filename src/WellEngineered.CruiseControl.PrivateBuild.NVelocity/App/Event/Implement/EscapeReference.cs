...
namespace WellEngineered.CruiseControl.PrivateBuild.NVelocity.App.Event.Implement
{
    ...
    public abstract class EscapeReference : IReferenceInsertionEventHandler, IRuntimeServicesAware
    {
        ...
        public virtual object ReferenceInsert(string reference, object value)
        {
            ...
            else if (System.Text.RegularExpressions.Regex.IsMatch(reference, this.matchRegExp))
            {
                return this.Escape(value);
            }
            else
        ...
        public virtual void SetRuntimeServices(IRuntimeServices rs)
        {
            ...
            */
            this.matchRegExp = StringUtils.NullTrim(rs.Configuration.GetString(this.MatchAttribute));
            ...
            */
            if (this.matchRegExp != null)
            {
                try
                {
                    System.Text.RegularExpressions.Regex.Match("", this.matchRegExp);
                }
                catch (ArgumentException E)
                {
                    rs.Log.Error("Invalid regular expression '" + this.matchRegExp + "'.  No escaping will be performed.", E);
                    this.matchRegExp = null;
                }
            }
        }
