    ...
    public class ReportInvalidReferences : IInvalidReferenceEventHandler, IRuntimeServicesAware
    {
        ...
        public virtual object InvalidMethod(IContext context, string reference, object object_Renamed, string method, Info info)
        {
            if (reference == null)
            {
                ...
                this.reportInvalidReference(object_Renamed.GetType().FullName + "." + method, info);
            }
            else
            {
                this.reportInvalidReference(reference, info);
        ...
        {
            this.stopOnFirstInvalidReference = rs.Configuration.GetBoolean(EVENTHANDLER_INVALIDREFERENCE_EXCEPTION, false);
        }
    }
}
