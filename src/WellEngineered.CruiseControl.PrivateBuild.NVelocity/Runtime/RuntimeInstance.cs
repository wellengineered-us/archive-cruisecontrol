    ...
    public class RuntimeInstance : IRuntimeServices
    {
        ...
        private string DefaultEncoding
        {
            get
            {
                ...
                {
                    this.encoding = this.GetString(RuntimeConstants.INPUT_ENCODING, RuntimeConstants.ENCODING_DEFAULT);
                }
        ...
        */
        ...
        {
            string[] uberspectors = this.configuration.GetStringArray(RuntimeConstants.UBERSPECT_CLASSNAME);
            for (int i = 0; i < uberspectors.Length; i++)
            {
                ...
                {
                    ((IUberspectLoggable)u).Log = this.Log;
                }
                ...
                else
                {
                    ...
                    {
                        ((IChainableUberspector)u).Wrap(this.uberSpect);
                        this.uberSpect = u;
                    }
                    else
                    {
        ...
        private void SetDefaultProperties()
        {
            try
            {
                ...
                using (Stream inputStream = this.GetType().Assembly.GetManifestResourceStream(RuntimeConstants.DEFAULT_RUNTIME_PROPERTIES))
                {
                    this.configuration.Load(inputStream);

                    if (this.log.DebugEnabled)
                    {
                        this.log.Debug("Default Properties File: " + new FileInfo(RuntimeConstants.DEFAULT_RUNTIME_PROPERTIES).FullName);
                    }
                }
            }
        ...
        public object GetProperty(string key)
        {
            ...
            */
            ...
            {
                o = this.overridingProperties[key];
            }
        ...
        private void InitializeProperties()
        {
            ...
            */
            if (this.configuration.isInitialized == false)
            {
        ...
        {
            this.Properties = ExtendedProperties.ConvertProperties(p);
            this.Init();
        }
        ...
        {
            this.Properties = new ExtendedProperties(configurationFile);
            this.Init();
        }
        ...
        private void InitializeResourceManager()
        {
            ...
            */
            ...
            string rm = this.GetString(RuntimeConstants.RESOURCE_MANAGER_CLASS);
            ...
            if (rm != null && rm.Length > 0)
            {
                ...
                */
                ...
                this.resourceManager = (IResourceManager)o;
        ...
        private void InitializeEventHandlers()
        {
            ...
            */
            ...
            string[] referenceinsertion = this.configuration.GetStringArray(NVelocity.Runtime.RuntimeConstants.EVENTHANDLER_REFERENCEINSERTION);
            if (referenceinsertion != null)
            {
                for (int i = 0; i < referenceinsertion.Length; i++)
                {
                    IEventHandler ev = this.InitializeSpecificEventHandler(referenceinsertion[i], NVelocity.Runtime.RuntimeConstants.EVENTHANDLER_REFERENCEINSERTION, typeof(IReferenceInsertionEventHandler));
                    if (ev != null)
                        this.eventCartridge.AddReferenceInsertionEventHandler((IReferenceInsertionEventHandler)ev);
                }
            }

            string[] nullset = this.configuration.GetStringArray(NVelocity.Runtime.RuntimeConstants.EVENTHANDLER_NULLSET);
            if (nullset != null)
            {
                for (int i = 0; i < nullset.Length; i++)
                {
                    IEventHandler ev = this.InitializeSpecificEventHandler(nullset[i], NVelocity.Runtime.RuntimeConstants.EVENTHANDLER_NULLSET, typeof(INullSetEventHandler));
                    if (ev != null)
                        this.eventCartridge.AddNullSetEventHandler((INullSetEventHandler)ev);
                }
            }

            string[] methodexception = this.configuration.GetStringArray(NVelocity.Runtime.RuntimeConstants.EVENTHANDLER_METHODEXCEPTION);
            if (methodexception != null)
            {
                for (int i = 0; i < methodexception.Length; i++)
                {
                    IEventHandler ev = this.InitializeSpecificEventHandler(methodexception[i], NVelocity.Runtime.RuntimeConstants.EVENTHANDLER_METHODEXCEPTION, typeof(IMethodExceptionEventHandler));
                    if (ev != null)
                        this.eventCartridge.AddMethodExceptionHandler((IMethodExceptionEventHandler)ev);
                }
            }

            string[] includeHandler = this.configuration.GetStringArray(NVelocity.Runtime.RuntimeConstants.EVENTHANDLER_INCLUDE);
            if (includeHandler != null)
            {
                for (int i = 0; i < includeHandler.Length; i++)
                {
                    IEventHandler ev = this.InitializeSpecificEventHandler(includeHandler[i], NVelocity.Runtime.RuntimeConstants.EVENTHANDLER_INCLUDE, typeof(IIncludeEventHandler));
                    if (ev != null)
                        this.eventCartridge.AddIncludeEventHandler((IIncludeEventHandler)ev);
                }
            }

            string[] invalidReferenceSet = this.configuration.GetStringArray(NVelocity.Runtime.RuntimeConstants.EVENTHANDLER_INVALIDREFERENCES);
            if (invalidReferenceSet != null)
            {
                for (int i = 0; i < invalidReferenceSet.Length; i++)
                {
                    IEventHandler ev = this.InitializeSpecificEventHandler(invalidReferenceSet[i], NVelocity.Runtime.RuntimeConstants.EVENTHANDLER_INVALIDREFERENCES, typeof(IInvalidReferenceEventHandler));
                    if (ev != null)
                    {
                        this.eventCartridge.AddInvalidReferenceEventHandler((IInvalidReferenceEventHandler)ev);
                    }
                }
            }
        }
        ...
        private void InitializeDirectives()
        {
            ...
            */
            this.runtimeDirectives = new Dictionary<string, Directive.Directive>();
            ...
            */
            ...
            try
            {
                using (Stream inputStream = this.GetType().Assembly.GetManifestResourceStream(RuntimeConstants.DEFAULT_RUNTIME_DIRECTIVES))
                {
        ...
        {
            this.runtimeDirectives[directive.Name] = directive;
        }
        ...
        {
            return (Directive.Directive)this.runtimeDirectives[name];
        }
        ...
        private void InitializeParserPool()
        {
            ...
            * Which parser pool?
            */
            string pp = this.GetString(NVelocity.Runtime.RuntimeConstants.PARSER_POOL_CLASS);
        ...
        public virtual Parser.Parser CreateNewParser()
        {
            ...
            parser.Directives = this.runtimeDirectives;
            return parser;
        }
        ...
        {
            return this.Parse(new System.IO.StringReader(string_Renamed), templateName);
        }
        ...
        {
            return this.Evaluate(context, textWriter, logTag, new StringReader(instring));
        }
        ...
        {
            return this.GetTemplate(name, this.DefaultEncoding);
        }
        ...
        public Template GetTemplate(string name, string encoding)
        {
            ...
            return (Template)this.resourceManager.GetResource(name, ResourceManagerConstants.RESOURCE_TEMPLATE, encoding);
        }
        ...
        public ContentResource GetContent(string name)
        {
            ...
            */
            ...
            return this.GetContent(name, this.DefaultEncoding);
        }
        ...
        public ContentResource GetContent(string name, string encoding)
        {
            ...
            return (ContentResource)this.resourceManager.GetResource(name, ResourceManagerConstants.RESOURCE_CONTENT, encoding);
        }
        ...
        {
            return this.vmFactory.DumpVMNamespace(namespaces);
        }
