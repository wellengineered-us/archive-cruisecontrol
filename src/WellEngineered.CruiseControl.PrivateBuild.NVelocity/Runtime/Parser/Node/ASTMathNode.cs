...
namespace WellEngineered.CruiseControl.PrivateBuild.NVelocity.Runtime.Parser.Node
{
    ...
    public abstract class ASTMathNode : SimpleNode
    {
        ...
        public override Object Value(IInternalContextAdapter context)
        {
            ...
            */
            object special = this.HandleSpecial(left, right, context);
            if (special != null)
            {
                return special;
            }
            ...
            */
            if (!(left is System.ValueType) || !(right is System.ValueType))
            {
                ...
                string msg = (wrongright ? "Right" : "Left") + " side of math operation (" + this.GetChild(wrongright ? 1 : 0).Literal + ") " + (wrongtype ? "is not a Number. " : "has a null value. ") + this.GetLocation(context);
                if (this.strictMode)
                {
                    this.log.Error(msg);
            ...
            return this.Perform(left, right, context);
        }
