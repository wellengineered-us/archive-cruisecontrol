...
namespace WellEngineered.CruiseControl.PrivateBuild.NVelocity.Runtime.Parser.Node
{
    ...
    public class ObjectComparer : IComparer
    {
        ...
        private class DoubleComparer : IObjectComparer
        {
            ...
            {
                if (x is double)
                    return this.Compare((double)x, y);

                return -this.Compare((double)y, x);
            }
        ...
        private class FloatComparer : IObjectComparer
        {
            ...
            {
                if (x is float)
                    return this.Compare((float)x, y);

                return -this.Compare((float)y, x);
            }
        ...
        private class ULongComparer : IObjectComparer
        {
            ...
            public int Compare(object x, object y)
            {
                if (x is ulong)
                {
                    ...
                    return this.Compare(xAsLong, y);
                }

                return -this.Compare((ulong)y, x);
            }
