    ...
    public class ASTStringLiteral : SimpleNode
    {
        ...
        public override object Init(IInternalContextAdapter context, object data)
        {
            ...
            */
            ...
            this.interpolate = this.rsvc.GetBoolean(RuntimeConstants.INTERPOLATE_STRINGLITERALS, true) && this.FirstToken.Image.StartsWith("\"") && ((this.FirstToken.Image.IndexOf('$') != -1) || (this.FirstToken.Image.IndexOf('#') != -1));

            /*
            * Get the contents of the string, minus the '/" at each end
            */

            this.image = this.FirstToken.Image.Substring(1, (this.FirstToken.Image.Length - 1) - (1));
            if (this.FirstToken.Image.StartsWith("\""))
            {
            ...
            */
            ...
            if (this.interpolate)
            {
                ...
                */
                ...
                {
                    string msg = "Failed to parse String literal at " + Log.Log.FormatFileString(templateName, this.Line, this.Column);
                    throw new TemplateInitException(msg, e, templateName, this.Column, this.Line);
                }
        ...
        public void AdjTokenLineNums(INode node)
        {
            ...
            while (tok != null && tok != node.LastToken)
            {
                ...
                if (tok.BeginLine == 1)
                    tok.BeginColumn += this.Column;

                if (tok.EndLine == 1)
                    tok.EndColumn += this.Column;

                tok.BeginLine += this.Line - 1;
                tok.EndLine += this.Line - 1;
                tok = tok.Next;
            }
        }
        ...
        /// <seealso cref="SimpleNode.jjtAccept(NVelocity.Runtime.Paser.Node.ParserVisitor,">
        /// java.lang.Object)
        /// </seealso>
