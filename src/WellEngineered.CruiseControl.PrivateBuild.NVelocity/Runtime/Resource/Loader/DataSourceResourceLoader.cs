...
namespace WellEngineered.CruiseControl.PrivateBuild.NVelocity.Runtime.Resource.Loader
{
    ...
    public class DataSourceResourceLoader : ResourceLoader
    {
        ...
        {
            return (resource.LastModified != this.ReadLastModified(resource, "checking timestamp"));
        }
        ...
        public override System.IO.Stream GetResourceStream(System.String name)
        {
            lock (this)
            {
                ...
                try
                {
                    ...
                    if (rs.Read())
                    {
                        System.IO.Stream stream = new System.IO.MemoryStream((byte[])rs[this.templateColumn]);
                        if (stream == null)
                        {
        ...
        private long ReadLastModified(Resource resource, System.String operation)
        {
            ...
            else
            {
                ...
                try
                {
                    ...
                    if (rs.Read())
                    {
                        System.DateTime ts = System.Convert.ToDateTime(rs[this.timestampColumn]);
        ...
        private IDataReader ReadData(IDbConnection conn, System.String columnNames, System.String templateName)
        {
            ...
            ps.CommandText = "SELECT " + columnNames + " FROM " + this.tableName + " WHERE " + this.keyColumn + " = '" + templateName + "'";
