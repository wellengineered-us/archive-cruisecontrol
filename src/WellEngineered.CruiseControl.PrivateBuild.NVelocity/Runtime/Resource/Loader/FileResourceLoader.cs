    ...
    public class FileResourceLoader : ResourceLoader
    {
        ...
        public override void Init(ExtendedProperties configuration)
        {
            ...
            if (this.log.DebugEnabled)
            {
                ...
                int sz = this.paths.Count;
                for (int i = 0; i < sz; i++)
                {
                    this.log.Debug("FileResourceLoader : adding path '" + ((string)this.paths[i]) + "'");
                }
        ...
        public override Stream GetResourceStream(string templateName)
        {
            ...
            */
            ...
            int size = this.paths.Count;
            for (int i = 0; i < size; i++)
            {
                string path = (string)this.paths[i];
                ...
                if (inputStream != null)
                {
                    ...
                    * time.
                    */
                    this.templatePaths[templateName] = path;
                    return inputStream;
                }
            }
        ...
        public override bool ResourceExists(string name)
        {
            ...
            for (int i = 0; i < size; i++)
            {
                ...
                try
                {
                    FileInfo file = this.GetFile(path, name);
        ...
        private Stream FindTemplate(string path, string template)
        {
            try
            {
                FileInfo file = this.GetFile(path, template);
        ...
        public override bool IsSourceModified(Resource resource)
        {
            ...
            */
            ...
            string path = (string)this.templatePaths[fileName];
            System.IO.FileInfo currentFile = null;

            for (int i = 0; currentFile == null && i < this.paths.Count; i++)
            {
                string testPath = (string)this.paths[i];
                System.IO.FileInfo testFile = this.GetFile(testPath, fileName);
            ...
            System.IO.FileInfo file = this.GetFile(path, fileName);
        ...
        {
            string path = (string)this.templatePaths[resource.Name];
            System.IO.FileInfo file = this.GetFile(path, resource.Name);
