...
namespace WellEngineered.CruiseControl.PrivateBuild.NVelocity.Runtime.Directive
{
    ...
    public class Parse : InputBase
    {
        ...
        public override void Init(IRuntimeServices rs, IInternalContextAdapter context, INode node)
        {
            ...
            this.maxDepth = this.rsvc.GetInt(NVelocity.Runtime.RuntimeConstants.PARSE_DIRECTIVE_MAXDEPTH, 10);
        }
        ...
        public override bool Render(IInternalContextAdapter context, System.IO.TextWriter writer, INode node)
        {
            ...
            */
            ...
            {
                this.rsvc.Log.Error("#parse() null argument");
                return false;
            }
            ...
            */
            ...
            string arg = EventHandlerUtil.IncludeEvent(this.rsvc, context, sourcearg, context.CurrentTemplateName, this.Name);
            ...
            */
            ...
            if (this.maxDepth > 0)
            {
                ...
                */
                ...
                if (templateStack.Length >= this.maxDepth)
                {
                    ...
                    this.rsvc.Log.Error("Max recursion depth reached (" + templateStack.Length + ')' + " File stack:" + path);
                    return false;
                }
            }
            ...
            */
            ...
            catch (ResourceNotFoundException rnfe)
            {
                ...
                */
                this.rsvc.Log.Error("#parse(): cannot find template '" + arg + "', called at " + Log.Log.FormatFileString(this));
                throw rnfe;
            }
            catch (ParseErrorException pee)
            {
                ...
                *  note it and throw
                */
                this.rsvc.Log.Error("#parse(): syntax error in #parse()-ed template '" + arg + "', called at " + Log.Log.FormatFileString(this));
            ...
            */
            ...
            {
                this.rsvc.Log.Error("Exception rendering #parse(" + arg + ") at " + Log.Log.FormatFileString(this));
                throw e;
            }
