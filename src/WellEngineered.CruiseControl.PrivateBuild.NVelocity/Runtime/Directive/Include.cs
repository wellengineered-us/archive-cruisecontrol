...
namespace WellEngineered.CruiseControl.PrivateBuild.NVelocity.Runtime.Directive
{
    ...
    public class Include : InputBase
    {
        ...
        public override void Init(IRuntimeServices rs, IInternalContextAdapter context, INode node)
        {
            ...
            *  do it each time
            */
            this.outputMsgStart = this.rsvc.GetString(NVelocity.Runtime.RuntimeConstants.ERRORMSG_START);
            this.outputMsgStart = this.outputMsgStart + " ";

            this.outputMsgEnd = this.rsvc.GetString(NVelocity.Runtime.RuntimeConstants.ERRORMSG_END);
        ...
        private bool RenderOutput(INode node, IInternalContextAdapter context, System.IO.TextWriter writer)
        {
            if (node == null)
            {
                this.rsvc.Log.Error("#include() null argument");
                return false;
            }
            ...
            */
            ...
            catch (ResourceNotFoundException rnfe)
            {
                ...
                */
                this.rsvc.Log.Error("#include(): cannot find resource '" + arg + "', called at " + Log.Log.FormatFileString(this));
                throw rnfe;
            }
            /**
            ...
            */
            ...
            {
                this.rsvc.Log.Error("#include(): arg = '" + arg + "', called at " + Log.Log.FormatFileString(this));
                throw e;
            }
