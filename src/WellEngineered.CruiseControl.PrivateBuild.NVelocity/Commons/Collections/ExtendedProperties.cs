...
namespace WellEngineered.CruiseControl.PrivateBuild.NVelocity.Commons.Collections
{
    ...
    public class ExtendedProperties : Hashtable
    {
        ...
        public ExtendedProperties(String file, String defaultFile)
        {
            ...
            this.basePath = this.basePath.Substring(0, (this.basePath.LastIndexOf(this.fileSeparator) + 1) - (0));
                        ...
                        if (equalSignIndex > 0)
                        {
                            ...
                            if (this.Include != null && key.ToUpper().Equals(this.Include.ToUpper()))
                            {
                                ...
                                else
                                {
                                     ...
                                     * before continuing.
                                     */
                                    if (value.StartsWith(string.Format(".{0}", this.fileSeparator)))
                                    {
                                        value = value.Substring(2);
                                    }
                                    file = new FileInfo(this.basePath + value);
                                }
                                ...
                                if (tmpBool)
                                {
                                    this.Load(new FileStream(file.FullName, FileMode.Open, FileAccess.Read));
                                }
                            }
                            else
                            {
        ...
        */
            ...
            lock (this)
            {
                if (output != null)
                {
                    ...
                    foreach (String key in this.Keys)
                    {
                        ...
                        if (value is String)
                        {
                            this.WriteKeyOutput(textWriter, key, (String)value);
                        }
                        else if (value is IEnumerable)
                        {
                            foreach (String currentElement in (IEnumerable)value)
                                this.WriteKeyOutput(textWriter, key, currentElement);
                        }
        ...
        public override String ToString()
        {
            ...
            {
                Object value = this[key];
                sb.AppendFormat("{0} => {1}", key, this.ValueToString(value)).Append(Environment.NewLine);
            }
            return sb.ToString();
        }
        ...
        */
        ...
        {
            Boolean b = this.GetBoolean(key, DEFAULT_BOOLEAN);
            if ((Object)b == null)
            {
                throw new System.Exception(string.Format("{0}{1}' doesn't map to an existing object", '\'', key));
            }
            else
            {
                return b;
        ...
        public sbyte GetByte(String key)
        {
            if (this.ContainsKey(key))
            {
                Byte b = this.GetByte(key, DEFAULT_BYTE);
                return (sbyte)b;
            }
            else
            {
        ...
        {
            Int32 i = this.GetInteger(key, DEFAULT_INT32);
            if ((Object)i == null)
            {
                throw new System.Exception(string.Format("{0}{1}' doesn't map to an existing object", '\'', key));
            }
        ...
        {
            Int64 l = this.GetLong(key, DEFAULT_INT64);
            if ((Object)l == null)
            {
                throw new System.Exception(string.Format("{0}{1}' doesn't map to an existing object", '\'', key));
            }
        ...
        {
            Single f = this.GetFloat(key, DEFAULT_SINGLE);
            if ((Object)f == null)
            {
                throw new System.Exception(string.Format("{0}{1}' doesn't map to an existing object", '\'', key));
            }
        ...
        {
            Double d = this.GetDouble(key, DEFAULT_DOUBLE);
            if ((Object)d == null)
            {
                throw new System.Exception(string.Format("{0}{1}' doesn't map to an existing object", '\'', key));
            }
