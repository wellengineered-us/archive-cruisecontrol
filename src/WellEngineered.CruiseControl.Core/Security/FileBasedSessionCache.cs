...
namespace WellEngineered.CruiseControl.Core.Security
{
        ...
        : SessionCacheBase
    {
		...
		public FileBasedSessionCache(IFileSystem fileSystem, IExecutionEnvironment executionEnvironment, IClock clock) : base(clock)
		{
			...
			this.storeLocation = Path.Combine(this.executionEnvironment.GetDefaultProgramDataFolder(ApplicationType.Server), "sessions");
			fileSystem.EnsureFolderExists(this.storeLocation);
		}
        ...
        public virtual string StoreLocation
        {
            ...
            set
            {
				string path = this.executionEnvironment.EnsurePathIsRooted(value);

				if (!string.IsNullOrEmpty(path))
					this.fileSystem.EnsureFolderExists(path);
        ...
        public override void Initialise()
        {
            ...
            DirectoryInfo storeDirectory = new DirectoryInfo(this.storeLocation);
        ...
        public override void RemoveFromCache(string sessionToken)
        {
            ...
            string sessionFile = this.GenerateFileName(sessionToken);
        ...
        {
            string sessionFile = Path.Combine(this.storeLocation, sessionToken + ".session");
            return sessionFile;
        }
    }
}
