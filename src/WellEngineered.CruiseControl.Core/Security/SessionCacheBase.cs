...
namespace WellEngineered.CruiseControl.Core.Security
{
        ...
        : ISessionCache
    {
        ...
        {
            get { return this.durationInMinutes; }
            set { this.durationInMinutes = value; }
        }
        ...
        {
            get { return this.expiryMode; }
            set { this.expiryMode = value; }
        }
        ...
        public virtual string AddToCache(string userName)
        {
            ...
            SessionDetails session = new SessionDetails(userName, this.clock.Now.AddMinutes(this.durationInMinutes));
            this.AddToCacheInternal(sessionToken, session);
            return sessionToken;
        }
        ...
        protected virtual void AddToCacheInternal(string sessionToken, SessionDetails session)
        {
            lock (this)
            {
                this.cache.Add(sessionToken, session);
            }
        }
        ...
        {
            SessionDetails details = this.RetrieveSessionDetails(sessionToken);
            if (details == null)
            {
                return null;
            }
        ...
        {
            if (this.cache.ContainsKey(sessionToken))
            {
                lock (this)
                {
        ...
        public virtual object RetrieveSessionValue(string sessionToken, string key)
        {
            ...
            if (details != null)
            {
                if (this.cache[sessionToken].Values.ContainsKey(key)) value = this.cache[sessionToken].Values[key];
            }
            return value;
        }
        ...
        protected virtual SessionDetails RetrieveSessionDetails(string sessionToken)
        {
            if (this.cache.ContainsKey(sessionToken))
            {
                SessionDetails session = this.cache[sessionToken];
                if (this.clock.Now < session.ExpiryTime)
                {
                    if (this.expiryMode == SessionExpiryMode.Sliding)
                    {
                        lock (this)
                        {
                            this.cache[sessionToken].ExpiryTime = this.clock.Now.AddMinutes(this.durationInMinutes);
                        }
                    }
                    return session;
                }
                ...
                {
                    this.RemoveFromCache(sessionToken);
                    return null;
                }
            }
            else
