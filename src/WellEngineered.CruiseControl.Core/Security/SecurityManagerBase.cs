...
using WellEngineered.CruiseControl.Core.Security.Auditing;
...
namespace WellEngineered.CruiseControl.Core.Security
{
        ...
        : ISecurityManager
    {
        ...
        {
            get { return this.sessionCache; }
            set { this.sessionCache = value; }
        }
        #endregion
        ...
        {
            get { return this.permissions.DefaultRight == SecurityRight.Deny; }
        }
        #endregion
        ...
        public virtual string Login(LoginRequest credentials)
        {
            ...
            IAuthentication authentication = this.RetrieveUser(identifier);
            ...
            if (authentication != null)
            {
                ...
                {
                    // Start a new session
                    sessionToken = this.sessionCache.AddToCache(userName);
                    displayName = authentication.GetDisplayName(credentials);
                    this.sessionCache.StoreSessionValue(sessionToken, displayNameKey, displayName);
                }
            }
            ...
            if (sessionToken != null)
            {
                ...
                this.LogEvent(null, userName, SecurityEvent.Login, SecurityRight.Allow, null);
            }
            else
            {
                Log.Warning(string.Format(System.Globalization.CultureInfo.CurrentCulture,"Login failure: {0} has failed to login", userName));
                this.LogEvent(null, userName, SecurityEvent.Login, SecurityRight.Deny, null);
            }
        ...
        {
            string userName = this.sessionCache.RetrieveFromCache(sessionToken);
            if (!string.IsNullOrEmpty(userName))
            {
                this.sessionCache.RemoveFromCache(sessionToken);
                Log.Debug(string.Format(System.Globalization.CultureInfo.CurrentCulture,"{0} has logged out", userName));
                this.LogEvent(null, userName, SecurityEvent.Logout, SecurityRight.Allow, null);
            }
            else
            {
                this.LogEvent(null, null, SecurityEvent.Logout, SecurityRight.Deny, "Session has already been logged out");
            }
        }
        #endregion
        ...
        {
            if (sessionToken == null) return null;
            var actualDisplayName = this.sessionCache.RetrieveSessionValue(sessionToken, displayNameKey) as string;
        ...
        public virtual void LogEvent(string projectName, string userName, SecurityEvent eventType, SecurityRight eventRight, string message)
        {
            ...
            {
                foreach (IAuditLogger logger in this.loggers)
                {
        ...
        public virtual List<AuditRecord> ReadAuditRecords(int startPosition, int numberOfRecords)
        {
            ...
            if (this.reader != null) records = this.reader.Read(startPosition, numberOfRecords);
            return records;
        }
        #endregion
        ...
        {
            List<AuditRecord> records = new List<AuditRecord>();
            if (this.reader != null) records = this.reader.Read(startPosition, numberOfRecords, filter);
            return records;
        }
        #endregion
