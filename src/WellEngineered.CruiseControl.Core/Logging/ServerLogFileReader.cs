...
namespace WellEngineered.CruiseControl.Core.Logging
{
    ...
    public class ServerLogFileReader
    {
        ...
        public string Read()
        {
            return this.Read(EnumeratorDirection.Forward);
        }
        ...
        {
            return this.Read(direction, null);
        }
        ...
        {
            return this.Read(EnumeratorDirection.Forward, '[' + project);
        }

        private string Read(EnumeratorDirection direction, string match)
        {
            CircularArray buffer = new CircularArray(this.maxLines);
            using (Stream stream = this.OpenFile())
            {
        ...
        {
            return new FileStream(this.filename, FileMode.Open, FileAccess.Read, FileShare.ReadWrite);
        }
