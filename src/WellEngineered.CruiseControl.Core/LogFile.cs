...
namespace WellEngineered.CruiseControl.Core
{
	...
	public class LogFile
	{
		...
		public LogFile(string filename)
		{
			...
			this._date = this.ParseDate(filename);
			this._label = this.ParseLabel(filename);
			this._succeeded = this.IsSuccessful(filename);
		}
		...
		{
			this._date = result.StartTime;
			this._label = result.Label;
			this._succeeded = result.Succeeded;
		}
		...
		{
			get { return DateUtil.FormatDate(this._date, this._formatter); }
		}
		...
		{
			get { return (this._succeeded) ? this.CreateSuccessfulBuildLogFileName() : this.CreateFailedBuildLogFileName(); }
		}

		private string CreateFailedBuildLogFileName()
		{
			return string.Format(System.Globalization.CultureInfo.CurrentCulture,"{0}{1}.xml", FilenamePrefix, this.FilenameFormattedDateString);
		}

		private string CreateSuccessfulBuildLogFileName()
		{
			return string.Format(System.Globalization.CultureInfo.CurrentCulture,"{0}{1}Lbuild.{2}.xml", FilenamePrefix, this.FilenameFormattedDateString, this._label);
		}
		...
		{
			get { return this._date.ToString(FilenameDateFormat, CultureInfo.CurrentCulture); }
		}
		...
		private DateTime ParseDate(string filename)
		{
			...
			return DateTime.ParseExact(dateString, FilenameDateFormat, this._formatter);
		}
