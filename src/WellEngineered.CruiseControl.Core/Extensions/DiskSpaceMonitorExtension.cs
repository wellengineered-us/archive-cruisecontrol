...
namespace WellEngineered.CruiseControl.Core.Extensions
{
        ...
        : ICruiseServerExtension
    {
        ...
        public virtual void Initialise(ICruiseServer server, ExtensionConfiguration extensionConfig)
        {
            ...
            else
            {
                server.IntegrationStarted += (o, e) =>
                {
                    ...
                    bool hasSpace = true;
                    foreach (var drive in this.driveSpaces.Keys)
                    {
                        var freeSpace = fileSystem.GetFreeDiskSpace(drive);
                        hasSpace &= (freeSpace >= this.driveSpaces[drive]);
                    }
            ...
            foreach (var element in extensionConfig.Items ?? new XmlElement[0])
            {
                if (element.Name == "drive")
                {
                    ...
                    this.AddDriveSpace(element.GetAttribute("name"), element.GetAttribute("unit"), element.InnerText);
                }
                else
                {
        ...
        public long RetrieveMinimumSpaceRequired(string drive)
        {
            ...
            if (this.driveSpaces.ContainsKey(drive))
            {
                return this.driveSpaces[drive];
            }
            else
            {
                return -1;
        ...
        private void AddDriveSpace(string driveName, string unit, string amount)
        {
            ...
            this.driveSpaces.Add(driveName.ToLowerInvariant(), numberOfBytes);
        }
        #endregion
        #endregion
    }
