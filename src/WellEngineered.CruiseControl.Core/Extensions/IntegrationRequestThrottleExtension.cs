...
namespace WellEngineered.CruiseControl.Core.Extensions
{
	/// <summary>
        ...
        : ICruiseServerExtension
    {
        ...
        public void Initialise(ICruiseServer server, ExtensionConfiguration extensionConfig)
        {
            ...
            server.IntegrationStarted += this.server_IntegrationStarted;
            server.IntegrationCompleted += this.server_IntegrationCompleted;
        }
        #endregion
        ...
        /// <param name="e">The <see cref="IntegrationCompletedEventArgs"/> instance containing the event data.</param>
        private void server_IntegrationCompleted(object sender, IntegrationCompletedEventArgs e)
        {
            lock (this.updateLock)
            {
                if (this.requests.Contains(e.ProjectName))
                {
                    this.requests.Remove(e.ProjectName);
                }
            }
        }
        #endregion
        ...
        /// </summary>
        /// <param name="sender">The source of the event.</param>
        /// <param name="e">The <see cref="IntegrationStartedEventArgs"/> instance containing the event data.</param>
        private void server_IntegrationStarted(object sender, IntegrationStartedEventArgs e)
        {
            ...
            lock (this.updateLock)
            {
                if (!this.requests.Contains(e.ProjectName))
                {
                    this.requests.Add(e.ProjectName);
                }

                numberOfRequests = this.requests.Count;
                currentRequests = this.requests.ToArray();
            }
