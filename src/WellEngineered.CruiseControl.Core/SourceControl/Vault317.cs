...
namespace WellEngineered.CruiseControl.Core.SourceControl
{
    ...
    public class Vault317 : Vault3
    {
        ...
        public override Modification[] GetModifications(IIntegrationResult from, IIntegrationResult to)
        {
            ...
            if (this.LookForChangesUsingVersionHistory(from, to))
                return this.GetModificationsFromItemHistory(from, to);
            else
            {
        ...
        {
            Log.Info(string.Format(System.Globalization.CultureInfo.CurrentCulture,"Retrieving detailed change list for {0} in Vault Repository \"{1}\" between {2} and {3}", this._shim.Folder, this._shim.Repository, from.StartTime, to.StartTime));
            ProcessResult result = this.ExecuteWithRetries(this.ForHistoryProcessInfo(from, to));
            Modification[] itemModifications = this.ParseModifications(result, from.StartTime, to.StartTime);
            ...
            {
                if (int.Parse(mod.ChangeNumber, CultureInfo.CurrentCulture) <= this._lastTxID)
                    modList.Add(mod);
            }
        ...
        private bool LookForChangesUsingVersionHistory(IIntegrationResult from, IIntegrationResult to)
        {
            ...
            bool bFoundChanges = this.GetFolderVersion(from, to);
        ...
        public override void GetSource(IIntegrationResult result)
        {
            ...
            Debug.Assert(this._folderVersion > 0, "_folderVersion <= 0 when attempting to get source.  This shouldn't happen.");
            //todo remove all debug.assert statements, replace by throw exception

            if (this._shim.CleanCopy)
            {
                string cleanCopyWorkingFolder = null;
                if (string.IsNullOrEmpty(this._shim.WorkingDirectory))
                {
                    cleanCopyWorkingFolder = this.GetVaultWorkingFolder(result);
                    ...
                    cleanCopyWorkingFolder = result.BaseFromWorkingDirectory(this._shim.WorkingDirectory);

                Log.Debug("Cleaning out source folder: " + cleanCopyWorkingFolder);
                this.fileDirectoryDeleter.DeleteIncludingReadOnlyObjects(cleanCopyWorkingFolder);

                System.IO.Directory.CreateDirectory(cleanCopyWorkingFolder);
            }

            Log.Info("Getting source from Vault");
            this.Execute(this.GetSourceProcessInfo(result));
        }
        ...
        public override void LabelSourceControl(IIntegrationResult result)
        {
            ...
            if (!this._shim.ApplyLabel || result.Status != IntegrationStatus.Success) return;

            Debug.Assert(this._folderVersion > 0, "_folderVersion <= 0 when attempting to label.  This shouldn't happen.");
            ...
            Log.Info(string.Format(System.Globalization.CultureInfo.CurrentCulture,"Applying label \"{0}\" to version {1} of {2} in repository {3}.",
                                   result.Label, this._folderVersion, this._shim.Folder, this._shim.Repository));
        ...
        private ProcessInfo LabelProcessInfo(IIntegrationResult result)
        {
            ...
            builder.Add(this._folderVersion);
        ...
        private bool GetFolderVersion(IIntegrationResult from, IIntegrationResult to)
        {
            ...
            bool bForceGetLatestVersion = (this._folderVersion == 0);

            // get version history
            ProcessResult result = this.ExecuteWithRetries(this.VersionHistoryProcessInfo(from, to, bForceGetLatestVersion));
            ...
            if (folderVersionNode != null)
            {
                if (bForceGetLatestVersion)
                {
                    ...
                    DateTime dtLastChange = DateTime.Parse(dateAttr.Value, this.culture);
                ...
                this._folderVersion = long.Parse(versionAttr.Value, CultureInfo.CurrentCulture);
                Log.Debug("Most recent folder version: " + this._folderVersion);
                ...
                this._lastTxID = long.Parse(txIdAttr.Value, CultureInfo.CurrentCulture);
                Log.Debug("Most recent TxID: " + this._lastTxID);
            }
        ...
        {
            var builder = new PrivateArguments();
            builder.Add("versionhistory ", this._shim.Folder);
            ...
            if (!bForceGetLatestVersion)
            {
                ...
                if (this._folderVersion != 0)
                {
                    builder.Add("-beginversion ", (this._folderVersion + 1).ToString(CultureInfo.CurrentCulture));
                }
                else
                {
            ...
            this.AddCommonOptionalArguments(builder);
            return this.ProcessInfoFor(builder, from);
        }
        ...
        private ProcessInfo GetSourceProcessInfo(IIntegrationResult result)
        {
            ...
            builder.Add("getversion ", this._folderVersion.ToString(CultureInfo.CurrentCulture));
            builder.Add(null, this._shim.Folder, true);

            if (!string.IsNullOrEmpty(this._shim.WorkingDirectory))
            {
                builder.Add(null, result.BaseFromWorkingDirectory(this._shim.WorkingDirectory), true);
