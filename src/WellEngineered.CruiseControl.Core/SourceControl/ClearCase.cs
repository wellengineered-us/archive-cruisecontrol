...
namespace WellEngineered.CruiseControl.Core.SourceControl
{
	...
	public class ClearCase : ProcessSourceControl
	{
		...
		{
            Modification[] modifications = base.GetModifications(this.CreateHistoryProcessInfo(from.StartTime, to.StartTime), from.StartTime, to.StartTime);
		...
		public override void LabelSourceControl(IIntegrationResult result)
		{
			if (result.Succeeded)
			{
				if (this.UseBaseline)
				{
					this.RenameBaseline(result.Label);
				}
				if (this.UseLabel)
				{
					ProcessResult processResult = base.Execute(this.CreateLabelTypeProcessInfo(result.Label));
					Log.Debug("standard output from label: " + processResult.StandardOutput);
					this.ExecuteIgnoreNonVobObjects(this.CreateMakeLabelProcessInfo(result.Label));
				}
			}
			else
			{
				this.DeleteTemporaryLabel();
			}
		}
		...
		private void CreateTemporaryLabel()
		{
			if (this.UseBaseline)
			{
				this.TempBaseline = this.CreateTemporaryBaselineName();
				this.ValidateBaselineConfiguration();
				base.Execute(this.CreateTempBaselineProcessInfo(this.TempBaseline));
			}
		}
		...
		{
			string args = string.Format(System.Globalization.CultureInfo.CurrentCulture,"mkbl -view {0} -identical {1}", this.ViewName, name);
			Log.Debug(string.Format(System.Globalization.CultureInfo.CurrentCulture,"command line is: {0} {1}", this.Executable, args));
		...
		{
			info.TimeOut = this.Timeout.Millis;
			...
			else if (result.Failed && this.HasFatalError(result.StandardError))
			{
		...
		public ProcessInfo CreateHistoryProcessInfo(DateTime from, DateTime to)
		{
			...
			string args = this.CreateHistoryArguments(fromDate);
			Log.Debug(string.Format(System.Globalization.CultureInfo.CurrentCulture,"cleartool commandline: {0} {1}", this.Executable, args));
			ProcessInfo processInfo = new ProcessInfo(this.Executable, args);
			return processInfo;
		}
		...
		public ProcessInfo CreateLabelTypeProcessInfo(string label)
		{
			...
			Log.Debug(string.Format(System.Globalization.CultureInfo.CurrentCulture,"mklbtype: {0} {1}; [working dir: {2}]", this.Executable, args, this.ViewPath));
			return new ProcessInfo(this.Executable, args, this.ViewPath);
		}
		...
		{
			string args = string.Format(CultureInfo.CurrentCulture, @" mklabel -recurse ""{0}"" ""{1}""", label, this.ViewPath);
			Log.Debug(string.Format(System.Globalization.CultureInfo.CurrentCulture,"mklabel: {0} {1}", this.Executable, args));
			return new ProcessInfo(this.Executable, args);
		}
		...
		{
			string args = string.Format(System.Globalization.CultureInfo.CurrentCulture,"rmbl -force {0}@\\{1}", this.TempBaseline, this.ProjectVobName);
			Log.Debug(string.Format(System.Globalization.CultureInfo.CurrentCulture,"remove baseline: {0} {1}", this.Executable, args));
			return new ProcessInfo(this.Executable, args);
		}
		...
		{
			string args = string.Format(System.Globalization.CultureInfo.CurrentCulture,"rename baseline:{0}@\\{1} \"{2}\"", this.TempBaseline, this.ProjectVobName, name);
			Log.Debug(string.Format(System.Globalization.CultureInfo.CurrentCulture,"rename baseline: {0} {1}", this.Executable, args));
			return new ProcessInfo(this.Executable, args);
		}
		...
		private string CreateHistoryArguments(string fromDate)
		{
			...
			builder.AppendIf(this.Branch != null, "-branch \"{0}\"", this.Branch);
			...
			builder.AppendArgument("\"{0}\"", this.ViewPath);
			return builder.ToString();
		}

		private void RemoveBaseline()
		{
			base.Execute(this.CreateRemoveBaselineProcessInfo());
		}

		private void RenameBaseline(string name)
		{
			this.ValidateBaselineConfiguration();
			this.ValidateBaselineName(name);
			base.Execute(this.CreateRenameBaselineProcessInfo(name));
		}
		...
		public override void GetSource(IIntegrationResult result)
		{
			...
			this.CreateTemporaryLabel();
			if (this.AutoGetSource)
			{
				ProcessInfo info = new ProcessInfo(this.Executable, this.BuildGetSourceArguments());
		...
		{
			return string.Format(CultureInfo.CurrentCulture, @"update -force -overwrite ""{0}""", this.ViewPath);
		}
	}
}