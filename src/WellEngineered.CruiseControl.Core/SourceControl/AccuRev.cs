...
namespace WellEngineered.CruiseControl.Core.SourceControl
{
	...
	public class AccuRev : ProcessSourceControl
    {
		...
		{
			if (this.LogIn & ((this.AccuRevPrincipal == null) || (this.AccuRevPassword == null)))
			{
		...
		private string GetBasisStreamName(IIntegrationResult result)
		{
			...
			this.PossiblyLogIn(result);
			ProcessResult cmdResults = this.RunCommand("info", result);
		...
		{
			this.PossiblyLogIn(from);
			string args = string.Format(System.Globalization.CultureInfo.CurrentCulture,"hist -a -s \"{0}\" -t \"{1}-{2}\"", 
				this.GetBasisStreamName(to),
				FormatCommandDate(to.StartTime),
				FormatCommandDate(from.StartTime));
			ProcessInfo histCommand = this.PrepCommand(args, from);

            this.mods = base.GetModifications(histCommand, from.StartTime, to.StartTime);
            base.FillIssueUrl(this.mods);
            return this.mods;
		}
		...
		public override void GetSource(IIntegrationResult result)
		{
			...
			if (this.AutoGetSource)
			{
				...
				this.PossiblyLogIn(result);
                var lastChangeNumber = Modification.GetLastChangeNumber(this.mods);
		...
		{
			if (this.LabelOnSuccess && result.Succeeded && (!(result.Label != null && result.Label.Length == 0)))
			{
				this.PossiblyLogIn(result);
				string args = string.Format(System.Globalization.CultureInfo.CurrentCulture,"mksnap -s \"{0}\" -b \"{1}\" -t \"{2}\"",
					result.Label,
					this.GetBasisStreamName(result),
					FormatCommandDate(result.StartTime));
				this.RunCommand(args, result);
			}
		}
        ...
        private void PossiblyLogIn(IIntegrationResult result)
		{
			...
			if ((this.AccuRevPrincipal == null) || (this.AccuRevPassword == null))
			{
				Log.Error("login=true requires principal= and password= to be specified.");
				return;
			}
            this.RunCommand(new PrivateArguments("login", this.AccuRevPrincipal, this.AccuRevPassword), result);
		}
		...
		{
			Log.Debug(string.Format(System.Globalization.CultureInfo.CurrentCulture,"Preparing to run AccuRev command: {0} {1}", this.Executable, args));
			ProcessInfo command = new ProcessInfo(this.Executable, args, result.BaseFromWorkingDirectory(this.Workspace));
			this.SetEnvironmentVariables(command.EnvironmentVariables, result);
			return command;
		}
		...
		{
			ProcessInfo command = this.PrepCommand(args, result);
			ProcessResult cmdResults = this.Execute(command);
			if (cmdResults.Failed)
			{
				Log.Error(string.Format(System.Globalization.CultureInfo.CurrentCulture,"AccuRev command \"{0} {1}\" failed with RC={2}", 
					this.Executable, args, cmdResults.ExitCode));
		...
		{
            if (!string.IsNullOrEmpty(this.AccuRevHomeDir))
				environmentVariables["ACCUREV_HOME"] = result.BaseFromArtifactsDirectory(this.AccuRevHomeDir);
            if (!string.IsNullOrEmpty(this.AccuRevPrincipal))
				environmentVariables["ACCUREV_PRINCIPAL"] = this.AccuRevPrincipal;
		}
	}
}
