...
namespace WellEngineered.CruiseControl.Core.SourceControl
{
        ...
        : SourceControlBase
	{
		...
		public Timeout Timeout
		{
			...
			set { this.timeout = (value == null) ? Timeout.DefaultTimeout : value; }
		}
		...
		{
			ProcessResult result = this.Execute(info);
			return this.ParseModifications(result, from, to);
		}
		...
		{
			return this.ParseModifications(new StringReader(result.StandardOutput), from, to);
		}
        ...
        {
            var mods = this.ParseModifications(new StringReader(result.StandardOutput), DateTime.MinValue, DateTime.MaxValue);
		...
		{
			return this.historyParser.Parse(reader, from, to);
		}
        ...
        {
            if ((this.IssueUrlBuilder != null) && (modifications != null))
            {
