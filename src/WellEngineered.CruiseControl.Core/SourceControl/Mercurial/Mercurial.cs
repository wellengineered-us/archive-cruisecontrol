...
namespace WellEngineered.CruiseControl.Core.SourceControl.Mercurial
{
	...
	public class Mercurial : ProcessSourceControl
	{
			...
			: base(historyParser, executor)
		{
			...
			this.CommitterName = DefaultCommitterName;
			this.CommitModifications = false;
			this.CommitUntracked = false;
			this.ModificationsCommitMessage = DefaultModificationsCommitMessage;
			this.MultipleHeadsFail = false;
			this.TagOnSuccess = false;
			this.TagNameFormat = DefaultTagNameFormat;
			this.TagCommitMessage = DefaultTagCommitMessage;
			this.PurgeModifications = false;
			this.PushModifications = false;
			this.RevertModifications = false;

			this._fileSystem = fileSystem;
			this._fileDirectoryDeleter = fileDirectoryDeleter;
		}
		...
		{
			RepositoryStatus result = this.HgCreateLocalRepository(to);
			...
			int parent = this.GetSmallestParentId(to);
			int tip = this.GetTipId(to);
			...
			Modification[] modifications = this.ParseModifications(this.HgLog(to, parent + 1, tip), from.StartTime, to.StartTime);
		...
		public override void GetSource(IIntegrationResult result)
		{
			...
			if(this.MultipleHeadsFail)
			{
				this.CheckMultipleHeads(result);
			}
			
			if(this.RevertModifications)
			{
				this.HgRevert(result);
			}

			if(this.PurgeModifications)
			{
				this.HgPurge(result);
			}

			this.HgUpdate(result);
		...
		public override void LabelSourceControl(IIntegrationResult result)
		{
			...
			if(this.CommitModifications)
			{
				string modificationsCommitMessage = string.Format(CultureInfo.CurrentCulture, this.ModificationsCommitMessage, result.Label);
				this.HgCommit(this.CommitUntracked, modificationsCommitMessage, result);
			}

			if(this.TagOnSuccess)
			{
				string tagName = string.Format(CultureInfo.CurrentCulture, this.TagNameFormat, result.Label);
				string tagCommitMessage = string.Format(CultureInfo.CurrentCulture, this.TagCommitMessage, result.Label);
				this.HgTag(tagName, tagCommitMessage, result);
			}

			if(!string.IsNullOrEmpty(this.Repository) && (this.CommitModifications || this.TagOnSuccess) && this.PushModifications)
			{
				this.HgPush(result);
			}
		}
		...
		private RepositoryStatus HgCreateLocalRepository(IIntegrationResult result)
		{
			...
			if (!this._fileSystem.DirectoryExists(hgRepositoryDirectory))
			{
				...
				this._fileDirectoryDeleter.DeleteIncludingReadOnlyObjects(workingDirectory);
				return this.HgCreateLocalRepository(result);
			}
		...
		{
			string wd = this.BaseWorkingDirectory(result);
			this._fileSystem.EnsureFolderExists(wd);
			...
			ProcessInfo pi = this.NewProcessInfo(buffer.ToString(), result);
			pi.WorkingDirectory = Path.GetDirectoryName(wd.Trim().TrimEnd(Path.DirectorySeparatorChar));
			ProcessResult processResult = this.Execute(pi);
		...
		private void HgConfigureRepository(IIntegrationResult result)
		{
			string workingDirectory = this.BaseWorkingDirectory(result);
			string hgRepositoryDirectory = Path.Combine(workingDirectory, ".hg");
			this._fileSystem.EnsureFolderExists(hgRepositoryDirectory);
		...
		private ProcessResult HgParents(IIntegrationResult result)
		{
			...
			ProcessResult processResult = this.Execute(this.NewProcessInfo(buffer.ToString(), result));
		...
		private ProcessResult HgHeads(IIntegrationResult result)
		{
			...
			ProcessResult processResult = this.Execute(this.NewProcessInfo(buffer.ToString(), result, ProcessPriorityClass.Normal, new int[] { 0, 1}));
		...
		private ProcessResult HgTip(IIntegrationResult result)
		{
			...
			{
				buffer.AddArgument("-b", this.Branch);
				buffer.AddArgument("-r", this.Branch);
			}
			else
			{
		...
		private ProcessResult HgLog(IIntegrationResult to, int fromId, int toId)
		{
			...
			var bpi = this.GetBuildProgressInformation(to);
			bpi.SignalStartRunTask(string.Concat("hg ", buffer.ToString()));

			this.ProcessExecutor.ProcessOutput += this.ProcessExecutor_ProcessOutput;

			ProcessResult processResult = this.Execute(this.NewProcessInfo(buffer.ToString(), to));
		...
		private ProcessResult HgPull(IIntegrationResult result)
		{
			...
			buffer.AddArgument(this.Repository);
		...
		private ProcessResult HgUpdate(IIntegrationResult result)
		{
			...
			if(!string.IsNullOrEmpty(this.Branch))
			{
				buffer.AddArgument(this.Branch);
			}
		...
		private ProcessResult HgCommit(bool addRemove, string commitMessage, IIntegrationResult result)
		{
			...
			if(!string.IsNullOrEmpty(this.CommitterName))
			{
				buffer.AddArgument("-u", this.CommitterName);
			}
		...
		{
			string tip = this.HgTip(to).StandardOutput.Trim();
			int t;
		...
		{
			string parents = this.HgParents(to).StandardOutput.Trim().TrimEnd(':');
		...
		{
			string heads = this.HgHeads(to).StandardOutput.Trim().TrimEnd(':');
		...
		{
			return Path.GetFullPath(result.BaseFromWorkingDirectory(this.WorkingDirectory));
		}
		...
		{
			return this.NewProcessInfo(args, result, ProcessPriorityClass.Normal, new int[] {0});
		}
		...
		private ProcessInfo NewProcessInfo(string args, IIntegrationResult result, ProcessPriorityClass priority, int[] successExitCodes)
		{
			...
			var processInfo = new ProcessInfo(this.Executable, args, this.BaseWorkingDirectory(result), priority,
			                                  successExitCodes);
		...
		private void ProcessExecutor_ProcessOutput(object sender, ProcessOutputEventArgs args)
		{
			...
			this._buildProgressInformation.AddTaskInformation(args.Data);
		}
