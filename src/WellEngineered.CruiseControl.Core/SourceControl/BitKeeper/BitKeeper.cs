...
namespace WellEngineered.CruiseControl.Core.SourceControl.BitKeeper
{
	...
	public class BitKeeper : ProcessSourceControl
	{
		...
		{
			ProcessResult result = this.Execute(this.NewProcessInfo(this.BuildHistoryProcessArgs(), to));
		...
		public override void LabelSourceControl(IIntegrationResult result)
		{
			if (this.TagOnSuccess && result.Succeeded)
			{
				...
				this.Execute(this.NewProcessInfo(this.BuildPushProcessArgs(), result));
			}
		}
		...
		public override void GetSource(IIntegrationResult result)
		{
			...
			ProcessInfo info = this.NewProcessInfo(this.BuildGetSourceArguments(), result);
			...
			if (!(this.CloneTo != null && this.CloneTo.Length == 0)) this.CloneSource(result);
		}

		private void CloneSource(IIntegrationResult result)
		{
			string clonePath = this.CloneTo;
	
			// Prepend working directory if path was not absolute
			if (!Path.IsPathRooted(clonePath))
				clonePath = Path.Combine(this.WorkingDirectory, this.CloneTo);
			...
			ProcessInfo ctInfo = this.NewProcessInfo(this.BuildCloneToArguments(), result);
		...
		{
			return this.NewProcessInfo(this.BuildTagProcessArgs(result.Label), result);
		}
		...
		private string BuildCloneToArguments()
		{
			...
			buffer.AppendArgument(this.CloneTo);
			return buffer.ToString();
		}

		private ProcessInfo NewProcessInfo(string args, IIntegrationResult result)
		{
			ProcessInfo pi = new ProcessInfo(this.Executable, args, result.BaseFromWorkingDirectory(this.WorkingDirectory));
