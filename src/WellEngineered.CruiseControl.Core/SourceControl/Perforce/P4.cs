...
namespace WellEngineered.CruiseControl.Core.SourceControl.Perforce
{
        ...
        : SourceControlBase
	{
		...
		public P4()
		{
			...
			this.processInfoCreator = new P4ConfigProcessInfoCreator();
			this.p4Initializer = new ProcessP4Initializer(this.processExecutor, this.processInfoCreator);
			this.p4Purger = new ProcessP4Purger(this.processExecutor, this.processInfoCreator);
		...
		{
			return string.Format(System.Globalization.CultureInfo.CurrentCulture,"changes -s submitted {0}", this.GenerateRevisionsForView(from, to));
		}

		private string GenerateRevisionsForView(DateTime from, DateTime to)
		{
			StringBuilder args = new StringBuilder();
			foreach (string viewline in this.View.Split(','))
			{
				if (args.Length > 0) args.Append(' ');
				args.Append(viewline);
				if (from == DateTime.MinValue)
				{
					args.Append("@" + this.FormatDate(to));
				}
				else
				{
					args.Append(string.Format(System.Globalization.CultureInfo.CurrentCulture,"@{0},@{1}", this.FormatDate(from), this.FormatDate(to)));
				}
			}
		...
		{
			DateTime offsetDate = date.AddHours(this.TimeZoneOffset);
		...
		{
			return this.processInfoCreator.CreateProcessInfo(this, this.BuildModificationsCommandArguments(from, to));
		}
		...
		public virtual ProcessInfo CreateDescribeProcess(string changes)
		{
			...
			return this.processInfoCreator.CreateProcessInfo(this, "describe -s " + changes);
		}
        ...
        public override Modification[] GetModifications(IIntegrationResult from, IIntegrationResult to)
		{
			...
			ProcessInfo process = this.CreateChangeListProcess(from.StartTime, to.StartTime);
			string processResult = this.Execute(process, "GetModifications");
			...
			{
				process = this.CreateDescribeProcess(changes);
				Modification[] mods = parser.Parse(new StringReader(this.Execute(process, "GetModifications")), from.StartTime, to.StartTime);
                if (!string.IsNullOrEmpty(this.P4WebURLFormat))
				{
					foreach (Modification mod in mods)
					{
						mod.Url = string.Format(CultureInfo.CurrentCulture, this.P4WebURLFormat, mod.ChangeNumber);
					}
				}
                this.FillIssueUrl(mods);
				return mods;
			}
		}
        ...
        public override void LabelSourceControl(IIntegrationResult result)
		{
			if (this.ApplyLabel && result.Succeeded)
			{
				...
				{}
				ProcessInfo process = this.CreateLabelSpecificationProcess(result.Label);
				this.Execute(process, "LabelSourceControl");

                process = this.CreateLabelSyncProcess(result.Label);
                this.Execute(process, "LabelSourceControl");
			}
		}
		...
		private string ParseErrors(string processOutput, string proccessError)
		{
            ...
            if (!StringUtil.IsWhitespace(this.ErrorPattern))
            {
                foreach (Match errorMatch in Regex.Matches(processOutput, this.ErrorPattern, DEFAULT_REGEX_OPTIONS))
                {
                    if (!this.IsAcceptableError(errorMatch.Groups[0].Value))
                    {
        ...
        {
            foreach (string acceptableError in this.AcceptableErrors)
            {
		...
		{
			ProcessInfo processInfo = this.processInfoCreator.CreateProcessInfo(this, "label -i");
			processInfo.StandardInputContent = string.Format(System.Globalization.CultureInfo.CurrentCulture,"Label:	{0}\n\nDescription:\n	Created by CCNet\n\nOptions:	unlocked\n\nView:\n{1}", label, this.ViewForSpecificationsAsNewlineSeparatedString);
			return processInfo;
		}
		...
		private string ViewForSpecificationsAsNewlineSeparatedString
		{
			get
			{
				...
				foreach (string viewLine in this.ViewForSpecifications)
				{
					builder.Append(" ");
		...
		{
			get { return this.View.Replace(",", Environment.NewLine); }
		}

		private ProcessInfo CreateLabelSyncProcess(string label)
		{
			return this.processInfoCreator.CreateProcessInfo(this, "labelsync -l " + label);
		}
        ...
        public override void GetSource(IIntegrationResult result)
		{
			...
			{
                ProcessInfo process = this.processInfoCreator.CreateProcessInfo(this, this.CreateSyncCommandLine(result.StartTime));
                this.Execute(process, "GetSource");
			}
		}
        ...
        private string CreateSyncCommandLine(DateTime modificationsToDate)
		{
            ...
            foreach (string viewline in this.View.Split(','))
            {
                commandline.Append(' ').Append(viewline).Append('@').Append(this.FormatDate(modificationsToDate));
            }
		...
		protected virtual string Execute(ProcessInfo process, string description)
		{
			...
			ProcessResult result = this.processExecutor.Execute(process);
            string errorSummary = this.ParseErrors(result.StandardOutput, result.StandardError);
        ...
        public override void Initialize(IProject project)
		{
			...
			{
				this.p4Initializer.Initialize(this, project.Name, project.WorkingDirectory);
			}
			else
			{
				this.p4Initializer.Initialize(this, project.Name, this.WorkingDirectory);
			}
		}
        ...
        public override void Purge(IProject project)
		{
            if (string.IsNullOrEmpty(this.WorkingDirectory))
			{
				this.p4Purger.Purge(this, project.WorkingDirectory);
			}
			else
			{
				this.p4Purger.Purge(this, this.WorkingDirectory);
			}
		}
        ...
        {
            if (this.IssueUrlBuilder != null)
            {
                this.IssueUrlBuilder.SetupModification(modifications);
            }
        }
	}
}
