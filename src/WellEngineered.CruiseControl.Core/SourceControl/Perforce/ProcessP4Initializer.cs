...
namespace WellEngineered.CruiseControl.Core.SourceControl.Perforce
{
    ...
    public class ProcessP4Initializer : IP4Initializer
    {
        ...
        private static readonly Regex viewRegx = new Regex(@"^(//[^ ""]+|""//[^""]+"")$", RegexOptions.Compiled);
        ...
        {
            this.CheckWorkingDirectoryIsValid(workingDirectory);
            this.CheckViewIsValid(p4.ViewForSpecifications);
            this.CreateClientNameIfOneNotSet(p4, project);
            ProcessInfo processInfo = this.processInfoCreator.CreateProcessInfo(p4, "client -i");
            processInfo.StandardInputContent = this.CreateClientSpecification(p4, workingDirectory);
        ...
        private void CheckViewIsValid(string[] viewLines)
        {
            ...
            {
                this.CheckViewIsValid(viewLine);
            }
        }

        private void CheckViewIsValid(string view)
        {
            if (!viewRegx.IsMatch(view))
            {
                throw new CruiseControlException(string.Format(CultureInfo.CurrentCulture, @"[{0}] is not a valid view - it should start with '//'", view));
            }
        }
        ...
        {
            return string.Format(System.Globalization.CultureInfo.CurrentCulture, "Client: {0}\n\nRoot:   {1}\n\nView:\n{2}", p4.Client, workingDirectory, this.GenerateClientView(p4.ViewForSpecifications, p4.Client));
        }
