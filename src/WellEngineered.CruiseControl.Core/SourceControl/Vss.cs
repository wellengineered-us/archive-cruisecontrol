...
namespace WellEngineered.CruiseControl.Core.SourceControl
{
	...
	public class Vss : ProcessSourceControl
	{
		...
		public string Executable
		{
			get
			{
				if (this.executable == null)
					this.executable = this.GetExecutableFromRegistry();
		...
		public string SsDir
		{
			...
			set { this.ssDir = StringUtil.StripQuotes(value); }
		}
		...
		{
			get { return this.locale.ServerCulture; }
			set { this.locale.ServerCulture = value; }
		}
	    ...
	    public override Modification[] GetModifications(IIntegrationResult from, IIntegrationResult to)
		{
	        ...
	        return this.GetModifications(from, to, tempOutputFileName);
        }
        ...
        public Modification[] GetModifications(IIntegrationResult from, IIntegrationResult to, string tempOutputFileName)
        {
            try
            {
                this.Execute(this.CreateHistoryProcessInfo(from, to, tempOutputFileName));
                ...
                try
                {
                    ...
                    Modification[] modifications = this.ParseModifications(outputReader, from.StartTime, to.StartTime);
		...
		{
			if (! this.ApplyLabel) return;

			this.Execute(this.NewProcessInfoWith(this.LabelProcessInfoArgs(result), result));
			this.tempLabel = null;
		}
		...
		public override void GetSource(IIntegrationResult result)
		{
			...
			this.CreateTemporaryLabel(result);

			if (! this.AutoGetSource) return;

			Log.Info("Getting source from VSS");
			this.Execute(this.NewProcessInfoWith(this.GetSourceArgs(result), result));
		}

        private PrivateArguments GetSourceArgs(IIntegrationResult result)
		{
            var builder = new PrivateArguments();
			builder.Add("get ", this.Project + "/*?*", true);
			builder.Add(RecursiveCommandLineOption);
			builder.AddIf(this.ApplyLabel, "-VL", this.tempLabel);
			builder.AddIf(!this.AlwaysGetLatest, "-Vd", this.locale.FormatCommandDate(result.StartTime));
			this.AppendUsernameAndPassword(builder);
			builder.Add("-I-Y -W -GF- -GTM");
			builder.AddIf(this.CleanCopy, "-GWR");
			return builder;
		}

		private ProcessInfo CreateHistoryProcessInfo(IIntegrationResult from, IIntegrationResult to, string tempOutputFileName)
		{
			return this.NewProcessInfoWith(this.HistoryProcessInfoArgs(from.StartTime, to.StartTime, tempOutputFileName), to);
		}

        private PrivateArguments HistoryProcessInfoArgs(DateTime from, DateTime to, string tempOutputFileName)
		{
			var builder = new PrivateArguments();
			builder.Add("history ", this.Project, true);
			builder.Add(RecursiveCommandLineOption);
			builder.Add(string.Format(System.Globalization.CultureInfo.CurrentCulture,"-Vd{0}~{1}", this.locale.FormatCommandDate(to), this.locale.FormatCommandDate(from)));
			this.AppendUsernameAndPassword(builder);
			builder.Add("-I-Y");
		...
		private void CreateTemporaryLabel(IIntegrationResult result)
		{
			if (this.ApplyLabel)
			{
				this.tempLabel = this.CreateTemporaryLabelName(result.StartTime);
				this.LabelSourceControlWith(this.tempLabel, result);
			}
		}

		private void LabelSourceControlWith(string label, IIntegrationResult result)
		{
			this.Execute(this.NewProcessInfoWith(this.LabelProcessInfoArgs(label, null), result));
		}
        ...
        private PrivateArguments LabelProcessInfoArgs(IIntegrationResult result)
		{
			if (result.Succeeded)
			{
				return this.LabelProcessInfoArgs(result.Label, this.tempLabel);
			}
			else
			{
				return this.DeleteLabelProcessInfoArgs();
			}
		}

        private PrivateArguments DeleteLabelProcessInfoArgs()
		{
			return this.LabelProcessInfoArgs(string.Empty, this.tempLabel);
		}

		private PrivateArguments LabelProcessInfoArgs(string label, string oldLabel)
		{
            var builder = new PrivateArguments();
			builder.Add("label ", this.Project, true);
			builder.Add("-L", label);
		...
		{
			string comServerPath = this.registry.GetExpectedLocalMachineSubKeyValue(SS_REGISTRY_PATH, SS_REGISTRY_KEY);
		...
		{
			string workingDirectory = result.BaseFromWorkingDirectory(this.WorkingDirectory);
			if (! Directory.Exists(workingDirectory)) Directory.CreateDirectory(workingDirectory);

			ProcessInfo processInfo = new ProcessInfo(this.Executable, args, workingDirectory);
			if (this.SsDir != null)
			{
				processInfo.EnvironmentVariables[SS_DIR_KEY] = this.SsDir;
			}
			return processInfo;
		}
        ...
        private void AppendUsernameAndPassword(PrivateArguments builder)
		{
            ...
            {
                PrivateString userPlusPass = "\"-Y" + this.Username + "," + this.Password.PrivateValue + "\"";
