...
namespace WellEngineered.CruiseControl.Core.SourceControl
{
	...
	public class QuietPeriod : IQuietPeriod
	{
		...
		{
			get { return this.modificationDelay.TotalSeconds; }
			set { this.modificationDelay = TimeSpan.FromSeconds(value); }
		}
		...
		public Modification[] GetModifications(ISourceControl sourceControl, IIntegrationResult lastBuild, IIntegrationResult thisBuild)
		{
			for (;;)
			{
				Modification[] modifications = this.GetModificationsWithLogging(sourceControl, lastBuild, thisBuild);
				
				DateTime timeOfThisBuild = thisBuild.StartTime;
				DateTime timeOfLatestModification = this.GetMostRecentModificationDateTime(modifications);

				TimeSpan timeInTheFutureOfLatestModification = timeOfLatestModification - timeOfThisBuild;
				
				if (timeInTheFutureOfLatestModification > this.AmountOfTimeInTheFutureToWarnAboutFutureModifications)
				{
				...
				if (timeInTheFutureOfLatestModification > this.AmountOfTimeInTheFutureToSkipQuietPeriod)
				{
					Log.Warning(" -> because this is more than a minute in the future, quiet period processing has been skipped");
					return modifications;
				}
				
				DateTime endOfQuietPeriod = timeOfLatestModification + this.modificationDelay;
				TimeSpan waitRequiredToReachEndOfQuietPeriod = endOfQuietPeriod - timeOfThisBuild;
				
				if (waitRequiredToReachEndOfQuietPeriod < this.GracePeriodInWhichItIsNotWorthApplyingTheQuietPeriod)
					return modifications;
				...
				this.dtProvider.Sleep(waitRequiredToReachEndOfQuietPeriod);
				thisBuild.StartTime = this.dtProvider.Now;
			}
		}
		...
		private Modification[] GetModificationsWithLogging(ISourceControl sc, IIntegrationResult from, IIntegrationResult to)
		{
			...
			Log.Info(this.GetModificationsDetectedMessage(modifications));
			return modifications;
		}
