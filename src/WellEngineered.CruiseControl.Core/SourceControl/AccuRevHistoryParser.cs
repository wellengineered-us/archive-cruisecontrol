...
namespace WellEngineered.CruiseControl.Core.SourceControl
{
	...
	public class AccuRevHistoryParser : IHistoryParser
	{
		...
		public Modification[] Parse(TextReader history, DateTime from, DateTime to)
		{
			...
			this.modificationTemplate = null;
			...
			while ((line = history.ReadLine()) != null)
			{
                ...
                switch (firstToken)
                {
                    ...
                    case "#":
                        ...
                        // adjacent lines.
                        if (this.modificationTemplate.Comment != null)
                            this.modificationTemplate.Comment += Environment.NewLine;
                        Match commentText = commentTextPattern.Match(line);
                        if (commentText.Groups.Count != 0)
                            this.modificationTemplate.Comment += commentText.Groups[1].ToString();
                        break;
                    case "ancestor:":
                    case "type:":
                    case "":
			...
			Log.Debug(string.Format(System.Globalization.CultureInfo.CurrentCulture,"AccuRev reported {0} modifications", this.modificationList.Count));
			return this.modificationList.ToArray();
		}
		...
		private void ParseTransaction(string line)
		{
			...
			this.modificationTemplate = new Modification();				
			...
			if (parsed.Success)
			{
				...
				this.modificationTemplate.ChangeNumber = tokens[1].ToString();
				this.modificationTemplate.Type = tokens[2].ToString();
				this.modificationTemplate.ModifiedTime = DateTime.Parse(tokens[3].ToString(), CultureInfo.CurrentCulture);
				this.modificationTemplate.UserName = tokens[4].ToString();
            }
			else
			{
		...
		private void ParseFileLine(string line)
		{
			...
			if (results.Success)
			{
				...
				this.modificationTemplate.Version = tokens[7].ToString();
				this.modificationTemplate.FolderName = tokens[3].ToString();
				this.modificationTemplate.FileName = tokens[6].ToString();
				// Add this to the modification list if it's inside our time range.
				if ((this.modificationTemplate.ModifiedTime >= this.fromDateTime) && 
				    (this.modificationTemplate.ModifiedTime <= this.toDateTime))
					this.AddModification(this.modificationTemplate);
			}
			else
			{
		...
		private void AddModification(Modification template)
		{
			...
			this.modificationList.Add(entry);
		}
