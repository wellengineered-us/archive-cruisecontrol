...
namespace WellEngineered.CruiseControl.Core.SourceControl
{
	...
	public class RobocopySourceControl : ProcessSourceControl
	{
		...
		{
			string destinationDirectory = from.BaseFromWorkingDirectory(this.WorkingDirectory);

			ProcessArgumentBuilder builder = new ProcessArgumentBuilder();

			this.AddStandardArguments(builder, destinationDirectory);

			builder.AddArgument("/L");

			Modification[] modifications = this.GetModifications(new ProcessInfo(this.Executable, builder.ToString(), null, ProcessPriorityClass.Normal, successExitCodes), from.StartTime, to.StartTime);
		...
		public override void GetSource(IIntegrationResult result)
		{
			...
			{
				string destinationDirectory = result.BaseFromWorkingDirectory(this.WorkingDirectory);
		
				ProcessArgumentBuilder builder = new ProcessArgumentBuilder();

				this.AddStandardArguments(builder, destinationDirectory);

                this.Execute(new ProcessInfo(this.Executable, builder.ToString(), null, ProcessPriorityClass.Normal, successExitCodes));
			}
		}
		...
		{
			builder.AddArgument(this.RepositoryRoot);
			builder.AddArgument(destinationDirectory);
			builder.Append(standardArguments);
			builder.Append(this.AdditionalArguments);
		}
	}
}