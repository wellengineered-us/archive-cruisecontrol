    ...
    public class ExternalSourceControl : ProcessSourceControl
    {
        ...
        public override Modification[] GetModifications(IIntegrationResult from, IIntegrationResult to)
        {
            string args = string.Format(CultureInfo.CurrentCulture, @"GETMODS ""{0}"" ""{1}"" {2}",
                ...
                this.ArgString);
            ProcessInfo command = this.PrepCommand(this.Executable, args, from);
        ...
        public override void GetSource(IIntegrationResult result)
        {
            ...
            if (this.AutoGetSource)
            {
                string args = string.Format(CultureInfo.CurrentCulture, @"GETSOURCE ""{0}"" ""{1}"" {2}",
                    ...
                    this.ArgString);
                this.RunCommand(this.Executable, args, result);
            }
        }
        ...
        {
            Log.Debug(string.Format(System.Globalization.CultureInfo.CurrentCulture,"Preparing to run source control command: {0} {1}", this.Executable, args));
        ...
        {
            ProcessInfo command = this.PrepCommand(executable, args, result);
