...
namespace WellEngineered.CruiseControl.Core.SourceControl.Telelogic
{
	...
	public class SynergyCommand : ISynergyCommand
	{
		...
		public SynergyCommand(SynergyConnectionInfo connectionInfo, SynergyProjectInfo projectInfo)
		{
			...
			AppDomain.CurrentDomain.DomainUnload += new EventHandler(this.AppDomain_Unload);
			AppDomain.CurrentDomain.ProcessExit += new EventHandler(this.AppDomain_Unload);
		}
		...
		private void Open()
		{
			...
			if (! this.isOpen)
			{
				info = SynergyCommandBuilder.Start(this.connection, this.project);
				...
				lock (PadLock)
				{
					...
					result = this.executor.Execute(info);
					Log.Debug("Releasing lock to open a session");
				}

				if (result.TimedOut)
				{
					message = String.Format(CultureInfo.CurrentCulture, @"Synergy connection timed out after {0} seconds.", this.connection.Timeout);
				...
				if (result.Failed)
				{
					if (this.connection.PollingEnabled)
					{
						if (this.IsDatabaseProtected(result.StandardError, this.connection.Host, this.connection.Database))
						{
							// sleep the thread for timeout until the database is unprotected
							Log.Warning(string.Format(System.Globalization.CultureInfo.CurrentCulture,"Database {0} on Host {1} Is Protected.  Waiting 60 seconds to reconnect.", this.connection.Host, this.connection.Database));
							Thread.Sleep(new TimeSpan(0, 1, 0));

							// save the original timeout
							originalTimeout = this.connection.Timeout;
							...
							this.connection.Timeout = originalTimeout;
				...
				{
					this.connection.SessionId = temp.Trim();
					Log.Info(String.Concat("CCM_ADDR set to '", this.connection.SessionId, "'"));
				}
				else
				{
				...
				info = SynergyCommandBuilder.GetSubProjects(this.connection, this.project);
				info.EnvironmentVariables[SessionToken] = this.connection.SessionId;
				this.executor.Execute(info);
				info = SynergyCommandBuilder.SetProjectRelease(this.connection, this.project);
				info.EnvironmentVariables[SessionToken] = this.connection.SessionId;
				this.executor.Execute(info);
		...
		private void Close()
		{
			...
			if (this.isOpen)
            {
				info = SynergyCommandBuilder.Stop(this.connection);
                ...
                {
                    Thread.CurrentThread.Name = this.connection.SessionId;
                }
		...
		private void Initialize()
		{
			...
			try
			{
				...
				info = SynergyCommandBuilder.GetProjectFullName(this.connection, this.project);
				info.EnvironmentVariables[SessionToken] = this.connection.SessionId;
				result = this.Execute(info);
				temp = result.StandardOutput.Trim();
				this.project.ObjectName = temp;
			}
			catch (Exception inner)
			{
				temp = String.Concat(@"CM Synergy Project """, this.project.ProjectSpecification, @""" not found");
			...
			try
			{
				...
				info = SynergyCommandBuilder.GetWorkArea(this.connection, this.project);
				info.EnvironmentVariables[SessionToken] = this.connection.SessionId;
				result = this.Execute(info);

				this.project.WorkAreaPath = Path.GetFullPath(result.StandardOutput.Trim());
				if (! Directory.Exists(this.project.WorkAreaPath))
				{
					throw(new CruiseControlException(String.Concat("CM Synergy work area '", result.StandardOutput.Trim(), "' not found.")));
				}

				Log.Info(String.Concat(this.project.ProjectSpecification, " work area is '", this.project.WorkAreaPath, "'"));
			}
			catch (Exception inner)
			{
				temp = String.Concat(@"CM Synergy Work Area for Project """, this.project.ProjectSpecification, @""" could not be determined.");
				throw(new CruiseControlException(temp, inner));
			}
		}
		...
		private void ValidateSession()
		{
			...
			bool isValid = (null != this.connection.SessionId && this.connection.SessionId.Length > 0);
			...
			{
				ProcessInfo info = SynergyCommandBuilder.Heartbeat(this.connection);
				if (null != this.project && null != this.project.WorkAreaPath && this.project.WorkAreaPath.Length > 0)
				{
					info = new ProcessInfo(info.FileName, info.Arguments, this.project.WorkAreaPath);
				}

				// set the session id for ccm.exe to use
				if (null != this.connection && null != this.connection.SessionId && this.connection.SessionId.Length > 0)
				{
					info.EnvironmentVariables[SessionToken] = this.connection.SessionId;
				}
				...
				/* don't call this.Execute(), as it will cause an infinite loop 
                 * once this.ValidateSession is called. */
				ProcessResult result = this.executor.Execute(info);

				// reset the valid flag, if ccm status does not report the session token
				isValid = this.IsSessionAlive(result.StandardOutput, this.connection.SessionId, this.connection.Database);
		...
		{
			return this.Execute(processInfo, true);
		}
		...
		public ProcessResult Execute(ProcessInfo processInfo, bool failOnError)
		{
			...
			if (null != this.project && null != this.project.WorkAreaPath && this.project.WorkAreaPath.Length > 0)
			{
				processInfo = new ProcessInfo(processInfo.FileName, processInfo.Arguments, this.project.WorkAreaPath);
			}

			// set the session id for ccm.exe to use
			processInfo.EnvironmentVariables[SessionToken] = this.connection.SessionId;
			// always use invariant (EN-US) date/time formats
			processInfo.EnvironmentVariables[DateTimeFormat] = DateTimeFormat;

			// convert from seconds to milliseconds
			processInfo.TimeOut = this.connection.Timeout*1000;

			ProcessResult result = this.executor.Execute(processInfo);
			if (result.TimedOut)
			{
				string message = String.Format(CultureInfo.CurrentCulture, @"Synergy source control operation has timed out after {0} seconds. Process command: ""{1}"" {2}", this.connection.Timeout, processInfo.FileName, processInfo.PublicArguments);
				throw(new CruiseControlException(message));
			}
