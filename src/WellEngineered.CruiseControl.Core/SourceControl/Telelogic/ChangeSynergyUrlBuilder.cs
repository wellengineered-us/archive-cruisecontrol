	...
	public class ChangeSynergyUrlBuilder : IModificationUrlBuilder
	{
		...
		public string Username
		{
			...
			set
			{
				...
				{
					this.username = Environment.ExpandEnvironmentVariables(value);
				}
			}
		}
		...
		{
			get { return this.obfuscatedPassword; }
			set
			{
				if (null == value)
				{
				...
				{
					this.obfuscatedPassword = Environment.ExpandEnvironmentVariables(value);
					this.obfuscatedPassword = this.ObfuscatePassword(this.obfuscatedPassword);
				}
			}
		}
		...
		{
			if (null == this.Database || 0 == this.Database.Length)
		...
		public void SetupModification(Modification[] modifications)
		{
			...
			if (null != this.username && 0 != this.username.Length)
			{
				...
				taskUrl = String.Format(CultureInfo.CurrentCulture, trustedUrl, this.Url, HttpUtility.UrlEncode(this.Role), HttpUtility.UrlEncode(this.Database), HttpUtility.UrlEncode(this.username), HttpUtility.UrlEncode(this.obfuscatedPassword));
			}
			else
			{
				// we have to first UrlEncode any of the redirection url querystring parameters
				taskQuery = String.Format(CultureInfo.CurrentCulture, loginQuery, HttpUtility.UrlEncode(this.Role), HttpUtility.UrlEncode(this.Database));

				// then UrlEncode the resulting redirection querystring
				taskQuery = HttpUtility.UrlEncode(taskQuery);

				// create the login url
				taskUrl = String.Format(CultureInfo.CurrentCulture, loginUrl, this.Url, HttpUtility.UrlEncode(this.Role), HttpUtility.UrlEncode(this.Database));
		...
		private string ObfuscatePassword(string password)
		{
			...
			return (this.ObfuscatePassword(seed, password));
		}
	}
}