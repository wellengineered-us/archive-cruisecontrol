...
namespace WellEngineered.CruiseControl.Core.SourceControl.Telelogic
{
        ...
        : SourceControlBase, IDisposable
	{
		...
		private void OpenNewCommand()
		{
			...
			this.command = new SynergyCommand(this.connection, this.project);
		}
		...
		public IModificationUrlBuilder UrlBuilder
		{
			...
			set
			{
				...
				ChangeSynergyUrlBuilder temp = this.urlBuilder as ChangeSynergyUrlBuilder;
				// check if the cast succeeded
				if (null != temp)
				{
					// initialize the ChangeSynergy credentials 
					temp.SetCredentials(this.connection);
				}
			}
		}
        ...
        public override void GetSource(IIntegrationResult integration)
		{
			...
			ProcessInfo info = SynergyCommandBuilder.Reconfigure(this.connection, this.project);
			this.command.Execute(info);
			...
			this.project.LastReconfigureTime = this.GetReconfigureTime();
		}
		...
		{
			return this.GetModifications(from.LastModificationDate);
		}
		...
		private Modification[] GetModifications(DateTime from)
		{
			...
			if (this.project.TemplateEnabled)
			{
				// setup the project to reconfigure using the default template
				this.command.Execute(SynergyCommandBuilder.UseReconfigureTemplate(this.connection, this.project));
			}

			// refresh the query based folders in the reconfigure properties
			this.command.Execute(SynergyCommandBuilder.UpdateReconfigureProperites(this.connection, this.project));

			// this may fail, if a build was forced, and no changes were found
			ProcessResult result = this.command.Execute(SynergyCommandBuilder.GetNewTasks(this.connection, this.project, from), false);
			if (! result.Failed)
			{
				...
				result = this.command.Execute(SynergyCommandBuilder.GetTaskObjects(this.connection, this.project), false);

				if (! result.Failed)
				{
					// Get the path information for each object associated with the tasks
					result = this.command.Execute(SynergyCommandBuilder.GetObjectPaths(this.connection, this.project), false);
					if (! result.Failed)
					{
						modifications = this.parser.Parse(comments, result.StandardOutput, from);
            ...
            this.FillIssueUrl(modifications);
        ...
        public override void LabelSourceControl(IIntegrationResult result)
		{
            if (result.Succeeded)
            {
			    DateTime currentReconfigureTime = this.GetReconfigureTime();
			    if (currentReconfigureTime != this.project.LastReconfigureTime)
			    {
				    string message = String.Format(CultureInfo.CurrentCulture, @"Invalid project state.  Cannot add tasks to shared folder '{0}' because " + @"the integration project '{1}' was internally reconfigured at '{2}' " + @"and externally reconfigured at '{3}'.  Projects cannot be reconfigured " + @"during an integration run.", this.project.TaskFolder, this.project.ProjectSpecification, this.project.LastReconfigureTime, currentReconfigureTime);
                ...
                * We find these by passing the the maximum range of dates to GetModifications */
			    result.Modifications = this.GetModifications(DateTime.MinValue);
			    ...
			    if (null != result.Modifications && result.Modifications.Length > 0)
			    {
				    ...
				    this.command.Execute(SynergyCommandBuilder.AddLabelToTaskComment(this.connection, this.project, result));

				    // append tasks to the shared folder, if one was specified
				    if (SynergyProjectInfo.DefaultTaskFolder != this.project.TaskFolder)
				    {
					    // append those tasks in the selection set to the shared build folder
					    this.command.Execute(SynergyCommandBuilder.AddTasksToFolder(this.connection, this.project, result));
				    }
			    }

			    // create a baseline, if requested
			    if (this.project.BaseliningEnabled)
			    {
				    this.command.Execute(SynergyCommandBuilder.CreateBaseline(this.connection, this.project, result));
			    }
            }
		}
		...
		private void Reconcile()
		{
			...
			this.command.Execute(SynergyCommandBuilder.Heartbeat(this.connection));

			if (null != this.project.ReconcilePaths)
			{
				string fullPath;
				foreach (string path in this.project.ReconcilePaths)
				{
					// normalize the path
					if (! Path.IsPathRooted(path))
						fullPath = Path.Combine(this.project.WorkAreaPath, path);
					else
						fullPath = path;
					...
					this.command.Execute(SynergyCommandBuilder.Reconcile(this.connection, this.project, path));
				}
			}
		}
		...
		{
			// setup the project to reconfigure using the default template
			ProcessResult result = this.command.Execute(SynergyCommandBuilder.GetLastReconfigureTime(this.connection, this.project));
			try
			{
				return DateTime.Parse(result.StandardOutput.Trim(), this.connection.FormatProvider);
			}
