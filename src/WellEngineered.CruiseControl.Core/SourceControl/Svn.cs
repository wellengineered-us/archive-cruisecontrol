...
namespace WellEngineered.CruiseControl.Core.SourceControl
{
    ...
    public class Svn : ProcessSourceControl
    {
        ...
        private IList<string> ListObstructions(IIntegrationResult result)
        {
            ...
            var processResult = this.Execute(info);
        ...
        public override Modification[] GetModifications(IIntegrationResult from, IIntegrationResult to)
        {
            ...
            {
                ((SvnHistoryParser)this.historyParser).IntegrationStatusUnknown = true;
            }

            string wd = Path.GetFullPath(to.BaseFromWorkingDirectory(this.WorkingDirectory));

            if (this.WorkingFolderIsKnownAsSvnWorkingFolder(wd) )
            {
                if (this.CleanUp)
                {
                    this.Execute(this.CleanupWorkingCopy(to));
                }

                if (this.Revert)
                {
                    this.Execute(this.RevertWorkingCopy(to));
                }
            ...
            {
                Log.Warning(string.Format(System.Globalization.CultureInfo.CurrentCulture,"{0} is not a svn working folder", wd));
            }

            List<Modification> modifications = new List<Modification>();
            List<string> repositoryUrls = new List<string>();
            repositoryUrls.Add(this.TrunkUrl);

            if (this.CheckExternals)
            {
                ProcessResult resultOfSvnPropget = this.Execute(this.PropGetProcessInfo(to));
            ...
            foreach (string repositoryUrl in repositoryUrls)
            {
                ...
                if (this.UseRevsionNumbers)
                {
                    ...
                    lastRepositoryRevision = revisionData.ContainsKey(lastRepositoryRevisionName)
                        ...
                        : null;
                    ProcessResult result = this.Execute(this.NewHistoryProcessInfoFromRevision(lastRepositoryRevision, to, repositoryUrl));
                    modsInRepository = this.ParseModifications(result, lastRepositoryRevision);
                }
                else
                {
                    // Use use the date range
                    ProcessResult result = this.Execute(this.NewHistoryProcessInfo(from, to, repositoryUrl));
                    modsInRepository = this.ParseModifications(result, from.StartTime, to.StartTime);
                }
                ...
                if (repositoryUrl == this.TrunkUrl)
                {
                    this.latestRevision = int.Parse(lastRepositoryRevision ?? "0", CultureInfo.CurrentCulture);
                    revisionData[lastRepositoryRevisionName] = lastRepositoryRevision;
                }
            }

            this.mods = modifications.ToArray();
            if (this.UrlBuilder != null)
            {
                this.UrlBuilder.SetupModification(this.mods);
            }
            this.FillIssueUrl(this.mods);
        ...
        {
            if (this.TagOnSuccess && result.Succeeded)
            {
        ...
        private ProcessInfo PropGetProcessInfo(IIntegrationResult result)
        {
            ...
            buffer.AddIf(this.CheckExternalsRecursive, "-R");
            this.AppendCommonSwitches(buffer);
            buffer.Add("svn:externals");
            buffer.Add(this.TrunkUrl);
        ...
        private ProcessInfo RevertWorkingCopy(IIntegrationResult result)
        {
            ...
            buffer.Add(null, Path.GetFullPath(result.BaseFromWorkingDirectory(this.WorkingDirectory)), true);
        ...
        public override void GetSource(IIntegrationResult result)
        {
            ...
            if (this.DoesSvnDirectoryExist(result) && !this.CleanCopy)
            {
            ...
            else
            {
                if (this.CleanCopy)
                {
                    ...
                    {
                        this.DeleteSource(result.WorkingDirectory);
                    }
                    else
                    {
                        this.DeleteSource(this.WorkingDirectory);
                    }
                }
        ...
        private void CheckoutSource(IIntegrationResult result)
        {
			...
			var bpi = this.GetBuildProgressInformation(result);
        ...
        private ProcessInfo NewCheckoutProcessInfo(IIntegrationResult result)
        {
            ...
            buffer.Add(string.Empty, this.TrunkUrl, true);
            buffer.Add(null, Path.GetFullPath(result.BaseFromWorkingDirectory(this.WorkingDirectory)), true);
            this.AppendCommonSwitches(buffer);
            return this.NewProcessInfo(buffer, result);
        }
        ...
        {
            return this.SvnFolderFound(result.BaseFromWorkingDirectory(this.WorkingDirectory));
        }
        ...
        private bool SvnFolderFound(string fld)
        {
            ...
            if (this.fileSystem.DirectoryExists(svnDirectory) || this.fileSystem.DirectoryExists(underscoreSvnDirectory))
            {
                return true;
            }
            ...
            return this.SvnFolderFound(parent.FullName);
        }
        ...
        private ProcessInfo NewGetSourceProcessInfo(IIntegrationResult result)
        {
            ...
            AppendRevision(buffer, this.latestRevision);
            this.AppendCommonSwitches(buffer);
            if (this.ForceUpdate) buffer.Add("--force");
            return this.NewProcessInfo(buffer, result);
        }
        ...
        private ProcessInfo NewLabelProcessInfo(IIntegrationResult result)
        {
            ...
            buffer.Add(null, this.TagMessage(result.Label), true);
            buffer.Add(null, this.TagSource(result), true);
            buffer.Add(null, this.TagDestination(result.Label), true);
            // Do not use Modification.GetLastChangeNumber() here directly.
            if (!this.TagWorkingCopy)
                AppendRevision(buffer, this.latestRevision);
        ...
        private ProcessInfo NewHistoryProcessInfo(IIntegrationResult from, IIntegrationResult to, string url)
        {
            ...
            buffer.Add(string.Format(System.Globalization.CultureInfo.CurrentCulture,"-r \"{{{0}}}:{{{1}}}\"", this.FormatCommandDate(from.StartTime), this.FormatCommandDate(to.StartTime)));
            buffer.Add("--verbose --xml");
            this.AppendCommonSwitches(buffer, url != this.TrunkUrl);
        ...
        {
            return string.Format(System.Globalization.CultureInfo.CurrentCulture,"-m \"{0}\"", string.Format(CultureInfo.CurrentCulture, this.TagCommitMessage, label));

        }

        private string TagSource(IIntegrationResult result)
        {
            if ((Modification.GetLastChangeNumber(this.mods) == null) || this.TagWorkingCopy)
            {
                return Path.GetFullPath(result.BaseFromWorkingDirectory(this.WorkingDirectory)).TrimEnd(Path.DirectorySeparatorChar);
            }
            return this.TrunkUrl;
        }

        private string TagDestination(string label)
        {
            return string.Format(System.Globalization.CultureInfo.CurrentCulture,"{0}/{1}", this.TagBaseUrl, string.Format(CultureInfo.CurrentCulture, this.TagNameFormat, label));
        }
        ...
        private ProcessInfo NewProcessInfo(PrivateArguments args, IIntegrationResult result)
        {
            ...
            ProcessInfo processInfo = new ProcessInfo(this.Executable, args, workingDirectory);
