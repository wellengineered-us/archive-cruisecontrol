...
namespace WellEngineered.CruiseControl.Core.SourceControl
{
	...
	public class VaultHistoryParser : IHistoryParser
	{
		...
		public Modification[] Parse(TextReader history, DateTime from, DateTime to)
		{
			...
			xml.LoadXml(this.ExtractXmlFromHistory(history));
			XmlNode parent = xml.SelectSingleNode("/vault/history");
			foreach (XmlNode node in parent.ChildNodes)
			{
				if (this.EntryWithinRange(node, from, to))
				{
					Modification modification = this.GetModification(node);
		...
		{
			DateTime date = DateTime.Parse(node.Attributes["date"].InnerText, this.culture);
		...
		private Modification GetModification(XmlNode node)
		{
			...
			modification.Type = this.GetTypeString(node.Attributes["type"].InnerText);
			modification.Comment = this.GetComment(node);
