...
namespace WellEngineered.CruiseControl.Core.SourceControl
{
        ...
        : SourceControlBase
	{
		...
		{
			get { return this._realScProvider; }
			set { this._realScProvider = value; }
		}
		...
		{
			Modification[] allModifications = this._realScProvider.GetModifications(from, to);
            var acceptedModifications = new List<Modification>();

			foreach (Modification modification in allModifications)
			{
                if (this.IsAcceptedByInclusionFilters(modification) &&
                    (!this.IsAcceptedByExclusionFilters(modification)))
                {
		...
		{
			this._realScProvider.LabelSourceControl(result);
		}
		...
		{
			this._realScProvider.GetSource(result);
		}
		...
		{
            this._realScProvider.Initialize(project);
		}
		...
		{
             this._realScProvider.Purge(project);
		}
		...
		{
			if (this._inclusionFilters.Length == 0)
				return true;

			foreach (IModificationFilter mf in this._inclusionFilters)
			{
				if (mf.Accept(m))
                {
		...
		{
            if (this._exclusionFilters.Length == 0)
				return false;

			foreach (IModificationFilter mf in this._exclusionFilters)
			{
                if (mf.Accept(m))
                {
        ...
        public override void ApplyParameters(Dictionary<string, string> parameters, IEnumerable<ParameterBase> parameterDefinitions)
        {
            ...
            var dynamicChild = this._realScProvider as IParamatisedItem;
