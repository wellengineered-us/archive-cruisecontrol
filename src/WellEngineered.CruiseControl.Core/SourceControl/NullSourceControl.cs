...
namespace WellEngineered.CruiseControl.Core.SourceControl
{
	...
	public class NullSourceControl : ISourceControl
	{
		...
		{
            if (this.FailGetModifications)
            {
                throw new System.Exception("Failing GetModifications");
            }
            else if (this.AlwaysModified)
            {
                ...
                if (string.Equals(this.ChangeNumberType, "guid", StringComparison.CurrentCultureIgnoreCase))
                { mod.ChangeNumber = Guid.NewGuid().ToString("N"); }

                if (string.Equals(this.ChangeNumberType, "integer", StringComparison.CurrentCultureIgnoreCase))
                { mod.ChangeNumber = this.revisionNumber.ToString(); }
		...
		{
            if (this.FailLabelSourceControl)
            {
