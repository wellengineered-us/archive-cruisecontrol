...
//			dfi.MonthDayPattern = @"M/d/yy h:mm:ss tt";
		...
		{
			Regex folderRegex = new Regex(this.starTeamRegExProvider.FolderRegEx);
			Regex fileRegex = new Regex(this.starTeamRegExProvider.FileRegEx);
			Regex historyRegex = new Regex(this.starTeamRegExProvider.FileHistoryRegEx);
			...
			for (Match mFolder = folderRegex.Match(s); mFolder.Success; mFolder = mFolder.NextMatch())
			{
				...
				for (Match mFile = fileRegex.Match(mFolder.Value); mFile.Success; mFile = mFile.NextMatch())
				{
					...
					if (mHistory.Success)
					{
						...
						mod.ModifiedTime = DateTime.Parse(mHistory.Result("${date_string}"), this.Culture.DateTimeFormat);
