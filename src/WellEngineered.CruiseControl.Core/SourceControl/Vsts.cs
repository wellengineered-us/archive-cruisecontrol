...
namespace WellEngineered.CruiseControl.Core.SourceControl
{
    ...
    public class Vsts : ProcessSourceControl, IConfigurationValidation
    {
        ...
        {
            get { return this.executable ?? (this.executable = this.ReadTfFromRegistry()); }
        ...
        {
            if (!this.ProjectExists(from))
            {
                Log.Error(string.Format(System.Globalization.CultureInfo.CurrentCulture, "[TFS] Project {0} is not valid on {1} TFS server", this.ProjectPath, this.Server));
            ...
            ProcessResult result = this.executor.Execute(this.NewHistoryProcessInfo(from, to));

            LookForErrorReturns(result);

            return this.ParseModifications(result, from.StartTime, to.StartTime);
        }
        ...
        {
            if (this.ApplyLabel && result.Succeeded)
            {
                LookForErrorReturns(this.executor.Execute(this.NewLabelProcessInfo(result)));
            }
        }
        ...
        {
            if (!this.AutoGetSource || !this.ProjectExists(result)) return;
            ...
            TfsWorkspaceStatus workspaceStatus = this.GetWorkspaceStatus(result);
            ...
            if (workspaceStatus.WorkspaceExists)
            {
                if (this.DeleteWorkspace)
                {
                    ...
                    Log.Debug("[TFS] Removing existing workspace " + this.Workspace);
                    LookForErrorReturns(this.executor.Execute(this.DeleteWorkSpaceProcessInfo(result)));

                    //Create Workspace
                    Log.Debug("[TFS] Creating New Workspace " + this.Workspace);
                    LookForErrorReturns(this.executor.Execute(this.CreateWorkSpaceProcessInfo(result)));

                    //Map Workspace
                    Log.Debug(string.Format(System.Globalization.CultureInfo.CurrentCulture, "[TFS] Mapping Workspace {0} to {1}", this.Workspace, this.WorkingDirectory));
                    LookForErrorReturns(this.executor.Execute(this.MapWorkSpaceProcessInfo(result)));
                }
            }
            else
            {
            ...
            Log.Debug("[TFS] Getting Files in " + this.Workspace);
            ProcessInfo pi = this.GetWorkSpaceProcessInfo(result);
            pi.TimeOut = 3600000;
            LookForErrorReturns(this.executor.Execute(pi));
        }
        ...
        {
            //Check for  Workspace
            Log.Debug(string.Format(System.Globalization.CultureInfo.CurrentCulture, "[TFS] Checking if Project {0} exists", this.ProjectPath));
            ProcessResult pr = this.executor.Execute(this.CheckProjectProcessInfo(result));
        ...
        {
            Log.Debug(string.Format(System.Globalization.CultureInfo.CurrentCulture, "[TFS] Fetching Workspace {0} details", this.Workspace));
            ProcessResult pr = this.executor.Execute(this.CheckWorkSpaceProcessInfo(result));

            LookForErrorReturns(pr);

            TfsWorkspaceStatus status = new TfsWorkspaceStatus();
            status.WorkspaceIsMappedCorrectly = pr.StandardOutput.Contains(this.ProjectPath + ": " + this.WorkingDirectory);
        ...
        private ProcessInfo CheckProjectProcessInfo(IIntegrationResult result)
        {
            ...
            buffer.Add("/server:", this.Server);
            buffer.AddQuote(this.ProjectPath);

            this.AppendSourceControlAuthentication(buffer);

            return this.NewProcessInfo(buffer, result);
        }
        ...
        private ProcessInfo MapWorkSpaceProcessInfo(IIntegrationResult result)
        {
            ...
            buffer.AddQuote(this.ProjectPath);
            buffer.AddQuote(this.WorkingDirectory);
            buffer.Add("/server:", this.Server);
        ...
        private ProcessInfo CreateWorkSpaceProcessInfo(IIntegrationResult result)
        {
            ...
            buffer.Add("/server:", this.Server);
            buffer.AddQuote(this.Workspace);
        ...
        private ProcessInfo NewLabelProcessInfo(IIntegrationResult result)
        {
            ...
            buffer.Add("/server:", this.Server);
            buffer.AddQuote(result.Label, string.Format(System.Globalization.CultureInfo.CurrentCulture, "@{0}", this.ProjectPath));
            buffer.AddQuote(this.WorkingDirectory);
        ...
        private ProcessInfo NewHistoryProcessInfo(IIntegrationResult from, IIntegrationResult to)
        {
            ...
            return this.NewProcessInfo(buffer, to);
        }
        ...
        {
            if (!string.IsNullOrEmpty(this.Username) && !string.IsNullOrEmpty(this.Password.PrivateValue))
                buffer.Add("/login:" + this.BuildTfsAuthenticationString());
        }

        private string BuildTfsAuthenticationString()
        {
            return this.BuildTfsUsername() + "," + this.Password.ToString(SecureDataMode.Private);
        }
        ...
        {
            string workingDirectory = Path.GetFullPath(result.BaseFromWorkingDirectory(this.WorkingDirectory));
            if (!Directory.Exists(workingDirectory)) Directory.CreateDirectory(workingDirectory);

            var processInfo = new ProcessInfo(this.Executable, args, workingDirectory);
            processInfo.StreamEncoding = Encoding.UTF8;

            if (!string.IsNullOrEmpty(this.CodePage))
            {
                int codePage;
                if (int.TryParse(this.CodePage, out codePage))
                {
                    processInfo.StreamEncoding = Encoding.GetEncoding(codePage);
                }
                else
                {
                    throw new CruiseControlException(string.Format(System.Globalization.CultureInfo.CurrentCulture, "Code page {0} could not be parsed to an encoding via instruction : Encoding.GetEncoding(codePage)", this.CodePage));
                }
            }
        ...
        {
            string registryValue = this.registry.GetLocalMachineSubKeyValue(VS2015_64_REGISTRY_PATH, VS_REGISTRY_KEY);

            if (registryValue == null)
            {
                registryValue = this.registry.GetLocalMachineSubKeyValue(VS2013_64_REGISTRY_PATH, VS_REGISTRY_KEY);
            }


            if (registryValue == null)
            {
                registryValue = this.registry.GetLocalMachineSubKeyValue(VS2012_64_REGISTRY_PATH, VS_REGISTRY_KEY);
            }

            if (registryValue == null)
            {
                registryValue = this.registry.GetLocalMachineSubKeyValue(VS2010_64_REGISTRY_PATH, VS_REGISTRY_KEY);
            }

            if (registryValue == null)
            {
                registryValue = this.registry.GetLocalMachineSubKeyValue(VS2008_64_REGISTRY_PATH, VS_REGISTRY_KEY);
            }

            if (registryValue == null)
            {
                registryValue = this.registry.GetLocalMachineSubKeyValue(VS2005_64_REGISTRY_PATH, VS_REGISTRY_KEY);
            }

            if (registryValue == null)
            {
                registryValue = this.registry.GetLocalMachineSubKeyValue(VS2015_32_REGISTRY_PATH, VS_REGISTRY_KEY);
            }

            if (registryValue == null)
            {
                registryValue = this.registry.GetLocalMachineSubKeyValue(VS2013_32_REGISTRY_PATH, VS_REGISTRY_KEY);
            }

            if (registryValue == null)
            {
                registryValue = this.registry.GetLocalMachineSubKeyValue(VS2012_32_REGISTRY_PATH, VS_REGISTRY_KEY);
            }

            if (registryValue == null)
            {
                registryValue = this.registry.GetLocalMachineSubKeyValue(VS2010_32_REGISTRY_PATH, VS_REGISTRY_KEY);
            }

            if (registryValue == null)
            {
                registryValue = this.registry.GetLocalMachineSubKeyValue(VS2008_32_REGISTRY_PATH, VS_REGISTRY_KEY);
            }

            if (registryValue == null)
            {
                registryValue = this.registry.GetLocalMachineSubKeyValue(VS2005_32_REGISTRY_PATH, VS_REGISTRY_KEY);
            }
        ...
        public void Validate(IConfiguration configuration, ConfigurationTrace parent, IConfigurationErrorProcesser errorProcesser)
        {
            ...
            if (this.workspaceName.Length > 64)
            {
                AllOk = false;
                ErrorInfo += System.Environment.NewLine + "Max length is 64 chars";
            }
            if (this.workspaceName.EndsWith(" "))
            {
                AllOk = false;
            ...
            {
                if (this.workspaceName.Contains(s))
                {
                    AllOk = false;
