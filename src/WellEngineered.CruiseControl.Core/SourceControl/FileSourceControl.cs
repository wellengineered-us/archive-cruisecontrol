...
namespace WellEngineered.CruiseControl.Core.SourceControl
{
        ...
        : SourceControlBase
    {
        ...
        {
            DirectoryInfo root = new DirectoryInfo(from.BaseFromWorkingDirectory(this.RepositoryRoot));
            var modifications = this.GetMods(root, from.StartTime);
        ...
        private List<Modification> GetMods(DirectoryInfo dir, DateTime from)
        {
            ...
            try
            {
                ...
                {
                    if (this.IsLocalFileChanged(file, from))
                    {
                        mods.Add(this.CreateModification(file));
                    }
                }

                foreach (DirectoryInfo sub in dir.GetDirectories())
                {
                    if (this.IsLocalFileChanged(sub, from))
                    {
                        mods.Add(this.CreateModification(sub));
                    }

                    if (this.CheckRecursively)
                    {
                        mods.AddRange(this.GetMods(sub, from));
                    }
                }
            }
        ...
        {
            Modification result = this.CreateModificationBase(info);
        ...
        public override void GetSource(IIntegrationResult result)
        {
            ...
            if (this.AutoGetSource)
                this.fileSystem.Copy(this.RepositoryRoot, result.WorkingDirectory);
        }
