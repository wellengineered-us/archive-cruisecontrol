...
namespace WellEngineered.CruiseControl.Core.SourceControl 
{
        ...
        : SourceControlBase
	{
		...
		{
			get { return this._vaultSourceControl; }
		}
		...
		{
            this.InitialiseDefaults();
            this._forcedVaultVersion = forceVersion;
			switch ( forceVersion )
			{
				case EForcedVaultVersion.Vault3:
					this._vaultSourceControl = new Vault3(this, historyParser, executor);
					break;
				case EForcedVaultVersion.Vault317:
					this._vaultSourceControl = new Vault317(this, historyParser, executor);
					break;
				default:
		...
		{
			this.GetCorrectVaultInstance();
			this.VaultSourceControl.Initialize(project);
		}
		...
		{
			this.GetCorrectVaultInstance();
			return this.VaultSourceControl.GetModifications(from, to);
		}
		...
		{
			this.GetCorrectVaultInstance();
			this.VaultSourceControl.LabelSourceControl(result);
		}
		...
		{
			this.GetCorrectVaultInstance();
			this.VaultSourceControl.GetSource(result);
		}
		...
		{
			this.GetCorrectVaultInstance();
			this.VaultSourceControl.Purge(project);
		}

		private void GetCorrectVaultInstance()
		{
			if ( this.VaultSourceControl != null )
				return;

			if ( this.VaultVersionIs317OrBetter() )
			{
				Log.Debug("Vault CLC is at least version 3.1.7.");
				this._vaultSourceControl = new Vault317(this);
			}
			else
			{
				Log.Debug("Vault CLC is older than version 3.1.7.");
				this._vaultSourceControl = new Vault3(this);
			}
		}

		private bool VaultVersionIs317OrBetter()
		{
			switch ( this._forcedVaultVersion )
			{
				case EForcedVaultVersion.Vault3:
			...
			Assembly vaultExe = Assembly.LoadFile(this.Executable);
			AssemblyName vaultExeName = vaultExe.GetName();

			Log.Debug(this.Executable + " is version " + vaultExeName.Version.ToString());
