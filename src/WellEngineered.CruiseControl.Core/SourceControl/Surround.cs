	...
	public class Surround : ProcessSourceControl
	{
		...
		public override Modification[] GetModifications(IIntegrationResult from, IIntegrationResult to)
		{
			string command = string.Format(System.Globalization.CultureInfo.CurrentCulture,"cc {0} -d{1}:{2} {3} -b{4} -p{5} {6} -z{7} -y{8}",
			                               ...
			                               (this.Recursive == 0) ?string.Empty : "-r",
			                               this.Branch,
			                               this.Repository,
			                               (this.SearchRegExp == 0) ? "-x-" : "-x",
			                               this.ServerConnect,
			                               this.ServerLogin);

            Modification[] modifications = this.GetModifications(this.CreateSSCMProcessInfo(command), from.StartTime, to.StartTime);
		...
		{
			return new ProcessInfo(this.Executable, command);
		}
		...
		{
			this.Execute(this.CreateSSCMProcessInfo("workdir " + this.WorkingDirectory + " " + this.Repository + " -z" + this.ServerConnect + " -y" + this.ServerLogin));
		}
		...
		public override void GetSource(IIntegrationResult result)
		{
			...
			string command = string.Format(System.Globalization.CultureInfo.CurrentCulture,"get * -q -tcheckin -wreplace {0} -d{1} -b{2} -p{3} -z{4} -y{5}",
			                               ...
			                               this.ServerConnect,
			                               this.ServerLogin);
			this.Execute(this.CreateSSCMProcessInfo(command));
		}
	}
}