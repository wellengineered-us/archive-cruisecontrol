...
namespace WellEngineered.CruiseControl.Core.SourceControl
{
	...
	public class Alienbrain : ProcessSourceControl
	{
		...
		{
			ProcessInfo processInfo = this.CreateModificationProcess(MODIFICATIONS_COMMAND_TEMPLATE, from.StartTime, to.StartTime);
			processInfo.TimeOut = this.Timeout.Millis;
		...
		public override void LabelSourceControl(IIntegrationResult result)
		{
			if (this.LabelOnSuccess && result.Succeeded)
			{
				...
				ProcessInfo process = this.CreateLabelProcess(LABEL_COMMAND_TEMPLATE, result);
		...
		public override void GetSource(IIntegrationResult result)
		{
			...
			if (this.AutoGetSource)
			{
				...
				if (result.Modifications.Length > 0)
				{
					...
					{
						ProcessInfo process = this.CreateGetProcess(m.FolderName + "/" + m.FileName);
						this.Execute(process);
					}
				}
				else
				{
					ProcessInfo process = this.CreateGetProcess();
					this.Execute(process);
				}
			}
		}
		...
		{
			string comServerPath = this.registry.GetExpectedLocalMachineSubKeyValue(AB_REGISTRY_PATH, AB_REGISTRY_KEY);
		...
		private void SelectBranch()
		{
			...
			{
				ProcessInfo process = this.CreateBranchProcess(BRANCH_COMMAND_TEMPLATE);
				this.Execute(process);
			}
		}
		...
		{
			ProcessResult result = this.Execute(processInfo);
		...
		{
			var arguments = String.Format(CultureInfo.CurrentCulture, processCommand, this.Project, this.Server, this.Database, this.Username, this.Password.PrivateValue, from.ToFileTime(), to.ToFileTime());
			return new ProcessInfo(this.Executable, arguments);
		}
		...
		{
			string arguments = String.Format(CultureInfo.CurrentCulture, processCommand, this.Project, this.Server, this.Database, this.Username, this.Password.PrivateValue, result.Label);
			return new ProcessInfo(this.Executable, arguments);
		}
		...
		{
			return this.CreateGetProcess(this.Project);
		}
		...
		public ProcessInfo CreateGetProcess(string filename)
		{
            ...
            args.Add("-s ", this.Server, true);
            args.Add("-d ", this.Database, true);
            args.Add("-u ", this.Username, true);
            args.Add("-p ", this.Password, true);
            args.AddIf(!string.IsNullOrEmpty(this.WorkingDirectory), "-localpath ", this.WorkingDirectory, true);
		...
		{
			string arguments = String.Format(CultureInfo.CurrentCulture, processCommand, this.Branch, this.Server, this.Database, this.Username, this.Password.PrivateValue);
			return new ProcessInfo(this.Executable, arguments);
		}
	}
}