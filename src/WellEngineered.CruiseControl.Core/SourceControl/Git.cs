...
namespace WellEngineered.CruiseControl.Core.SourceControl
{
    ...
    public class Git : ProcessSourceControl
    {
        ...
        public override Modification[] GetModifications(IIntegrationResult from, IIntegrationResult to)
        {
            ...
            RepositoryAction result = this.CreateUpateLocalRepository(to);
        ...
        public override void GetSource(IIntegrationResult result)
        {
            ...
            this.GitCheckoutRemoteBranch(this.Branch, result);

			// update submodules
			if (this.FetchSubmodules)
				this.GitUpdateSubmodules(result);

            // clean up the local working copy
			if (this.CleanUntrackedFiles)
				this.GitClean(result);
        }
        ...
        {
            if (!this.TagOnSuccess || result.Failed)
                return;

            string tagName = string.Format(CultureInfo.CurrentCulture, this.TagNameFormat, result.Label);
            string commitMessage = string.Format(CultureInfo.CurrentCulture, this.TagCommitMessage, result.Label);

            if (this.CommitBuildModifications)
            {
                // add all modified and all untracked files to the git index.
                if (this.CommitUntrackedFiles)
                    this.GitAddAll(result);

                // commit all modifications during build before tagging.
                this.GitCommitAll(commitMessage, result);
            }

            // create a tag and push it.
            this.GitCreateTag(tagName, commitMessage, result);
            this.GitPushTag(tagName, result);
        }
        ...
        private RepositoryAction CreateUpateLocalRepository(IIntegrationResult result)
        {
            ...
            if (!this._fileSystem.DirectoryExists(workingDirectory))
            {
                ...
                this.GitClone(result);

                // init submodules
                if (this.FetchSubmodules)
                    this.GitInitSubmodules(result);

                // setup some required configuration settings for the local repository
                this.SetupLocalRepository(result);
            ...
            if (!this._fileSystem.DirectoryExists(gitRepositoryDirectory))
            {
                ...
                return this.CreateUpateLocalRepository(result);
            }

            // we are in a local git repository, fetch the latest remote changes
            this.GitFetch(result);
        ...
        {
            if (!string.IsNullOrEmpty(this.CommitterName) && !string.IsNullOrEmpty(this.CommitterEMail))
            {
                this.GitConfigSet("user.name", this.CommitterName, result);
                this.GitConfigSet("user.email", this.CommitterEMail, result);
            }
            else if (string.IsNullOrEmpty(this.GitConfigGet("user.name", result)) || string.IsNullOrEmpty(this.GitConfigGet("user.email", result)))
            {
        ...
        private string GitLogOriginHash(string branchName, IIntegrationResult result)
        {
            ...
            return this.Execute(this.NewProcessInfo(buffer.ToString(), result)).StandardOutput.Trim();
        }
        ...
        private ProcessResult GitLogHistory(string branchName, string from, IIntegrationResult to)
        {
            ...
            this.AppendLogOptions(buffer);
            return this.Execute(this.NewProcessInfo(buffer.ToString(), to));
        }
        ...
        {
            if (this.MaxAmountOfModificationsToFetch > 0)
            {
                buffer.AddArgument(string.Concat("-", this.MaxAmountOfModificationsToFetch.ToString())); // The history may be very long. It may lead to timeout. That's why we need to limit it
            }
        ...
        private void GitConfigSet(string name, string value, IIntegrationResult result)
        {
            ...
            this.Execute(this.NewProcessInfo(buffer.ToString(), result));
        }
        ...
        private string GitConfigGet(string name, IIntegrationResult result)
        {
            ...
            buffer.AddArgument(name);
            return
                this.Execute(this.NewProcessInfo(buffer.ToString(), result, ProcessPriorityClass.Normal, new int[] { 0, 1, 2 })).
                    StandardOutput.Trim();
        }
