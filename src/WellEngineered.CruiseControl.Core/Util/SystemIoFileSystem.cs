	...
	public class SystemIoFileSystem : IFileSystem
	{
		...
		public void Copy(string sourcePath, string destPath)
		{
			...
			if (Directory.Exists(sourcePath))
				this.CopyDirectoryToDirectory(sourcePath, destPath);
			else
		...
		private void CopyDirectoryToDirectory(string sourcePath, string destPath)
		{
			...
			{
				this.CopyDirectoryToDirectory(subdir.FullName, Path.Combine(destPath, subdir.Name));
			}
			foreach (string file in Directory.GetFiles(sourcePath))
			{
				this.CopyFileToDirectory(Path.Combine(sourcePath, file), destPath);
			}
		}
		...
		private void CopyFile(string sourcePath, string destPath)
		{
			if (Directory.Exists(destPath))
				this.CopyFileToDirectory(sourcePath, destPath);
			else
				this.CopyFileToFile(sourcePath, destPath);
		}

		private void CopyFileToDirectory(string sourcePath, string destPath)
		{
			this.CopyFileToFile(sourcePath, Path.Combine(destPath, Path.GetFileName(sourcePath)));
		}
        ...
        {
            this.AtomicSave(file, content, Encoding.UTF8);
        }
        ...
        public void EnsureFolderExists(string fileName)
        {
            ...
            this.EnsureGivenFolderExists(directory);
        }
        #endregion
        ...
        {
            this.EnsureFolderExists(fileName);
