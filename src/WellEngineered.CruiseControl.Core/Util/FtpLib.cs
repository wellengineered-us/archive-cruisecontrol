...
namespace WellEngineered.CruiseControl.Core.Util
{
    ...
    public class FtpLib : IFtpLib
    {
        ...
        public FtpLib(Tasks.TaskBase callingTask, BuildProgressInformation buildProgressInformation)
        {
            ...
            this.bpi = buildProgressInformation;

            this.ftpServer = new FTPConnection();

            this.ftpServer.ReplyReceived += this.HandleMessages;

            this.ftpServer.CommandSent += this.HandleMessages;

            this.ftpServer.Downloaded += new FTPFileTransferEventHandler(this.FtpServer_Downloaded);

            this.ftpServer.Uploaded += new FTPFileTransferEventHandler(this.FtpServer_Uploaded);
        ...
        public System.TimeSpan TimeDifference
        {
            get
            {
                return this.ftpServer.TimeDifference;
            }
 
            set
            {
                this.ftpServer.TimeDifference = value;
            }
        }
        ...
        public void LogIn(string serverName, string userName, string password, bool activeConnectionMode)
        {
...

            {
                ...
                if (activeConnectionMode)
                {
                    ...
                    this.ftpServer.ConnectMode = FTPConnectMode.ACTIVE;
                }
                else
                {
                    Log.Trace("Passive mode enabled");
                    this.ftpServer.ConnectMode = FTPConnectMode.PASV;
                }

                this.ftpServer.TransferType = FTPTransferType.BINARY;
            }
        }
            ...
            foreach (FTPFile currentFileOrDirectory in ftpServerFileInfo)
            {
                if (recursive)
                {
                    if (currentFileOrDirectory.Dir && currentFileOrDirectory.Name != "." && currentFileOrDirectory.Name != "..")
                    {
                        ...
                        this.DownloadFolder(localTargetFolder, ftpTargetFolder, recursive);
        ...
        public void UploadFolder(string remoteFolder, string localFolder, bool recursive)
        {
            ...
            {
                if (!this.FileExistsAtFtp(ftpServerFileInfo, localFile))
                {
                    this.ftpServer.UploadFile(System.IO.Path.Combine(localFolder, localFile), localFile);
                }
                else
                {
                    if (this.FileIsDifferentAtFtp(ftpServerFileInfo, localFile, localFolder))
                    {
            ...
            foreach (var folder in folders)
            {
                ...
                if (!this.FolderExistsAtFtp(ftpServerFileInfo, folder))
                {
                ...
                this.UploadFolder(ftpTargetFolder, localTargetFolder, recursive);
            }
        }
        ...
        public Modification[] ListNewOrUpdatedFilesAtFtpSite(string localFolder, string remoteFolder, bool recursive)
        {
            ...
            this.GetTheList(mods, localFolder, remoteFolder, recursive);
            ...
            foreach (FTPFile currentFileOrDirectory in ftpServerFileInfo)
            {
                if (recursive)
                {
                    if (currentFileOrDirectory.Dir && currentFileOrDirectory.Name != "." && currentFileOrDirectory.Name != "..")
                    {
                        ...
                        this.GetTheList(mods, localTargetFolder, ftpTargetFolder, recursive);
        ...
        private bool FileExistsAtFtp(FTPFile[] ftpServerFileInfo, string localFileName)
        {
        ...
        private bool FolderExistsAtFtp(FTPFile[] ftpServerFileInfo, string localFileName)
        {
        ...
        private bool FileIsDifferentAtFtp(FTPFile[] ftpServerFileInfo, string localFile, string localFolder)
        {
        ...
        {
            this.bpi.AddTaskInformation(e.Message);
        ...
        {
            this.callingTask.CurrentStatus.AddChild(new ItemStatus(information));

            if (this.callingTask.CurrentStatus.ChildItems.Count > 10)
            {
                this.callingTask.CurrentStatus.ChildItems.RemoveAt(0);
            }
