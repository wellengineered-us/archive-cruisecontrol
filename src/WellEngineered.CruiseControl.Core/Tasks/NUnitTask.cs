...
namespace WellEngineered.CruiseControl.Core.Tasks
{
	/// <summary>
    /// <para type="tip">
    ///  See <link>using WellEngineered.CruiseControl.NET with NUnit</link> for more details.
    /// </para>
    /// <para>
        ...
        : TaskBase
    {
        ...
        {
            result.BuildProgressInformation.SignalStartRunTask(!string.IsNullOrEmpty(this.Description) ? this.Description : "Executing NUnit");

            string outputFile = result.BaseFromArtifactsDirectory(this.OutputFile);

            ProcessResult nunitResult = this.executor.Execute(this.NewProcessInfo(outputFile, result));
            this.DeleteNUnitAgentProcesses();
        ...
        {
            NUnitArgument nunitArgument = new NUnitArgument(this.Assemblies, outputFile);
            nunitArgument.ExcludedCategories = this.ExcludedCategories;
            nunitArgument.IncludedCategories = this.IncludedCategories;
            string args = nunitArgument.ToString();

            Log.Debug(string.Format(System.Globalization.CultureInfo.CurrentCulture,"Running unit tests: {0} {1}", this.NUnitPath, args));

            ProcessInfo info = new ProcessInfo(this.NUnitPath, args, result.WorkingDirectory, this.Priority);
