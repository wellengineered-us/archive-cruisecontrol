...
namespace WellEngineered.CruiseControl.Core.Tasks
{
        ...
        : BaseExecutableTask
    {
        ...
        {
            result.BuildProgressInformation.SignalStartRunTask(!string.IsNullOrEmpty(this.Description) ? this.Description : string.Format(System.Globalization.CultureInfo.CurrentCulture, "Executing {0}", this.Executable));
        ...
        {
            return string.Format(CultureInfo.CurrentCulture, @" BaseDirectory: {0}, Executable: {1}", this.ConfiguredBaseDirectory, this.Executable);
        }
