        ...
        : TaskBase
    {
        ...
        public string Executable
        {
            get
            {
                ...
                {
                    this.executable = this.ReadPowerShellFromRegistry();
                }
        ...
        public string SuccessExitCodes
        {
            get
            {
                ...
                {
                    foreach (int code in this.successExitCodes)
                    {
            ...
            set
            {
                if (!string.IsNullOrEmpty(value))
                {
                    ...
                    this.successExitCodes = new int[codes.Length];

                    for (int i = 0; i < codes.Length; ++i)
                    {
                        this.successExitCodes[i] = Int32.Parse(codes[i], CultureInfo.CurrentCulture);
                    }
                }
                else
                {
        ...
        {
            result.BuildProgressInformation.SignalStartRunTask(!string.IsNullOrEmpty(this.Description)
                                                                                                                                         ? this.Description
                                                                                                                                         : string.Format(System.Globalization.CultureInfo.CurrentCulture, "Executing {0}", this.Executable));

            ProcessInfo processInfo = this.NewProcessInfoFrom(result);

            ProcessResult processResult = this.AttemptToExecute(processInfo);
        ...
        {
            ProcessInfo info = new ProcessInfo(this.Executable, this.Args(result), this.BaseDirectory(result), this.Priority, this.successExitCodes);
            info.TimeOut = this.BuildTimeoutSeconds * 1000;
        ...
        {
            return result.BaseFromWorkingDirectory(this.ConfiguredScriptsDirectory);
        }
        ...
        protected ProcessResult AttemptToExecute(ProcessInfo info)
        {
            try
            {
                return this.executor.Execute(info);
            }
            catch (IOException e)
            {
        ...
        {
            return string.Format(CultureInfo.CurrentCulture, @" BaseDirectory: {0}, PowerShell: {1}", this.ConfiguredScriptsDirectory, PowerShellExe);
        }
        ...
        private string ReadPowerShellFromRegistry()
        {
            ...
            registryValue = this.Registry.GetLocalMachineSubKeyValue(regkeypowershell2, regkeyholder);

            if (registryValue == null)
            {
                registryValue = this.Registry.GetLocalMachineSubKeyValue(regkeypowershell1, regkeyholder);
            }
        ...
        private string Args(IIntegrationResult result)
        {
            ...
            if (!string.IsNullOrEmpty(this.Script))
            {
                ...
                if (this.Script.IndexOf(":") == 1) //drive letter specified, so it's not a relative path
                {
                    builder.AppendArgument(@"""" + this.Script + @"""");
                }
                else
                {

                    if (this.ConfiguredScriptsDirectory.EndsWith("\\"))
                    {
                        builder.AppendArgument(@"""" + this.ConfiguredScriptsDirectory + this.Script + @"""");
                    }
                    else
                    {
                        builder.AppendArgument(@"""" + this.ConfiguredScriptsDirectory + "\\" + this.Script + @"""");
            ...
            if (!string.IsNullOrEmpty(this.BuildArgs)) builder.AppendArgument(this.BuildArgs);
