...
namespace WellEngineered.CruiseControl.Core.Tasks
{
    ...
    public class NullTask : TaskBase
	{
        ...
        {
            result.BuildProgressInformation.SignalStartRunTask(!string.IsNullOrEmpty(this.Description) ? this.Description : "Executing null task");
            System.Threading.Thread.Sleep(5000);

            if (this.SimulateFailure)
            {
                result.AddTaskResult(this.SimulateFailureMessage);
                throw new System.Exception(this.SimulateFailureMessage);
            }
            else
            {
                result.AddTaskResult("All OK for " + (!string.IsNullOrEmpty(this.Description) ? this.Description : "Null task"));
            }
