...
namespace WellEngineered.CruiseControl.Core.Tasks
{
        ...
        : TaskBase
    {
		...
		{
            result.BuildProgressInformation.SignalStartRunTask(!string.IsNullOrEmpty(this.Description) ? this.Description : "Merging Files");

            var actualFileSystem = this.FileSystem ?? new SystemIoFileSystem();
            var actualLogger = this.Logger ?? new DefaultLogger();
            ...
            var targetFolder = this.TargetFolder;
			...
			foreach (var mergeFile in this.MergeFiles)
			{
