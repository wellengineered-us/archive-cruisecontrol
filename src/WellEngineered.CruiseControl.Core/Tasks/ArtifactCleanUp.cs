...
namespace WellEngineered.CruiseControl.Core.Tasks
{
        ...
        : TaskBase
    {
        ...
        {
            get { return this.cleanUpValue; }
            set { this.cleanUpValue = value; }
        }
        ...
        {
            result.BuildProgressInformation.SignalStartRunTask(!string.IsNullOrEmpty(this.Description) ? this.Description : "Cleaning up");

            switch (this.cleanUpMethod)
            {
                case CleanUpMethod.KeepLastXBuilds:
                    if (this.BuildLogFolderSet(result))
                        this.KeepLastXBuilds(result.BuildLogDirectory, this.CleaningUpValue);
                    break;

                case CleanUpMethod.DeleteBuildsOlderThanXDays:
                    if (this.BuildLogFolderSet(result))
                        this.DeleteBuildsOlderThanXDays(result.BuildLogDirectory, this.CleaningUpValue);
                    break;

                case CleanUpMethod.KeepMaximumXHistoryDataEntries:
                    this.KeepMaximumXHistoryDataEntries(result, this.cleanUpValue);
                    break;

                case CleanUpMethod.DeleteSubDirsOlderThanXDays:
                    this.DeleteSubDirsOlderThanXDays(result, this.cleanUpValue);
                    break;

                case CleanUpMethod.KeepLastXSubDirs:
                    this.KeepLastXSubDirs(result, this.cleanUpValue);
                    break;
        ...
        private void KeepLastXSubDirs(IIntegrationResult result, int amountToKeep)
        {
            ...
            Log.Debug("Deleting Subdirs of {0}", result.ArtifactDirectory);
        ...
        {
            Log.Debug("Deleting {0}", folderName);
