...
namespace WellEngineered.CruiseControl.Core.Tasks
{
        ...
        : TaskBase, IConfigurationValidation
	{
		...
		public FinalBuilderTask(IRegistry registry, ProcessExecutor executor)
		{
			...
			this._fbversion = FinalBuilderVersion.FBUnknown;
			this._fbcmdpath = String.Empty;
		...
		{
			get { return (int)this._fbversion; }
			set { this._fbversion = (FinalBuilderVersion)value; }
		}
		...
		{
            get { return this._fbcmdpath; }
			set {	this._fbcmdpath = value;		}
		}
        ...
        protected override bool Execute(IIntegrationResult result)
		{
            result.BuildProgressInformation.SignalStartRunTask(!string.IsNullOrEmpty(this.Description) ? this.Description : 
                            string.Format(System.Globalization.CultureInfo.CurrentCulture,"Executing FinalBuilder : BuildFile: {0} ", this.ProjectFile));

            ProcessResult processResult = this.AttemptToExecute(this.NewProcessInfoFrom(result), result.ProjectName);
			result.AddTaskResult(new ProcessTaskResult(processResult));

			if (processResult.TimedOut)
			{
				throw new BuilderException(this, "Build timed out (after " + this.Timeout + " seconds)");
			}
		...
		protected ProcessResult AttemptToExecute(ProcessInfo info, string projectName)
		{
			try
			{
				return this._executor.Execute(info);
			}
			catch (Exception e)
			{
		...
		{
            ProcessInfo info = new ProcessInfo(this.GetFBPath(), this.GetFBArgs());
			info.TimeOut = this.Timeout*1000;
            int idx = this.ProjectFile.LastIndexOf('\\');
            if (idx > -1)
              info.WorkingDirectory = this.ProjectFile.Remove(idx, this.ProjectFile.Length - idx); // Trim down proj. file to get working dir.
		...
		private string GetFBArgs()
		{
            ...
            if (this.UseTemporaryLogFile)
            {
                args.Append("/TL ");
            }
			else if (this.DontWriteToLog)
			{
				args.Append("/S ");
			}

			
			if (this.FBVariables != null && this.FBVariables.Length > 0)
			{
				args.Append("/V");
				for(int j = 0; j < this.FBVariables.Length; j++)				
				{					
					args.Append(this.FBVariables[j].Name);
					args.Append("=");
					args.Append(StringUtil.AutoDoubleQuoteString(this.FBVariables[j].Value));
					if(j < this.FBVariables.Length - 1)
					{
						args.Append(";");
					}
					else
			...
			args.Append("/P");
			args.Append(StringUtil.AutoDoubleQuoteString(this.ProjectFile));		    
        ...
        {
            if (this._fbversion == FinalBuilderVersion.FBUnknown) // Try and autodetect FB Version from project file name
            {
                try
                {
                    return Byte.Parse(this.ProjectFile.Substring(this.ProjectFile.Length - 1, 1), CultureInfo.CurrentCulture);
                }
                catch
                {
		...
		{
            int fbversion = this.GetFBVersion();			
			...
			string executableDir = this._registry.GetLocalMachineSubKeyValue(keyName, "Location");
            if (string.IsNullOrEmpty((executableDir)))
            {
                executableDir = this._registry.GetLocalMachineSubKeyValue(keyName64, "Location");
                if (string.IsNullOrEmpty((executableDir)))
                {
