...
using WellEngineered.CruiseControl.Core.Tasks.Conditions;
...
namespace WellEngineered.CruiseControl.Core.Tasks
{
        ...
        : TaskBase, IConfigurationValidation
    {
        ...
        private bool EvaluateConditions(ILogger logger,  IIntegrationResult result)
        {
            ...
            var passed = true;
            foreach (ITaskCondition condition in this.TaskConditions)
            {
        ...
        private void RunTask(ITask task, IIntegrationResult result, RunningSubTaskDetails taskDetails)
        {
            ...
            if (tsk!= null)
            {
                tsk.ApplyParameters(this.parameters, this.parameterDefinitions);
            }

            if (result != null)
            {
                result.BuildProgressInformation.OnStartupInformationUpdatedUserObject = taskDetails;
                result.BuildProgressInformation.OnStartupInformationUpdated = this.SubTaskStartupInformationUpdated;
            }
            task.Run(result);
        }
        #endregion
        ...
        private class RunningSubTaskDetails
        {
            ...
            /// </summary>
            public bool RunningIfTasks { get { return this.runningIfTasks; } }
            /// <summary>
            /// The parent "result", used by the delegate to update the status while running
            /// </summary>
            public IIntegrationResult ParentResult { get { return this.parentResult; } }
        }
        ...
        private void SubTaskStartupInformationUpdated(string information, object UserObject)
        {
            ...
            Details.ParentResult.BuildProgressInformation.UpdateStartupInformation(this.GetStatusInformation(Details));
        }
        ...
        private string GetStatusInformation(RunningSubTaskDetails Details)
        {
            string Value = !string.IsNullOrEmpty(this.Description)
                            ? this.Description
                            : string.Format("Running {1} tasks ({0} task(s))", this.Tasks.Length, Details.RunningIfTasks ? "\"if\"" : "\"else\"");
