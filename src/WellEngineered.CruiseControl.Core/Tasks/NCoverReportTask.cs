...
namespace WellEngineered.CruiseControl.Core.Tasks
{
        ...
        : BaseExecutableTask
    {
        ...
        {
            result.BuildProgressInformation.SignalStartRunTask(!string.IsNullOrEmpty(this.Description) ? this.Description : "Running NCover reporting");
            ...
            var outputDirectory = new DirectoryInfo(this.RootPath(this.OutputDir, false));
            var oldFiles = this.GenerateOriginalFileList(outputDirectory);
            ...
            if (processResult.Succeeded)
            {
                ...
                var newFiles = this.ListFileDifferences(oldFiles, outputDirectory);
        ...
        protected override string GetProcessArguments(IIntegrationResult result)
        {
            ...
            buffer.AppendArgument(this.RootPath(coverageFile, true));

            // Add all the NCover arguments
            buffer.AppendIf(this.ClearCoverageFilters, "//ccf");
            foreach (var filter in this.CoverageFilters ?? new CoverageFilter[0])
            {
                buffer.AppendArgument("//cf {0}", filter.ToParamString());
            }
            foreach (var threshold in this.MinimumThresholds ?? new CoverageThreshold[0])
            {
                buffer.AppendArgument("//mc {0}", threshold.ToParamString());
            }
            buffer.AppendIf(this.UseMinimumCoverage, "//mcsc");
            buffer.AppendIf(this.XmlReportFilter != NCoverReportFilter.Default, "//rdf {0}", this.XmlReportFilter.ToString());
            foreach (var threshold in this.SatisfactoryThresholds ?? new CoverageThreshold[0])
            {
                buffer.AppendArgument("//sct {0}", threshold.ToParamString());
            }
            buffer.AppendIf(this.NumberToReport > 0, "//smf {0}", this.NumberToReport.ToString(CultureInfo.CurrentCulture));
            buffer.AppendIf(!string.IsNullOrEmpty(this.TrendOutputFile), "//at \"{0}\"", this.RootPath(this.TrendOutputFile, false));
            buffer.AppendArgument("//bi \"{0}\"", string.IsNullOrEmpty(this.BuildId) ? result.Label : this.BuildId);
            buffer.AppendIf(!string.IsNullOrEmpty(this.HideElements), "//hi \"{0}\"", this.HideElements);
            buffer.AppendIf(!string.IsNullOrEmpty(this.TrendInputFile), "//lt \"{0}\"", this.RootPath(this.TrendInputFile, false));
            this.GenerateReportList(buffer);
            buffer.AppendIf(!string.IsNullOrEmpty(this.ProjectName), "//p \"{0}\"", this.ProjectName);
            buffer.AppendIf(this.SortBy != NCoverSortBy.None, "//so \"{0}\"", this.SortBy.ToString());
            buffer.AppendIf(this.TopUncoveredAmount > 0, "//tu \"{0}\"", this.TopUncoveredAmount.ToString(CultureInfo.CurrentCulture));
            buffer.AppendIf(this.MergeMode != NCoverMergeMode.Default, "//mfm \"{0}\"", this.MergeMode.ToString());
            buffer.AppendIf(!string.IsNullOrEmpty(this.MergeFile), "//s \"{0}\"", this.RootPath(this.MergeFile, false));
        ...
        private void GenerateReportList(ProcessArgumentBuilder buffer)
        {
            ...
            {
                reportList.AddRange(this.Reports);
            }
            else
            {
            ...
            foreach (var report in reportList)
            {
                ...
                {
                    path = this.RootPath(string.Format(System.Globalization.CultureInfo.CurrentCulture,"{0}.html", report), false);
                }
