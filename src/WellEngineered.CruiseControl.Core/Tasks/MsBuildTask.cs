...
namespace WellEngineered.CruiseControl.Core.Tasks
{
    ...
    /// this, and a tutorial on how to use the task, see <link>using WellEngineered.CruiseControl.NET with MSBuild</link>.
    /// </para>
    /// <para type="tip">
        ...
        : BaseExecutableTask
	{
        ...
        public string Logger { get { return this.logger; } set { this.logger = CheckAndQuoteLoggerSetting(value); } }
		...
		{
			return string.IsNullOrEmpty(this.Executable) ? this.GetDefaultExecutable() : this.Executable;
		}
		...
		protected override string GetProcessArguments(IIntegrationResult result)
		{
            ...
            if (!string.IsNullOrEmpty(this.Targets))
			{
				...
				foreach (string target in this.Targets.Split(';'))
				{
			...
			b.AppendArgument(this.BuildArgs);
			b.AddArgument(this.ProjectFile);
			b.AppendArgument(this.GetLoggerArgs(result));
        ...
        /// <remarks></remarks>
				protected override bool Execute(IIntegrationResult result)
				{
					result.BuildProgressInformation.SignalStartRunTask(!string.IsNullOrEmpty(this.Description) ? this.Description :
						string.Format(System.Globalization.CultureInfo.CurrentCulture, "Executing MSBuild :BuildFile: {0}", this.ProjectFile));

					var info = this.CreateProcessInfo(result);
					ProcessResult processResult = this.TryToRun(info, result);

					string buildOutputFile = this.MsBuildOutputFile(result);
		...
		private string GetLoggerArgs(IIntegrationResult result)
		{
			...
			if (string.IsNullOrEmpty(this.Logger))
			{
                ...
                var loggerPath = this.shadowCopier.RetrieveFilePath("ThoughtWorks.CruiseControl.MsBuild.dll");
            ...
            foreach (string parameter in this.LoggerParameters)
