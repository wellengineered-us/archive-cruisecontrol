...
namespace WellEngineered.CruiseControl.Core.Tasks
{
        ...
        : BaseExecutableTask
    {
        ...
        {
            result.BuildProgressInformation.SignalStartRunTask(!string.IsNullOrEmpty(this.Description) ? this.Description : "Executing NDepend");
            ...
            var outputDirectory = this.RootPath(this.OutputDir, false);
            ...
            if (this.Publish && processResult.Succeeded)
            {
                ...
                {
                    this.logger.Debug("Copying {0} new file(s)", newFiles.Length);
                    ...
                    foreach (var newFile in newFiles)
                    {
                        ...
                        {
                            lastDir = newDir;
                            if (!this.fileSystem.DirectoryExists(newDir))
                            {
                                this.fileSystem.CreateDirectory(newDir);
                            }
                        }

                        // Copy the file and marge it if XML
                        if (this.fileSystem.FileExists(newFile))
                        {
                            this.fileSystem.Copy(newFile, newPath);
                            if (Path.GetExtension(newFile) == ".xml")
                            {
                                result.AddTaskResult(this.fileSystem.GenerateTaskResultFromFile(newFile));
                            }
                        }
                        else
                        {
                            this.logger.Warning("Unable to find file: " + newFile);
                        }
                    }
                }
            }
        ...
        protected override string GetProcessFilename()
        {
            ...
            {
                path = this.RootPath(defaultExecutable, false);
            }
            else
            {
                path = this.RootPath(this.Executable, false);
            }
            return path;
        }
        #endregion
        ...
        protected override string GetProcessArguments(IIntegrationResult result)
        {
            ...
            buffer.Append(this.RootPath(this.ProjectFile, true));
            buffer.AppendIf(this.Silent, "/Silent");
            buffer.AppendIf(this.EmitXml, "/EmitVisualNDependBinXml ");
            if ((this.InputDirs != null) && (this.InputDirs.Length > 0))
            {
                List<string> dirs = new List<string>();
                foreach (string dir in this.InputDirs)
                {
                    dirs.Add(this.RootPath(dir, true));
                }
                buffer.AppendArgument("/InDirs {0}", string.Join(" ", dirs.ToArray()));
            }
            buffer.AppendArgument("/OutDir {0}", this.RootPath(this.OutputDir, true));
            if (!string.IsNullOrEmpty(this.ReportXslt))
            {
                buffer.AppendArgument("/XslForReport  {0}", this.RootPath(this.ReportXslt, true));
            }
        ...
        private string RootPath(string path, bool doubleQuote)
        {
            ...
            {
                var basePath = this.rootPath ?? this.BaseDirectory ?? string.Empty;
        ...
        private string[] ListFileDifferences(Dictionary<string, DateTime> originalList, string outputDirectory)
        {
            ...
            if (this.fileSystem.FileExists(contentsFile))
            {
                using (var reader = this.fileSystem.Load(contentsFile))
                {
                    ...
                    foreach (var folder in rootEl.Elements("Directory"))
                    {
                        ...
                        files.AddRange(this.fileSystem.GetFilesInDirectory(fullPath, true));
                    }
            ...
            {
                string[] newList = { };
                if (this.fileSystem.DirectoryExists(outputDirectory))
                {
                    newList = this.fileSystem.GetFilesInDirectory(outputDirectory);
                }
                ...
                foreach (var newFile in newList)
                {
                    if (originalList.ContainsKey(newFile))
                    {
                        ...
                        if (originalList[newFile] != this.fileSystem.GetLastWriteTime(newFile))
                        {
        ...
        private Dictionary<string, DateTime> GenerateOriginalFileList(string outputDirectory)
        {
            ...
            if (this.fileSystem.DirectoryExists(outputDirectory))
            {
                var oldFiles = this.fileSystem.GetFilesInDirectory(outputDirectory);
                foreach (var oldFile in oldFiles)
                {
                    originalFiles.Add(oldFile, this.fileSystem.GetLastWriteTime(oldFile));
                }
            }
