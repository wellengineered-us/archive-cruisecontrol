...
namespace WellEngineered.CruiseControl.Core.Tasks
{
    ...
    public class FtpTask : TaskBase
    {
        ...
        protected override bool Execute(IIntegrationResult result)
        {
            ...
            result.BuildProgressInformation.SignalStartRunTask(!string.IsNullOrEmpty(this.Description) ? this.Description : this.GetDescription(relativeLocalFolder));
            ...
            try
            {
                ftp.LogIn(this.ServerName, this.UserName, this.Password, this.UseActiveConnectionMode);

                ftp.TimeDifference = new TimeSpan(this.TimeDifference, 0, 0);



                if (!this.FtpFolderName.StartsWith("/"))
                {
                    remoteFolder = System.IO.Path.Combine(ftp.CurrentWorkingFolder(),this.FtpFolderName);
                }

                if (this.Action == FtpAction.UploadFolder)
                {
                    Log.Debug("Uploading {0} to {1}, recursive : {2}", relativeLocalFolder, remoteFolder, this.RecursiveCopy);
                    ftp.UploadFolder(remoteFolder, relativeLocalFolder, this.RecursiveCopy);
                }

                if (this.Action == FtpAction.DownloadFolder)
                {
                    Log.Debug("Downloading {0} to {1}, recursive : {2}", remoteFolder, relativeLocalFolder, this.RecursiveCopy);
                    ftp.DownloadFolder(relativeLocalFolder, remoteFolder, this.RecursiveCopy);
                }
        ...
        private string GetDescription(string relativeLocalFolder)
        {
            if (this.Action == FtpAction.DownloadFolder)
            {
                return string.Concat("Downloading ", this.FtpFolderName, " to ", relativeLocalFolder);
            }

            return string.Concat("Uploading ", relativeLocalFolder, " to ", this.FtpFolderName);
        }
