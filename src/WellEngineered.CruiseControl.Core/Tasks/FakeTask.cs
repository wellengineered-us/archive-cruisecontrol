...
namespace WellEngineered.CruiseControl.Core.Tasks
{
    ...
    public class FakeTask : BaseExecutableTask
    {
        ...
        {
            var fakeOutputFile = this.GetFakeOutputFile(result);

            //delete old nant output logfile, if exist
            this.fileDirectoryDeleter.DeleteIncludingReadOnlyObjects(fakeOutputFile);

            result.BuildProgressInformation.SignalStartRunTask(!string.IsNullOrEmpty(this.Description) ? this.Description :
                string.Format(System.Globalization.CultureInfo.CurrentCulture,"Executing FAKE - {0}", this.ToString()));
        ...
        protected override string GetProcessArguments(IIntegrationResult result)
        {
            ...
            buffer.AppendArgument(StringUtil.AutoDoubleQuoteString(this.BuildFile));
            buffer.AppendArgument("logfile={0}", StringUtil.AutoDoubleQuoteString(this.GetFakeOutputFile(result)));
        ...
        public override string ToString()
        {
            ...
            return string.Format(CultureInfo.CurrentCulture, @" BaseDirectory: {0}, Executable: {1}, BuildFile: {2}", baseDirectory, this.Executable, this.BuildFile);
        }
