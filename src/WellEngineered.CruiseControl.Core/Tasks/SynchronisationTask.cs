...
namespace WellEngineered.CruiseControl.Core.Tasks
{
        ...
        : TaskContainerBase
    {
        ...
        protected override bool Execute(IIntegrationResult result)
        {
            ...
            if (Monitor.TryEnter(contexts[contextToUse], this.TimeoutPeriod * 1000))
            {
                try
                {
                    // Launch each task
                    ...
                    {
                        var taskName = string.Format(System.Globalization.CultureInfo.CurrentCulture, "{0} [{1}]", this.Tasks[loop].GetType().Name, loop);
                        logger.Debug("Starting task '{0}'", taskName);
                        try
                        {
                            // Start the actual task
                            var taskResult = result.Clone();
                            var task = this.Tasks[loop];
                            this.RunTask(task, taskResult, new RunningSubTaskDetails(loop, result));
        ...
        protected override string GetStatusInformation(RunningSubTaskDetails Details)
        {
            string Value = !string.IsNullOrEmpty(this.Description)
                            ? this.Description
                            : string.Format("Running synchronized tasks ({0} task(s))", this.Tasks.Length);
