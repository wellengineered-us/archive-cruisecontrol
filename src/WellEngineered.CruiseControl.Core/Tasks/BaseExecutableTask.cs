...
namespace WellEngineered.CruiseControl.Core.Tasks
{
        ...
        : TaskBase, IConfigurationValidation
	{
		...
		{
			ProcessInfo info = new ProcessInfo(this.GetProcessFilename(), this.GetProcessArguments(result), this.GetProcessBaseDirectory(result),this.GetProcessPriorityClass(), this.GetProcessSuccessCodes());
			info.TimeOut = this.GetProcessTimeout();
		...
		{
			this.buildProgressInformation = result.BuildProgressInformation;

			try
			{
				// enable Stdout monitoring
				this.executor.ProcessOutput += this.ProcessExecutor_ProcessOutput;
		...
		{
			if (this.buildProgressInformation == null)
				return;
			...
			this.buildProgressInformation.AddTaskInformation(e.Data);
        }
