...
namespace WellEngineered.CruiseControl.Core.Tasks
{
        ...
        : TaskBase
    {
        ...
        {
            result.BuildProgressInformation.SignalStartRunTask(!string.IsNullOrEmpty(this.Description) ? this.Description : "Writing Modifications");                

            XmlSerializer serializer = new XmlSerializer(typeof(Modification[]));
            StringWriter writer = new Utf8StringWriter();
            serializer.Serialize(writer, result.Modifications);
            string filename = this.ModificationFile(result);
            this.fileSystem.EnsureFolderExists(filename);
            this.fileSystem.Save(filename, writer.ToString());
        ...
        private string ModificationFile(IIntegrationResult result)
        {
        	if (!this.AppendTimeStamp)
        		return Path.Combine(result.BaseFromArtifactsDirectory(this.OutputPath), this.Filename);

        	FileInfo fi = new FileInfo(this.Filename);
        	string dummy = this.Filename.Remove(this.Filename.Length - fi.Extension.Length, fi.Extension.Length);
        	...
        	return Path.Combine(result.BaseFromArtifactsDirectory(this.OutputPath), newFileName);
        }
