...
namespace WellEngineered.CruiseControl.Core.Tasks
{
        ...
        : TaskBase
    {
        ...
        protected override bool Execute(IIntegrationResult result)
        {
            ...
            result.BuildProgressInformation.SignalStartRunTask(!string.IsNullOrEmpty(this.Description) ? this.Description : "Reading Modifications");                


        	System.Collections.ArrayList allModifications = new System.Collections.ArrayList();
            

            foreach (string file in this.GetModificationFiles(result))
            {
                ...
                if (this.deleteAfterRead) filesToDelete.Add(file);
            }
        ...
        {
            FileInfo fi = new FileInfo(Path.Combine(result.BaseFromArtifactsDirectory(this.OutputPath), this.Filename));
