...
namespace WellEngineered.CruiseControl.Core.Tasks
{
    ...
    /// </para>
    /// <para type="tip">
    /// See <link>using WellEngineered.CruiseControl.NET with NAnt</link> for more information on working with NAnt and CruiseControl.Net.
    /// </para>
    /// <para type="tip">
        ...
        : BaseExecutableTask
    {
        ...
        {
            string nantOutputFile = this.GetNantOutputFile(result);
            //delete old nant output logfile, if exist
            this.fileDirectoryDeleter.DeleteIncludingReadOnlyObjects(nantOutputFile);

            result.BuildProgressInformation.SignalStartRunTask(!string.IsNullOrEmpty(this.Description) ? this.Description :
                    string.Format(System.Globalization.CultureInfo.CurrentCulture, "Executing Nant :BuildFile: {0} Targets: {1} ", this.BuildFile, string.Join(", ", this.Targets)));

            var info = this.CreateProcessInfo(result);
            ProcessResult processResult = this.TryToRun(info, result);
        ...
        protected override string GetProcessArguments(IIntegrationResult result)
        {
            ...
            buffer.AppendIf(this.NoLogo, "-nologo");
            buffer.AppendArgument(@"-buildfile:{0}", StringUtil.AutoDoubleQuoteString(this.BuildFile));
            buffer.AppendArgument("-logger:{0}", this.Logger);
            buffer.AppendArgument("-logfile:{0}", StringUtil.AutoDoubleQuoteString(this.GetNantOutputFile(result)));
            buffer.AppendArgument("-listener:{0}", this.Listener);
            this.AppendBuildArguments(buffer);
            AppendIntegrationResultProperties(buffer, result);
            this.AppendTargets(buffer);
        ...
        {
            var args = System.Text.RegularExpressions.Regex.Split(this.BuildArgs, @"\n");
        ...
        {
            string baseDirectory = this.ConfiguredBaseDirectory ?? string.Empty;
            return string.Format(CultureInfo.CurrentCulture, @" BaseDirectory: {0}, Targets: {1}, Executable: {2}, BuildFile: {3}", baseDirectory, string.Join(", ", this.Targets), this.Executable, this.BuildFile);
        }
