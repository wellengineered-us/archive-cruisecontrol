...
namespace WellEngineered.CruiseControl.Core.Triggers
{
	...
	public class ProjectTrigger : ITrigger
	{
        ...
        /// </summary>
        /// <remarks></remarks>
		public const string DefaultServerUri = default(string);
		...
		{
			Log.Debug("Retrieving ProjectStatus from server: " + this.ServerUri);
            ...
            try
            {
                ...
                var project = currentStatuses.FirstOrDefault(p => p.Name == this.Project);
                if (project != null)
                {
                ...
                throw new NoSuchProjectException(this.Project);
            }
            finally
            {
                if (loggedIn)
		...
		{
			IntegrationRequest request = this.InnerTrigger.Fire();
			if (request == null) return null;
			this.InnerTrigger.IntegrationCompleted(); // reset inner trigger (timer)

            try
            {
                this.currentStatus = this.GetCurrentProjectStatus();
                if (this.lastStatus == null)
                {
                    this.lastStatus = this.currentStatus;
                    if (this.TriggerFirstTime && this.currentStatus.BuildStatus == this.TriggerStatus)
                    {
                        return request;
                    }
                    return null;
                }
                if (this.currentStatus.LastBuildDate > this.lastStatus.LastBuildDate && this.currentStatus.BuildStatus == this.TriggerStatus)
                {
                    return request;
                }
            }
            ...
            {
                Log.Warning("Skipping Fire() because ServerUri " + this.ServerUri + " was not found.");
            }
