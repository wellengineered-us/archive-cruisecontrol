...
namespace WellEngineered.CruiseControl.Core.Triggers
{
	...
	public class UrlTrigger : IntervalTrigger
	{
		...
		public override IntegrationRequest Fire()
		{
			...
			Log.Debug(string.Format(System.Globalization.CultureInfo.CurrentCulture,"More than {0} seconds since last integration, checking url.", this.IntervalSeconds));
		...
		private bool HasUrlChanged()
		{
			try
			{
				DateTime newModifiedTime = this.httpRequest.GetLastModifiedTimeFor(this.uri, this.lastModified);
			...
			{
				Log.Error("Error accessing url: " + this.uri);
				Log.Error(e);
			}
			return false;
		}
