...
using WellEngineered.CruiseControl.Core.Publishers;
using WellEngineered.CruiseControl.Core.Publishers.Statistics;
...
using WellEngineered.CruiseControl.Core.Triggers;
...
namespace WellEngineered.CruiseControl.Core
{
        ...
        IConfigurationValidation, IStatusSnapshotGenerator, IParamatisedProject
    {
        ...
        private Common.SourceControlErrorHandlingPolicy sourceControlErrorHandling = Common.SourceControlErrorHandlingPolicy.ReportEveryFailure;
        ...
        public Project()
        {
            this.integrationResultManager = new IntegrationResultManager(this);
            this.integratable = new IntegrationRunner(this.integrationResultManager, this, this.quietPeriod);
            ...
            this.currentProjectStatus = new ProjectStatusSnapshot();
            this.PropertyChanged += delegate(object sender, PropertyChangedEventArgs e)
            {
                switch (e.PropertyName)
                {
                    case "Name":
                        lock (this.currentProjectStatus)
                        {
                            this.currentProjectStatus.Name = this.Name;
                        }
                        break;
                    case "Description":
                        lock (this.currentProjectStatus)
                        {
                            this.currentProjectStatus.Description = this.Description;
                        }
                        break;
                }
            };
        ...
        {
            get { return this.maxSourceControlRetries; }
            set { this.maxSourceControlRetries = value < 0 ? 0 : value; }
        }
        ...
        {
            get { return this.sourceControlErrorHandling; }
            set { this.sourceControlErrorHandling = value; }
        }
        ...
        public string QueueName
        {
            get
            {
                if (string.IsNullOrEmpty(this.queueName)) return this.Name;
                return this.queueName;
            }
            set { this.queueName = value.Trim(); }
        }
        ...
        {
            get { return this.quietPeriod.ModificationDelaySeconds; }
            set { this.quietPeriod.ModificationDelaySeconds = value; }
        }


        /// <summary>
        /// Write a small summary file that can be retrieved by clients, this contains very minimalistic info.
        /// </summary>
        /// <version>1.7</version>
        /// <default>false</default>
        [ReflectorProperty("writeSummaryFile", Required = false)]
        public bool WriteSummaryFile
        {
            get { return this.writeSummaryFile; }
            set { this.writeSummaryFile = value; }
        }
        ...
        {
            get { return this.labeller; }
            set { this.labeller = value; }
        }
        ...
        {
            get { return this.integrationResultManager.CurrentIntegration; }
        }
        ...
        {
            Log.Trace("Integrating {0}", this.Name);
            ...
            lock (this.currentProjectStatus)
            {
                ...
                this.currentProjectItems.Clear();
                this.sourceControlOperations.Clear();
                this.currentProjectStatus.Status = ItemBuildStatus.Running;
                this.currentProjectStatus.ChildItems.Clear();
                this.currentProjectStatus.TimeCompleted = null;
                this.currentProjectStatus.TimeOfEstimatedCompletion = null;
                this.currentProjectStatus.TimeStarted = DateTime.Now;
                this.GenerateSourceControlOperation(SourceControlOperation.CheckForModifications);
                this.GenerateTaskStatuses("Pre-build tasks", this.PrebuildTasks);
                this.GenerateSourceControlOperation(SourceControlOperation.GetSource);
                this.GenerateTaskStatuses("Build tasks", this.Tasks);
                this.GenerateTaskStatuses("Publisher tasks", this.Publishers);
            }
            ...
            try
            {
                ...
                var dynamicSourceControl = this.sourceControl as IParamatisedItem;
                if (dynamicSourceControl != null)
                {
                    dynamicSourceControl.ApplyParameters(request.BuildValues, this.parameters);
                }
                result = this.integratable.Integrate(request);
                summary.Label = result.Label;
                this.writeSummary(result, logDirectory, summary, timer, fileSystem, serialiser);
            }
            ...
            finally
            {
                ...
                lock (this.currentProjectStatus)
                {
                    this.CancelAllOutstandingItems(this.currentProjectStatus);
                    this.currentProjectStatus.TimeCompleted = DateTime.Now;
                    ...
                    switch (resultStatus)
                    {
                        case IntegrationStatus.Success:
                            this.currentProjectStatus.Status = ItemBuildStatus.CompletedSuccess;
                            break;
                        case IntegrationStatus.Unknown:
                            // This probably means the build was cancelled (i.e. no changes detected)
                            this.currentProjectStatus.Status = ItemBuildStatus.Cancelled;
                            break;
                        default:
                            this.currentProjectStatus.Status = ItemBuildStatus.CompletedFailed;
                            break;
                    }
                }
            }
        ...
        {
            if (!this.writeSummaryFile) return;
            if (r == null) return;
        ...
        {
            for (Int32 i = this.messages.Count - 1; i >= 0; i--)
            {
                Message m = (Message)this.messages[i];
                if (m.Kind == kind)
                {
        ...
        private void CancelAllOutstandingItems(ItemStatus value)
        {
                ...
                (value.Status == ItemBuildStatus.Pending))
            {
                ...
                {
                    this.CancelAllOutstandingItems(item);
        ...
        private void GenerateSourceControlOperation(SourceControlOperation operation)
        {
            ...
            else if (this.SourceControl is IStatusSnapshotGenerator)
            {
                sourceControlStatus = (this.SourceControl as IStatusSnapshotGenerator).GenerateSnapshot();
            }
            else
            {
                sourceControlStatus = new ItemStatus(
                    string.Format(System.Globalization.CultureInfo.CurrentCulture, "{0}: {1}",
                        this.SourceControl.GetType().Name,
                        operation));
            ...
            {
                this.currentProjectStatus.AddChild(sourceControlStatus);
                this.sourceControlOperations.Add(operation, sourceControlStatus);
            }
        }
        ...
        private void GenerateTaskStatuses(string name, IList tasks)
        {
            ...
            foreach (ITask task in tasks)
            {
                ...
                if (taskItem != null)
                {
                    ...
                    this.currentProjectItems.Add(task, taskItem);
                }
            }

            // Only add the group item if it contains children
            if (groupItem.ChildItems.Count > 0) this.currentProjectStatus.AddChild(groupItem);
        }
        ...
        {
            this.currentActivity = ProjectActivity.Pending;
        }
        ...
        {
            this.currentActivity = ProjectActivity.Sleeping;
        }
        ...
        public void Prebuild(IIntegrationResult result)
        {
            ...
            this.Prebuild(result, parameters);
        }
        ...
        {
            this.RunTasks(result, this.PrebuildTasks, parameterValues);
        }
        ...
        {
            this.RunTasks(result, this.tasks, parameterValues);
        }
        ...
        private void RunTasks(IIntegrationResult result, IList tasksToRun, Dictionary<string, string> parameterValues)
        {
            foreach (ITask task in tasksToRun)
            {
                ...
                if (dummy != null)
                {
                    dummy.ApplyParameters(parameterValues, this.parameters);
                }

                this.RunTask(task, result, false);
                if (result.Failed) break;
            }
            this.CancelTasks(tasksToRun);
        }
        ...
        {
            this.AddMessage(new Message(userName, Message.MessageKind.BuildAbortedBy));
            ProcessExecutor.KillProcessCurrentlyRunningForProject(this.Name);
        }
        ...
        public void PublishResults(IIntegrationResult result)
        {
            var parameters = new Dictionary<string, string>();
            if (result.Parameters != null) parameters = NameValuePair.ToDictionary(result.Parameters);
            this.PublishResults(result, parameters);
        }
        ...
        public void PublishResults(IIntegrationResult result, Dictionary<string, string> parameterValues)
        {
            ...
            this.CancelTasks(this.PrebuildTasks);
            this.CancelTasks(this.Tasks);

            var merged = false;
            var mergeFailed = false;
            foreach (ITask publisher in this.publishers)
            {
                ...
                try
                {
                    ...
                    this.RunTask(publisher, result, true);
                }
                catch (Exception e)
                {
            ...
            {
                this.AddBreakersToMessages(result);
                this.AddFailedTaskToMessages();
            }
        ...
        private void RunTask(ITask task, IIntegrationResult result, bool isPublisher)
        {
            ...
            if (this.currentProjectItems.ContainsKey(task)) status = this.currentProjectItems[task];
        ...
        private void CancelTasks(IList tasks)
        {
            ...
            foreach (ITask task in tasks)
            {
                if (this.currentProjectItems.ContainsKey(task))
                {
                    ItemStatus status = this.currentProjectItems[task];
        ...
        private void AddBreakersToMessages(IIntegrationResult result)
        {
            ...
            this.AddMessage(new Message(breakingusers, Message.MessageKind.Breakers));
        }
        ...
        private void AddFailedTaskToMessages()
        {
            ...
            FindFailedTasks(this.currentProjectStatus, failedTasks);
        ...
        {
            Log.Info(string.Format(System.Globalization.CultureInfo.CurrentCulture, "Initializing Project [{0}]", this.Name));
            this.SourceControl.Initialize(this);
        }
        ...
        {
            Log.Info(string.Format(System.Globalization.CultureInfo.CurrentCulture, "Purging Project [{0}]", this.Name));
            if (purgeSourceControlEnvironment)
            {
                this.SourceControl.Purge(this);
            }
            if (purgeWorkingDirectory && Directory.Exists(this.WorkingDirectory))
            {
                new IoService().DeleteIncludingReadOnlyObjects(this.WorkingDirectory);
            }
            if (purgeArtifactDirectory && Directory.Exists(this.ArtifactDirectory))
            {
                new IoService().DeleteIncludingReadOnlyObjects(this.ArtifactDirectory);
            }
        }
        ...
        {
            get { return StatisticsPublisher.LoadStatistics(this.ArtifactDirectory); }
        }
        ...
        {
            get { return ModificationHistoryPublisher.LoadHistory(this.ArtifactDirectory); }
        }
        ...
        public string RSSFeed
        {
            get
            {
                ...
                return RssPublisher.LoadRSSDataDocument(this.ArtifactDirectory);
            }
        }
        ...
        {
            if (this.CurrentActivity == ProjectActivity.Building ||
                this.CurrentActivity == ProjectActivity.CheckingModifications)
                return this.integrationResultManager.CurrentIntegration.BuildProgressInformation.GetBuildProgressInformation();
        ...
        {
            get { return this.integrationResultManager.LastIntegration; }
        }
        ...
        public void AddMessage(Message message)
        {
            if (message.Kind == Message.MessageKind.Fixer)
            {
                ...
                var existingFixerMessage = (from m in this.messages where m.Kind == Message.MessageKind.Fixer select m).SingleOrDefault();
                if (existingFixerMessage != null)
                {
                    this.messages.Remove(existingFixerMessage);
                }
            }
        ...
        {
            string[] buildNames = this.GetBuildNames();
        ...
        {
            XmlLogPublisher publisher = this.GetLogPublisher();
            string logDirectory = publisher.LogDirectory(this.ArtifactDirectory);
        ...
        {
            if (this.Labeller is IParamatisedItem)
            {
                (this.Labeller as IParamatisedItem).ApplyParameters(result.IntegrationRequest.BuildValues,
                    this.parameters);
            }
            result.Label = this.Labeller.Generate(result);
        }
        ...
        {
            get { return this.startupMode; }
            set { this.startupMode = value; }
        }
        ...
        {
            if (this.security.RequiresServerSecurity &&
            ...
            this.ValidateItem(this.sourceControl, configuration, parent, errorProcesser);
            this.ValidateItem(this.labeller, configuration, parent, errorProcesser);
            this.ValidateItems(this.PrebuildTasks, configuration, parent, errorProcesser);
            this.ValidateItems(this.tasks, configuration, parent, errorProcesser);
            this.ValidateItems(this.publishers, configuration, parent, errorProcesser);
            this.ValidateItem(this.state, configuration, parent, errorProcesser);
            this.ValidateItem(this.security, configuration, parent, errorProcesser);
        ...
        {
            if (this.ContainsInvalidChars(this.Name))
            {
        ...
        public virtual ItemStatus GenerateSnapshot()
        {
            lock (this.currentProjectStatus)
            {
                ...
                if (this.currentProjectStatus.Status == ItemBuildStatus.Unknown)
                {
                    switch (this.LastIntegration.Status)
                    {
                        case IntegrationStatus.Success:
                            this.currentProjectStatus.Status = ItemBuildStatus.CompletedSuccess;
                            break;
                ...
                return this.currentProjectStatus.Clone();
            }
        }
        #endregion
        ...
        {
            if (this.sourceControlOperations.ContainsKey(operation))
            {
                this.sourceControlOperations[operation].Status = status;
                switch (status)
                {
                    case ItemBuildStatus.Running:
                        this.sourceControlOperations[operation].TimeStarted = DateTime.Now;
                        break;
                    case ItemBuildStatus.CompletedFailed:
                    case ItemBuildStatus.CompletedSuccess:
                        this.sourceControlOperations[operation].TimeCompleted = DateTime.Now;
                        break;
                }
            }
        }
        ...
        public virtual List<PackageDetails> RetrievePackageList()
        {
            ...
            foreach (string packgeInfo in Directory.GetFiles(this.ArtifactDirectory, "*-packages.xml", SearchOption.AllDirectories))
            {
                packages.AddRange(this.LoadPackageList(packgeInfo));
            }
            return packages;
        }
        ...
        {
            var listFile = Path.Combine(buildLabel, this.Name + "-packages.xml");
            listFile = Path.Combine(this.ArtifactDirectory, listFile);
            if (File.Exists(listFile))
            {
                var packages = this.LoadPackageList(listFile);
                return packages;
            }
            else
            {
        ...
        private List<PackageDetails> LoadPackageList(string fileName)
        {
            ...
            if (File.Exists(fileName))
            {
                ...
                foreach (XmlElement packageElement in packageList.SelectNodes("/packages/package"))
                {
                    ...
                    packageFileName = packageFileName.Replace(this.ArtifactDirectory, string.Empty);
        ...
        public virtual List<ParameterBase> ListBuildParameters()
        {
            ...
            if (this.AskForForceBuildReason != DisplayLevel.None)
            {
                var reasonParameter = new TextParameter
                {
                    ...
                    IsRequired = (this.AskForForceBuildReason == DisplayLevel.Required)
                };
        ...
        {
            this.ClearMessages(Message.MessageKind.Breakers);
            this.ClearMessages(Message.MessageKind.FailingTasks);
            this.ClearMessages(Message.MessageKind.BuildAbortedBy);
