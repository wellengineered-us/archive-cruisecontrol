...
namespace WellEngineered.CruiseControl.Core.Reporting.Dashboard.Navigation
{
	...
	public class AbsolutePathUrlBuilderDecorator : IUrlBuilder
	{
		...
		{
			return this.Decorate(this.decoratedUrlBuilder.BuildUrl(action));
		}
		...
		{
			return this.Decorate(this.decoratedUrlBuilder.BuildUrl(action, partialQueryString));
		}
		...
		{
			return this.Decorate(this.decoratedUrlBuilder.BuildUrl(action, partialQueryString, path));
		}
		...
		{
			set { this.decoratedUrlBuilder.Extension = value; }
			get { return this.decoratedUrlBuilder.Extension; }
		}
		...
		{
			return string.Format(System.Globalization.CultureInfo.CurrentCulture,"{0}{1}{2}", this.basePath, this.basePath.EndsWith("/") ? string.Empty : "/", relativeUrl);
		}
	}
}