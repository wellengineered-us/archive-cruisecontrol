...
namespace WellEngineered.CruiseControl.Core.Label
{
        ...
        : DefaultLabeller
	{
		...
		public override string Generate(IIntegrationResult integrationResult)
		{
			...
			{
				return this.NewLabel(InitialLabel);
			}
			else if (lastIntegration.Status == IntegrationStatus.Success || this.IncrementOnFailed)
			{
				return this.NewLabel(this.IncrementLabel(lastIntegration.Label));
			}
			else
			{
		...
		private string NewLabel(int suffix)
		{
			...
			buffer.Append(this.LabelPrefix);
			if (!(this.LabelPrefix != null && this.LabelPrefix.Length == 0) && ! this.LabelPrefix.EndsWith(this.Separator)) buffer.Append(this.Separator);
			buffer.Append(this.CurrentIteration());
		...
		{
			string iterationPtn = @".*?((\d+)" + this.Separator.Replace(".", @"\.") + "(\\d+$)).*";
			...
			if (numericIteration < this.CurrentIteration())
			{
				return InitialLabel;
			}
			else
		...
		{
			return this.GetIteration(this.ReleaseStartDate);
		}

		private int GetIteration(DateTime startDate)
		{
			return this.GetIteration(startDate, this.dateTimeProvider.Today);
		}

		private int GetIteration(DateTime startDate, DateTime endDate)
		{
			double daysFromStart = (endDate - startDate).TotalDays;
			return (int) daysFromStart/(this.Duration*DaysInWeek);
		}
	}
}