...
namespace WellEngineered.CruiseControl.Core.Label
{
        ...
        : LabellerBase
	{
		...
		public override string Generate(IIntegrationResult integrationResult)
		{
			...
			Version version = this.ParseVersion(now, integrationResult.LastIntegration);
            ...
            return string.Format(System.Globalization.CultureInfo.CurrentCulture,"{0}.{1}.{2}.{3}",
                   now.Year.ToString(this.YearFormat, CultureInfo.CurrentCulture), now.Month.ToString(this.MonthFormat, CultureInfo.CurrentCulture), now.Day.ToString(this.DayFormat, CultureInfo.CurrentCulture), revision.ToString(this.RevisionFormat, CultureInfo.CurrentCulture));
		}
