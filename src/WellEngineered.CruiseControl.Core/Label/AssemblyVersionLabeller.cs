        ...
        : LabellerBase
    {
		...
		public override string Generate(IIntegrationResult integrationResult)
		{
			...
			if (this.Build > -1)
			{
                ...
                Log.Debug("[assemblyVersionLabeller] Build number ist set to '{0}' via configuration.", this.Build);
			}
			else
			{
                ...
                if (integrationResult.LastIntegration.Status == IntegrationStatus.Success || this.IncrementOnFailure)
				{
					currentBuild++;
				}
                else
			...
			Log.Debug(string.Format(System.Globalization.CultureInfo.InvariantCulture,
                                    "[assemblyVersionLabeller] Major: {0} Minor: {1} Build: {2} Revision: {3}", this.Major, this.Minor, currentBuild, currentRevision));

            string result = string.Concat(this.Major.ToString(this.MajorLabelFormat), ".", this.Minor.ToString(this.MinorLabelFormat), ".", currentBuild.ToString(this.BuildLabelFormat), ".", currentRevision.ToString(this.RevisionLabelFormat));
