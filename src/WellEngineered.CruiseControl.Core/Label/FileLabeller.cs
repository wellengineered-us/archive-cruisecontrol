...
namespace WellEngineered.CruiseControl.Core.Label
{
        ...
        : LabellerBase
    {
        ...
        {
            get { return this.labelFilePath; }
            set { this.labelFilePath = value; }
        }
        ...
        {
            get { return this.allowDuplicateSubsequentLabels; }
            set { this.allowDuplicateSubsequentLabels = value; }
        }
        ...
        {
            string label = this.fileReader.GetLabel(integrationResult.BaseFromWorkingDirectory(this.labelFilePath));
            string suffix = this.GetSuffixBasedOn(label, integrationResult.LastIntegration.Label);
            return string.Format(System.Globalization.CultureInfo.CurrentCulture,"{0}{1}{2}", this.prefix, label, suffix);
        }
        #endregion
        ...
        private string GetSuffixBasedOn(string currentLabel, string lastIntegrationLabel)
        {
            ...
            if (!this.allowDuplicateSubsequentLabels && currentLabel != null && Equals(currentLabel, labelWithoutSuffix))
            {
                if (splits.Length > 1)
                {
        ...
        public class FileReader
        {
            ...
            {
                string label = this.ReadLabel(labelFilePath);
