...
namespace WellEngineered.CruiseControl.Core.Publishers
{
        ...
        : TaskBase
    {
        ...
        public int NumberOfItems
        {
            ...
            set { this.numberOfItems = value > 255 ? 255 : value; }
        }
        ...
        {
            result.BuildProgressInformation.SignalStartRunTask(!string.IsNullOrEmpty(this.Description) ? this.Description : "Making RSS feed");

            string feedFile = RSSDataFileLocation(result.ArtifactDirectory);
            XmlElement channelElement = this.LoadOrInitialiseChannelElement(result, feedFile);
            this.GenerateDocument(result, channelElement);
        ...
        private XmlElement LoadOrInitialiseChannelElement(IIntegrationResult result, string feedFile)
        {
            ...
            {
                channelElement = this.InitialiseFeed(rssFeed, result.ProjectName, result.ProjectUrl);
            }
        ...
        {
            XmlElement rootElement = this.CreateElement(rssFeed, "rss");
            rootElement.SetAttribute("version", "2.0");
            rssFeed.AppendChild(rootElement);

            XmlElement channelElement = this.CreateElement(rssFeed, "channel");
            rootElement.AppendChild(channelElement);
            channelElement.AppendChild(this.CreateTextElement(rssFeed, "title", "CruiseControl.Net - {0}", projectName));
            if (!string.IsNullOrEmpty(projectUrl)) channelElement.AppendChild(this.CreateTextElement(rssFeed, "link", projectUrl));
            channelElement.AppendChild(this.CreateTextElement(rssFeed, "description", "Latest build results for '{0}'", projectName));
            channelElement.AppendChild(this.CreateTextElement(rssFeed, "language", "en"));
            channelElement.AppendChild(this.CreateTextElement(rssFeed, "ttl", "5"));
            channelElement.AppendChild(this.CreateTextElement(rssFeed, "generator", "CruiseControl.Net"));
        ...
        {
            XmlElement element = this.CreateElement(document, name);
        ...
        private void GenerateDocument(IIntegrationResult result, XmlNode channelElement)
        {
            ...
            int position = 0;
            while (count > this.numberOfItems)
            {
            ...
            XmlElement integrationElement = this.BuildIntegrationElement(ownerDocument, result);
        ...
        {
            XmlElement integrationElement = this.CreateElement(ownerDocument, "item");
            integrationElement.AppendChild(this.CreateTextElement(integrationElement.OwnerDocument,
                "title",
                "Build {0} : {1}  {2}  {3}",
                result.Label,
                result.Status,
                this.GetAmountOfModifiedFiles(result),
                this.GetFirstCommentedModification(result)));
            integrationElement.AppendChild(this.CreateTextElement(
                integrationElement.OwnerDocument, "description", this.GetAmountOfModifiedFiles(result)));
            integrationElement.AppendChild(this.CreateTextElement(
        ...
        private string GetBuildModifications(IIntegrationResult result)
        {
            ...
            return this.WriteModificationsSummary(modifications) + this.WriteModificationsDetails(modifications);
        }
