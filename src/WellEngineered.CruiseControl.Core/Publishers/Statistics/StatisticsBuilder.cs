...
namespace WellEngineered.CruiseControl.Core.Publishers.Statistics
{
    ...
    public class StatisticsBuilder
    {
        ...
        {
            this.Add(new FirstMatch("StartTime", "/cruisecontrol/build/@date"));
            this.Add(new FirstMatch("Duration", "/cruisecontrol/build/@buildtime"));

            this.Add(new Statistic("TestCount", "sum(//test-results/@total)"));
            this.Add(new Statistic("TestFailures", "sum(//test-results/@failures)"));
            this.Add(new Statistic("TestIgnored", "sum(//test-results/@not-run)"));

			this.Add(new Statistic("GendarmeDefects", "count(//gendarme-output//rule/target/defect)"));

			this.Add(new Statistic("FxCop Warnings", "count(//FxCopReport//Message[Issue/@Level='Warning' or Issue/@Level='CriticalWarning'])"));
			this.Add(new Statistic("FxCop Errors", "count(//FxCopReport//Message[Issue/@Level='Error' or Issue/@Level='CriticalError'])"));

            this.Add(new FirstMatch("BuildErrorType", "//failure/builderror/type"));
            this.Add(new FirstMatch("BuildErrorMessage", "//failure/builderror/message"));
        ...
        {
            return this.ProcessBuildResults(ToXml(result));
        }
        ...
        {
            if (!this.logStatistics.Contains(statistic))
            {
                this.logStatistics.Add(statistic);
            }
            else
            {
                int indexOf = this.logStatistics.IndexOf(statistic);
                this.logStatistics.Remove(statistic);
                if (statistic.Include)
                {
                    this.logStatistics.Insert(indexOf, statistic);
                }
            }
        }
        ...
        private StatisticsResults ProcessLog(IXPathNavigable doc)
        {
            ...
            foreach (StatisticBase s in this.logStatistics)
            {
