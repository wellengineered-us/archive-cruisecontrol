...
namespace WellEngineered.CruiseControl.Core.Publishers.Statistics
{
    ...
    public class StatisticsChartGenerator
    {
        ...
        {
            get { return this.relevantStats; }
            set { this.relevantStats = value; }
        }
        ...
        {
            foreach (string relevantStat in this.relevantStats)
            {
                ...
                try
                {
                    this.GetPlotter(savePath).DrawGraph(ordinateData, abscissaData, relevantStat);
                }
                catch (Exception)
                {
