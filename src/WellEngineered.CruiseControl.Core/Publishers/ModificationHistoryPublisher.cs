...
namespace WellEngineered.CruiseControl.Core.Publishers
{
        ...
        : TaskBase
    {
        ...
        {
            get { return this.onlyLogWhenChangesFound; }
            set { this.onlyLogWhenChangesFound = value; }
        }
        ...
        {
            if ((this.OnlyLogWhenChangesFound) & (result.Modifications.Length == 0)) return true;

            result.BuildProgressInformation.SignalStartRunTask(!string.IsNullOrEmpty(this.Description) ? this.Description : "Saving modification history");                

            string dataHistoryFile = System.IO.Path.Combine(result.ArtifactDirectory, DataHistoryFileName);

            this.WriteModifications(dataHistoryFile, result);
        ...
        {            
            ...
            this.WriteXMLAttributeAndValue(currentBuildInfoWriter, "BuildDate", DateUtil.FormatDate(result.EndTime));
            this.WriteXMLAttributeAndValue(currentBuildInfoWriter, "Success", result.Succeeded.ToString(CultureInfo.CurrentCulture));
            this.WriteXMLAttributeAndValue(currentBuildInfoWriter, "Label", result.Label);
