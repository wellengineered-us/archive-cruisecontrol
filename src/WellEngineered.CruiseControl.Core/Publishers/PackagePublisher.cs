...
namespace WellEngineered.CruiseControl.Core.Publishers
{
        ...
        : TaskBase
    {
        ...
        protected override bool Execute(IIntegrationResult result)
        {
            ...
            if (this.AlwaysPackage || (result.Status == IntegrationStatus.Success))
            {
                var logMessage = string.Format(CultureInfo.CurrentCulture, "Building package '{0}'", this.PackageName);
                ...
                try
                {
                    ...
                    try
                    {
                        ...
                        zipStream.SetLevel(this.CompressionLevel == 0 ? 5 : this.CompressionLevel);
                        foreach (IPackageItem packageEntry in this.PackageList)
                        {
                        ...
                        if (this.ManifestGenerator != null)
                        {
                            ...
                            this.AddManifest(result, packagedFiles, zipStream);
                        }
                    }
                    finally
                    {
                    ...
                    // Move the file over
                    string actualFile = this.MoveFile(result, tempFile);

                    // Add a list entry
                    Log.Debug("Adding to package list(s)");
                    string listFile = Path.Combine(result.Label, result.ProjectName + "-packages.xml");
                    this.AddToPackageList(result, listFile, actualFile, fileList.Count);
                }
                finally
                {
        ...
        private void AddToPackageList(IIntegrationResult result, string listFile, string fileName, int numberOfFiles)
        {
            ...
            XmlElement packageElement = listXml.SelectSingleNode(
                string.Format(CultureInfo.CurrentCulture, "/packages/package[@name='{0}']", this.PackageName)) as XmlElement;
        ...
        {
            string actualFile = EnsureFileExtension(Path.Combine(result.Label, this.PackageName), ".zip");
            actualFile = result.BaseFromArtifactsDirectory(actualFile);
            if (File.Exists(actualFile))
            {
                this.DeleteFileWithRetry(actualFile);
            }
            ...
            {
                var basePath = this.OutputDirectory;
                ...
                File.Copy(
                    actualFile,
                    EnsureFileExtension(Path.Combine(basePath, this.PackageName), ".zip"),
                    true);
            }
        ...
        private void AddManifest(IIntegrationResult result, List<string> packagedFiles, ZipOutputStream zipStream)
        {
            ...
            XmlDocument manifest = this.ManifestGenerator.Generate(result, packagedFiles.ToArray());
