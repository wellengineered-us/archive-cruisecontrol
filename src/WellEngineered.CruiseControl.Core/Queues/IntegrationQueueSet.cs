	...
	public class IntegrationQueueSet
	{
		...
		public IIntegrationQueue this[string queueName]
		{
			get
			{
				lock (this)
				{
					return (IIntegrationQueue) this.queueSet[queueName];
				}
			}
		}
		...
		public void Add(string queueName, IQueueConfiguration config)
		{
			lock (this)
			{
				if (!this.queueSet.ContainsKey(queueName))
				{
					this.queueSet.Add(queueName, new IntegrationQueue(queueName, config, this));
				}
			}
		}
		...
		public string[] GetQueueNames()
		{
			lock (this)
			{
				string[] queueNames = new string[this.queueSet.Keys.Count];
				this.queueSet.Keys.CopyTo(queueNames, 0);
				return queueNames;
			}
		}
		...
		public QueueSetSnapshot GetIntegrationQueueSnapshot()
		{
			lock (this)
			{
				return this.BuildQueueSetSnapshot();
			}
		}
		...
		private QueueSetSnapshot BuildQueueSetSnapshot()
		{
			...
			foreach (IIntegrationQueue queue in this.queueSet.Values)
			{
				if (queue != null)
				{
