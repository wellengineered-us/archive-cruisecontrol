...
namespace WellEngineered.CruiseControl.Core.Config.Preprocessor
{
    ...
    public abstract class PreprocessorException : ApplicationException
    {
        ...
        public class XmlContext
        {
            ...
            {
                string path = this.Path ?? "Unknown";
                string line = "Unknown", pos = "Unknown";
                if ( this.LineInfo != null && this.LineInfo.HasLineInfo() )
                {
                    line = this.LineInfo.LineNumber.ToString(CultureInfo.CurrentCulture);
                    pos = this.LineInfo.LinePosition.ToString(CultureInfo.CurrentCulture);
                }
