    ...
    public abstract class ElementProcessor : IElementProcessor
    {
        ...
        protected IEnumerable< XNode > _ProcessNodes(IEnumerable< XNode > nodes)
        {
            if ( nodes.Any() )
            {
                ...
                {
                    return_nodes.AddRange( this._Env._DefaultNodeProcessor.Process( node ) );
                }
                return return_nodes;
            }
            return new XNode[] {};
        ...
        {
            IEnumerable< XNode > result = this._Env.EvalTextSymbols( value );
            ...
            if (textvalue != value)
            {
                return this._ProcessNodes(result);
            }
            else
            {
                return result;
        ...
        protected void _DefineFromAttributes(XElement element)
        {
            ...
            {
                this._Env.DefineTextSymbol( attr.Name.LocalName, attr.Value );
            }
        }
