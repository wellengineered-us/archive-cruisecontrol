...
namespace WellEngineered.CruiseControl.Core.Config.Preprocessor
{
    ...
    public class ConfigPreprocessor
    {
                                                  ...
                                                  XmlUrlResolver resolver, Uri inputUri)
        {
            ...
            this._env = new PreprocessorEnvironment( this._settings, base_uri, resolver );
            ...
            foreach ( XNode out_node in
                doc.Nodes().SelectMany(
                    node => this._env._DefaultNodeProcessor.Process( node ) ).Where( node => !(node is XText)) )
            {
                out_node.WriteTo( output );
            }

            // Notify listeners of all files encountered.
            if (this.SubfileLoaded != null)
            {
                foreach (Uri path in this._env.Fileset)
                {
                    this.SubfileLoaded( path );
                }
            }
            return this._env;
        }
