...
namespace WellEngineered.CruiseControl.Core.Config
{
        ...
        : INetReflectorConfigurationReader
	{
		...
		public NetReflectorConfigurationReader()
		{
            ...
            string pluginLocation = System.Configuration.ConfigurationManager.AppSettings["PluginLocation"];
            if (!string.IsNullOrEmpty(pluginLocation))
            {
                if (Directory.Exists(pluginLocation))
                {
                    this.typeTable.Add(pluginLocation, CONFIG_ASSEMBLY_PATTERN);
                }
                else
                {
			...
			this.reader = new NetReflectorReader(this.typeTable);
		}
            ...
            IConfigurationErrorProcesser errorProcesser)
		{
            ...
            this.VerifyDocumentHasValidRootElement(document, actualErrorProcesser);
            ...
            this.typeTable.InvalidNode += invalidNodeHandler;
            try
            {
                ...
                this.ValidateConfiguration(configuration, actualErrorProcesser);
            }
