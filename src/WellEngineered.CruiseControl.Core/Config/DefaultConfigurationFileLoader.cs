...
using WellEngineered.CruiseControl.Core.Config.Preprocessor;
...
namespace WellEngineered.CruiseControl.Core.Config
{
	...
	public class DefaultConfigurationFileLoader : IConfigurationFileLoader
	{
		...
		{
			this.reader = reader;
			this.handler = new ValidationEventHandler(HandleSchemaEvent);
		}
		...
		public IConfiguration Load(FileInfo configFile)
		{
			...
			return this.PopulateProjectsFromXml(this.LoadConfiguration(configFile));
		}
	    ...
	    {
	        this.preprocessor.SubfileLoaded += handler;
	    }
		...
		public XmlDocument LoadConfiguration(FileInfo configFile)
		{
            ...
            try
            {
                ...
                XmlDocument config = this.AttemptLoadConfiguration(configFile);
                return config;
            }
            finally
            {
		...
		private XmlDocument AttemptLoadConfiguration(FileInfo configFile)
		{
			try
			{
				return this.CreateXmlValidatingLoader(configFile).Load();
			}
		...
		private XmlValidatingLoader CreateXmlValidatingLoader(FileInfo configFile)
		{
            ...
            using (XmlReader reader = XmlReader.Create(configFile.FullName, settings2))
            {
                ...
                {
                    this.preprocessor.PreProcess(reader, writer, new XmlUrlResolver(), null);
                }
            }
			...
			loader.ValidationEventHandler += this.handler;
			return loader;
		}
		...
		{
			return this.reader.Read(configXml, null);
		}
