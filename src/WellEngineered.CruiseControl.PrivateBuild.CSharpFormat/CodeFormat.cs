...
namespace WellEngineered.CruiseControl.PrivateBuild.CSharpFormat
{
	/// <summary>
	...
	public abstract class CodeFormat : SourceFormat
	{
		...
		protected CodeFormat()
		{
			...
			string regKeyword = r.Replace(this.Keywords, @"(?<=^|\W)$0(?=\W)");
			string regPreproc = r.Replace(this.Preprocessors, @"(?<=^|\s)$0(?=\s|$)");
			r = new Regex(@" +");
