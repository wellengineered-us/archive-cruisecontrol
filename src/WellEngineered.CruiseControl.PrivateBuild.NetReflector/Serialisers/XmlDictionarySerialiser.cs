...
namespace WellEngineered.CruiseControl.PrivateBuild.NetReflector.Serialisers
{
	public class XmlDictionarySerialiser : XmlMemberSerialiser
	{
		...
		protected override void WriteValue(XmlWriter writer, object value)
		{
			...
			foreach (object key in dictionary.Keys)
			{
				...
				if (typeAttribute == null)
				{
					...
					writer.WriteAttributeString(this.attribute.Key, key.ToString());
		...
		{
			IDictionary dictionary = this.Instantiator.Instantiate(instanceType) as IDictionary;
			// null check
			foreach (XmlNode child in XmlElementList.Create(node.ChildNodes))
			{
				object key = this.GetHashkey(child);
		...
		{
			return XmlUtil.GetChildNode(node, this.attribute.Key).InnerText;
		}
	}
}