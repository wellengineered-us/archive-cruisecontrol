...
namespace WellEngineered.CruiseControl.PrivateBuild.Log4Net.Config
{
	/// <summary>
	...
	/// <see cref="LoggerManager"/> methods is made from the assembly with the configuration
	/// attributes. 
	/// </para>
	/// <para>
	...
	public /*sealed*/ class XmlConfiguratorAttribute : ConfiguratorAttribute
	{
		...
		{
			get { return this.m_configFileExtension; }
		...
		public bool Watch
		{
			get { return this.m_configureAndWatch; }
		...
		private void ConfigureFromUri(Assembly sourceAssembly, ILoggerRepository targetRepository)
		{
			...
			else
			{
				...
				{
					// Just the base dir + the config file
					fullPath2ConfigFile = new Uri(new Uri(applicationBaseDirectory), this.m_configFile);
				}
				else
				{
					fullPath2ConfigFile = new Uri(this.m_configFile);
				}
			}
			...
			if (fullPath2ConfigFile != null)
			{
				if (fullPath2ConfigFile.IsFile)
				{
					...
					this.ConfigureFromFile(targetRepository, new FileInfo(fullPath2ConfigFile.LocalPath));
				}
				else
				{
					if (this.m_configureAndWatch)
