...
using WellEngineered.CruiseControl.PrivateBuild.Log4Net.Appender;
using WellEngineered.CruiseControl.PrivateBuild.Log4Net.Core;
using WellEngineered.CruiseControl.PrivateBuild.Log4Net.ObjectRenderer;
using WellEngineered.CruiseControl.PrivateBuild.Log4Net.Util;

namespace WellEngineered.CruiseControl.PrivateBuild.Log4Net.Repository.Hierarchy
{
	/// <summary>
	...
	public class XmlHierarchyConfigurator
	{
		...
		protected void ParseRenderer(XmlElement element) 
		{
			...
			else 
			{
				...
				{
#if NETSTANDARD1_3
					this.m_hierarchy.RendererMap.Put(SystemInfo.GetTypeFromString(this.GetType().GetTypeInfo().Assembly, renderedClassName, true, true), renderer);
#else
