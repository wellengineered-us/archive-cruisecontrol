...
using WellEngineered.CruiseControl.PrivateBuild.Log4Net.Plugin;
using WellEngineered.CruiseControl.PrivateBuild.Log4Net.Repository;
using WellEngineered.CruiseControl.PrivateBuild.Log4Net.Repository.Hierarchy;
...
namespace WellEngineered.CruiseControl.PrivateBuild.Log4Net.Core
{
	/// <summary>
	...
	public class DefaultRepositorySelector : IRepositorySelector
	{
		...
		/// an appropriate value would be <see cref="Hierarchy"/>.
		/// </para>
		/// </remarks>
		...
		/// </para>
		/// <para>
		/// The default values are to use the <see cref="Hierarchy"/> 
...
#if NETSTANDARD1_3
				object[] repositoryAttributes = assembly.GetCustomAttributes(typeof(RepositoryAttribute)).ToArray();
#else
...
#if NETSTANDARD1_3
			object[] configAttributes = assembly.GetCustomAttributes(typeof(ConfiguratorAttribute)).ToArray();
#else
...
#if NETSTANDARD1_3
			object[] configAttributes = assembly.GetCustomAttributes(typeof(PluginAttribute)).ToArray();
#else
...
#if NETSTANDARD1_3
			object[] configAttributes = assembly.GetCustomAttributes(typeof(AliasRepositoryAttribute)).ToArray();
#else
