...
namespace WellEngineered.CruiseControl.WebDashboard.ServerConnection
{
    public class ServerAggregatingCruiseManagerWrapper : ICruiseManagerWrapper, IFarmService
	{
        ...
        public IBuildSpecifier[] GetBuildSpecifiers(IProjectSpecifier projectSpecifier, string sessionToken)
		{
            ...
            return this.CreateBuildSpecifiers(projectSpecifier, response);
		}
        ...
        {
            this.ForceBuild(projectSpecifier, sessionToken, new Dictionary<string, string>());
		}
        ...
        {
            var locations = this.ServerLocations;
        ...
        {
			return this.GetProjectStatusListAndCaptureExceptions(this.GetServerSpecifiers(), sessionToken);
        }
        ...
        public ProjectStatusListAndExceptions GetProjectStatusListAndCaptureExceptions(IServerSpecifier serverSpecifier, string sessionToken)
				{
			return this.GetProjectStatusListAndCaptureExceptions(new[] {serverSpecifier}, sessionToken);
			}
        ...
        private ProjectStatusListAndExceptions GetProjectStatusListAndCaptureExceptions(IServerSpecifier[] serverSpecifiers, string sessionToken)
        {
            ...
            foreach (IServerSpecifier serverSpecifier in serverSpecifiers)
            {
                ...
                {
                    this.AddException(exceptions, serverSpecifier, e);
                }
            }
		...
		{
			exceptions.Add(new CruiseServerException(serverSpecifier.ServerName, this.GetServerUrl(serverSpecifier).Url, e));
		}
		...
		{
            var config = this.GetServerUrl(serverSpecifier);
            CruiseServerClientBase manager = this.clientFactory.GenerateClient(config.Url,
        ...
        {
			get { return this.configuration.Servers; }
        }
        ...
        {
            return this.GetCruiseServerSnapshotListAndExceptions(this.GetServerSpecifiers(), sessionToken);
        }
        ...
        {
            return this.GetCruiseServerSnapshotListAndExceptions(new[] { serverSpecifier }, sessionToken);
        }

        public string Login(string server, LoginRequest credentials)
		{
            var manager = GetCruiseManager(this.GetServerConfiguration(server), null);
        ...
        public void Logout(string server, string sessionToken)
			{
            GetCruiseManager(this.GetServerConfiguration(server), sessionToken).Logout();
		}

        public string GetDisplayName(string server, string sessionToken)
        {
            return GetCruiseManager(this.GetServerConfiguration(server), sessionToken).DisplayName;
        }
        ...
        {
            GetCruiseManager(this.GetServerConfiguration(server), sessionToken)
