...
namespace WellEngineered.CruiseControl.WebDashboard.Dashboard
{
	public class CruiseManagerBuildNameRetriever : IBuildNameRetriever
	{
		...
		{
			return this.cruiseManagerWrapper.GetLatestBuildSpecifier(projectSpecifier, sessionToken);	
		}

        public IBuildSpecifier GetNextBuildSpecifier(IBuildSpecifier buildSpecifier, string sessionToken)
		{
			IBuildSpecifier[] buildSpecifiers = this.cruiseManagerWrapper.GetBuildSpecifiers(buildSpecifier.ProjectSpecifier, sessionToken);
