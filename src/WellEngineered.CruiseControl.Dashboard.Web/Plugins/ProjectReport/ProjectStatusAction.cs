...
namespace WellEngineered.CruiseControl.WebDashboard.Plugins.ProjectReport
{
    ...
    public class ProjectStatusAction : ICruiseAction
    {
        ...
        public IResponse Execute(ICruiseRequest cruiseRequest)
        {
            ...
            var snapshot = this.farmServer.TakeStatusSnapshot(projectSpecifier, cruiseRequest.RetrieveSessionToken());
        ...
        private string ConvertStatusToJson(ProjectStatusSnapshot status)
        {
            ...
            jsonText.Append("{");
            jsonText.AppendFormat("time:{0},", this.ToJsonDate(status.TimeOfSnapshot));
            this.AppendStatusDetails(status, jsonText);
            jsonText.Append("}");
        ...
        private void AppendStatusDetails(ItemStatus status, StringBuilder builder)
        {
            ...
            builder.AppendFormat(",name:'{0}'", this.ToJsonString(status.Name));
            builder.AppendFormat(",status:'{0}'", status.Status);
            if (!string.IsNullOrEmpty(status.Description)) builder.AppendFormat(",description:'{0}'", this.ToJsonString(status.Description));
            if (status.TimeStarted.HasValue) builder.AppendFormat(",started:{0}", this.ToJsonDate(status.TimeStarted.Value));
            if (status.TimeCompleted.HasValue) builder.AppendFormat(",completed:{0}", this.ToJsonDate(status.TimeCompleted.Value));
            if (status.TimeOfEstimatedCompletion.HasValue) builder.AppendFormat(",estimated:{0}", this.ToJsonDate(status.TimeOfEstimatedCompletion.Value));
