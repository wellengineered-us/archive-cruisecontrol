...
namespace WellEngineered.CruiseControl.WebDashboard.Plugins.Administration
{
        ...
        : IDisposable
    {
        ...
        public Package(Stream packageStream)
        {
            ...
            this.ExtractAllFiles(packageStream);

            // Attempt to load the manifest
            string manifestFile = Path.Combine(this.tempFolder, "Manifest.xml");
            if (File.Exists(manifestFile))
            {
                ...
                {
                    this.manifest = serialiser.Deserialize(inputStream) as PackageManifest;
                    inputStream.Close();
                }
            }
        }
        ...
        public void Install()
        {
            if (!this.IsValid) throw new ApplicationException("This is not a valid package");

            // Start the stopwatch
            this.FireMessage(TraceLevel.Info, "Starting installation");
            ...
            {
                // Copy all the files
                this.FireMessage(TraceLevel.Verbose, "Installing files");
                foreach (FileLocation folder in this.manifest.FileLocations)
                {
                    this.CopyFiles(folder);
                }

                // Add all the configuration settings
                this.FireMessage(TraceLevel.Verbose, "Updating configuration");
                this.UpdateConfigurationFile(true);
            }
            catch (Exception error)
            {
                // Tell the calling app that an unexpected error occurred
                this.FireMessage(TraceLevel.Error,
            ...
            finally
            {
                ...
                this.FireMessage(TraceLevel.Info,
        ...
        public void Uninstall()
        {
            ...
            // Start the stopwatch
            this.FireMessage(TraceLevel.Info, "Starting uninstallation");
            Stopwatch stopwatch = new Stopwatch();
            stopwatch.Start();
            ...
            try
            {
                ...
                this.FireMessage(TraceLevel.Verbose, "Removing files");
                foreach (FileLocation folder in this.manifest.FileLocations)
                {
                ...
                this.FireMessage(TraceLevel.Verbose, "Updating configuration");
                this.UpdateConfigurationFile(false);
            }
            catch (Exception error)
            {
                // Tell the calling app that an unexpected error occurred
        ...
        private void ExtractAllFiles(Stream packageStream)
        {
            ...
            while (entry != null)
            {
                string fileName = Path.Combine(this.tempFolder, entry.Name);
        ...
        private void CopyFiles(FileLocation folder)
        {
            ...
            this.FireMessage(TraceLevel.Verbose, "Copying files to target '{0}'", folder.Location);
            string target = ProgramDataFolder.MapPath(folder.Location);
            if (!Directory.Exists(target))
            {
                this.FireMessage(TraceLevel.Info, "Adding target folder '{0}'", folder.Location);
                Directory.CreateDirectory(target);
            }
            foreach (string file in folder.Files)
            {
                // Make sure the source file exists
                string source = Path.Combine(this.tempFolder, file);
                if (File.Exists(source))
                {
                    // Set to overwrite in case there is an older version of the file
                    this.FireMessage(TraceLevel.Info, "Deploying file '{0}' to target '{1}'", file, folder.Location);
                    File.Copy(source, Path.Combine(target, file), true);
                }
                else
                {
                    this.FireMessage(TraceLevel.Warning, "Source file '{0}' does not exist in package", file);
                }
            }
        }
        #endregion
        ...
        {
            // Make sure the target folder exists
            this.FireMessage(TraceLevel.Verbose, "Removing files from target '{0}'", folder.Location);
            string target = ProgramDataFolder.MapPath(folder.Location);
            if (Directory.Exists(target))
            {
                foreach (string file in folder.Files)
                {
                    ...
                    {
                        // Set to overwrite in case there is an older version of the file
                        this.FireMessage(TraceLevel.Info, "Removing file '{0}' from target '{1}'", file, folder.Location);
        ...
        private void UpdateConfigurationSetting(XmlDocument configXml, ConfigurationSetting setting, bool addSettings)
        {
            ...
            if (element != null)
            {
                this.FireMessage(TraceLevel.Verbose, "Element found ('{0}'), updating", setting.Path);
            ...
            {
                if (addSettings) this.FireMessage(TraceLevel.Warning, "Unable to find element '{0}'", setting.Path);
            }
        }
        #endregion
        ...
        private void UpdateConfigurationFile(bool addSettings)
        {
            ...
            {
                this.FireMessage(TraceLevel.Verbose, "Loading configuration file");
                configXml.Load(configFile);

                // Add or update each setting
                bool configChanged = false;
                foreach (ConfigurationSetting setting in this.manifest.ConfigurationSettings)
                {
                    this.UpdateConfigurationSetting(configXml, setting, addSettings);
                    configChanged = true;
                }
                ...
                if (configChanged)
                {
                    ...
                    this.FireMessage(TraceLevel.Verbose, "Saving configuration file");
            ...
            {
                this.FireMessage(TraceLevel.Warning, "Configuration file does not exist");
            }
        }
        #endregion
        #endregion
