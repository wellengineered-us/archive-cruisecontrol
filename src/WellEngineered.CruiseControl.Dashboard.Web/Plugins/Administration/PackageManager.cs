...
namespace WellEngineered.CruiseControl.WebDashboard.Plugins.Administration
{
    ...
    public class PackageManager
    {
        ...
        public PackageManifest StorePackage(string fileName, Stream stream)
        {
            ...
            this.SaveToFile(stream, packageDetails.FullName);
            bool delete = false;
            ...
            if (manifest != null)
            {
                ...
                XmlDocument packageList = this.LoadPackageList();
                this.UpdatePackagesList(manifest, packageList, true);
                this.SavePackageList(packageList);
            }
            ...
            if (packageDetails.Exists)
            {
                ...
                using (Stream packageStream = File.OpenRead(packagePath))
                {
                    using (Package package = new Package(packageStream))
                    {
                        ...
                        this.UpdatePackagesList(package.Manifest, packageList, true);
                        this.SavePackageList(packageList);
                    }
                }
            }
        ...
        public string RemovePackage(string fileName)
        {
            ...
            if (packageDetails.Directory.Exists)
            {
                ...
                if (manifest != null)
                {
                    // Update the package list
                    XmlDocument packageList = this.LoadPackageList();
                    this.UpdatePackagesList(manifest, packageList, false);
                    this.SavePackageList(packageList);
        ...
        public virtual List<PackageManifest> ListPackages()
        {
            ...
            // Load the document
            XmlDocument document = this.LoadPackageList();
