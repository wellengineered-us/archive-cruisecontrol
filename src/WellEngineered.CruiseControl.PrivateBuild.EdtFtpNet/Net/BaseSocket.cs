...
namespace WellEngineered.CruiseControl.PrivateBuild.EdtFtpNet.Net
{
	/// <summary>  
	...
	public class StandardSocket : BaseSocket
	{
            ...
            : base(addressFamily, socketType, protocolType, logTag)
		{
            ...
            try
            {
                this.socket.SetSocketOption(SocketOptionLevel.Socket, SocketOptionName.ReceiveBuffer, SOCKET_BUFFER_SIZE);
                this.socket.SetSocketOption(SocketOptionLevel.Socket, SocketOptionName.SendBuffer, SOCKET_BUFFER_SIZE);
            }
            catch (Exception ex)
            {
		...
		public override BaseSocket Accept(int timeout)
		{
            ...
            bool success = this.socket.Poll(timeout*1000, SelectMode.SelectRead);
            if (!success)
            {
                this.socket.Close();
        ...
        public override void Connect(EndPoint remoteEP, int timeout)
        {
            ...
            this.InternalConnect(remoteEP, timeout);
        }

        private void InternalConnect(EndPoint remoteEP, int timeout)
        {
            if (timeout == 0) timeout = -1;
            IAsyncResult result = this.socket.BeginConnect(remoteEP, null, null);
