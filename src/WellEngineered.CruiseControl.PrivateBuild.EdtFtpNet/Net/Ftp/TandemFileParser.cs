...
{    
	...
	{			
		...
		{
            this.log = Logger.GetLogger("TandemFileParser");
		}
		...
		public override FTPFile Parse(string raw)
		{
            ...
            try
            {
                lastModified = DateTime.ParseExact(lastModifiedStr, this.formats,
                                    this.ParsingCulture.DateTimeFormat, DateTimeStyles.None);
            }
            catch (FormatException)
            {
                this.log.Warn("Failed to parse date string '" + lastModifiedStr + "'");
            }
			...
			{
				this.log.Warn("Failed to parse size: " + fields[2]);
			}

            string owner = fields[5] + fields[6];
            string permissions = fields[7].Trim(this.trimChars);
