...
{    
    ...
    {            
        ...
        public override FTPFile Parse(string raw) 
        {
            ...
            long size = Int64.Parse(sizeUsed) * this.blocksize;
            
            // 3 & 4 fields are date time
            string lastModifiedStr = this.TweakDateString(fields);
            DateTime lastModified = DateTime.MinValue;
            try
            {
                lastModified = DateTime.Parse(lastModifiedStr.ToString(), this.ParsingCulture.DateTimeFormat);
            }
            catch (FormatException)
            {
