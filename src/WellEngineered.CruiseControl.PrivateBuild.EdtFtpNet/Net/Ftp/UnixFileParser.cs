...
namespace WellEngineered.CruiseControl.PrivateBuild.EdtFtpNet.Net.Ftp
{
	...
	{		
		...
		{
            this.log = Logger.GetLogger("UnixFileParser");
        }
		...
		{		
            ...
            if (Char.IsDigit(fields[index][0])) // assume it is if a digit
            {
				...
				{
                    this.log.Warn("Failed to parse link count: " + linkCountStr);
				}
            }
            ...
            if (this.IsNumeric(fields[index+2]) &&  fields.Length-(index+2) > 4) 
            {
			    owner = fields[index++];
			    group = fields[index++];
            }
            // no owner
            else if (this.IsNumeric(fields[index + 1]) && fields.Length - (index + 1) > 4)
            {
                group = fields[index++];
            }
			...
			{
                this.log.Warn("Failed to parse size: " + sizeStr);
			}                     
            ...
            int dayOfMonth = -1;
            if (this.IsNumeric(fields[index]))
            {
			...
			if (field.IndexOf((System.Char) ':') < 0 && field.IndexOf((System.Char) '.') < 0)
			{
                ...
                try
                {
                    lastModified = DateTime.ParseExact(stamp.ToString(), this.format1,
                                                this.ParsingCulture.DateTimeFormat, DateTimeStyles.None);
                }
                catch (FormatException)
                {
                    this.log.Warn("Failed to parse date string '" + stamp.ToString() + "'");
                }
			}
			else
			{
				// add the year ourselves as not present
                int year = this.ParsingCulture.Calendar.GetYear(DateTime.Now);
				stamp.Append(year).Append('-').Append(field);
                try
                {

                    lastModified = DateTime.ParseExact(stamp.ToString(), this.format2,
                                                this.ParsingCulture.DateTimeFormat, DateTimeStyles.None);
                }
                catch (FormatException)
                {
