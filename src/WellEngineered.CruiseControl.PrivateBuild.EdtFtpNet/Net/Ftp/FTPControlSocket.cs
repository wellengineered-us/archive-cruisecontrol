...
{    
    ...
    public class FTPControlSocket
    {
        ...
        internal virtual void ConnectSocket(BaseSocket socket, string address, int port)
        {
            ...
            socket.Connect(new IPEndPoint(this.remoteAddr, port), this.timeout);
        }
        ...
        {           
            ...
            this.ValidateReply(reply, "220", "230");
            return reply;
        } 
        ...
        internal void InitStreams(Encoding encoding)
        {
            ...
            if (encoding == null)
            {
                ...
                {
                    this.log.Debug("Could not set encoding to Windows-1252: {0}", ex1.Message);
                    try
                    {
                    ...
                    {
                        this.log.Debug("Could not set encoding to ISO-8859-1: {0}", ex2.Message);
                        encoding = Encoding.ASCII;
                    }
                }
            }
            this.encoding = encoding;
            this.log.Info("Command encoding=" + encoding.ToString());
        ...
        internal void SetActivePortRange(PortRange portRange)
        {
            ...
            if (!portRange.UseOSAssignment)
            {
                ...
                this.nextPort = rand.Next(this.activePortRange.LowPort,this.activePortRange.HighPort);
                this.log.Debug("SetActivePortRange("+ this.activePortRange.LowPort + "," + this.activePortRange.HighPort + "). NextPort=" + this.nextPort);
            }
        }
        ...
        internal void Kill()
        {
            ...
            {
                this.log.Debug("Killed socket", e);
            }
        ...
        internal virtual void Logout()
        {
            ...
            if (ex != null) 
            {
                this.log.Error("Caught exception", ex);
                throw ex;
            }
        }
        ...
        public bool Connected 
        {
            get 
            {
                return (this.controlSock == null ? false : this.controlSock.Connected);
            }
        }
        ...
        internal virtual FTPDataSocket CreateDataSocketActive()
        {
            try 
            {
                ...
                if (this.activePortRange != null) 
                {
                    int range = this.activePortRange.HighPort-this.activePortRange.LowPort+1;
        ...
        internal virtual void SetDataPort(IPEndPoint ep)
        {
...
#else
                hostBytes = BitConverter.GetBytes(this.activeIPAddress.Address);
#endif
            }
            ...
            byte[] portBytes = this.ToByteArray((ushort)ep.Port);
            ...
            this.ValidateReply(reply, "200", "250");
        }
        ...
        {
            bool synchronizeConnection = this.SynchronizePassiveConnections;
            Mutex mutex = null;
            ...
            try
            {
                return this.CreateDataSocketPASVInternal();
            }
            finally
            {
        ...
        protected virtual FTPDataSocket CreateDataSocketPASVInternal()
        {
            ...
            this.log.Debug("Server supplied address=" + ipAddress);
            ...
            this.log.Debug("autoPassiveIPSubstitution=" + this.autoPassiveIPSubstitution);
            this.log.Debug("remoteAddr=" + (this.remoteAddr == null ? "null" : this.remoteAddr.ToString()));
            if (this.autoPassiveIPSubstitution && this.remoteAddr != null)
            {
                hostIP = this.remoteAddr.ToString();
                this.log.Debug("Substituting server supplied IP ({0}) with remote host IP ({1})",
                    ipAddress, hostIP);
            }
        ...
        {
            this.log.Debug("NewPassiveDataSocket(" + ipAddress + "," + port + ")");
            IPAddress ad = IPAddress.Parse(ipAddress);  
            IPEndPoint ipe = new IPEndPoint(ad, port); 
            BaseSocket sock =
                new StandardSocket(AddressFamily.InterNetwork, SocketType.Stream, ProtocolType.Tcp, this.logTag);
            try
            {
                this.SetSocketTimeout(sock, this.timeout);
                sock.Connect(ipe, this.timeout);
                this.log.Debug("Connected");
            }
            catch (Exception ex)
            {
                this.log.Error("Failed to create connecting socket", ex);
                sock.Close();
                throw;
            }
        ...
        {                        
            ...
            try
            {
                ...
                endPoint = new IPEndPoint(((IPEndPoint)this.controlSock.LocalEndPoint).Address, ((IPEndPoint)sock.LocalEndPoint).Port);
                this.SetDataPort(endPoint);
            }
            catch (Exception ex)
            {
                this.log.Error("Failed to create listening socket", ex);
                sock.Close();
                throw;
            }
        ...
        public virtual FTPReply SendCommand(string command)
        {
            ...
            {
                this.log.Error("Exception in SendCommand", e);
                if (this.CommandError != null)
                    this.CommandError(this, new FTPErrorEventArgs(e));
                throw;
            }
        }
        ...
        private string ReadLine()
        {
            ...
            while (true)
            {
                ...
                {
                    this.log.Error("Read failed ('" + err.ToString() + "' read so far)");
        ...
        public virtual FTPReply ValidateReply(FTPReply reply, params string[] expectedReplyCodes)
        {
            ...
            buf.Append("]");
            this.log.Debug("Expected reply codes = " + buf.ToString() + " (strict=" + this.strictReturnCodes + ")");
        ...
        internal void SetSocketTimeout(BaseSocket sock, int timeout)
        {
            if (timeout > 0) 
            {
                ...
                {
                    this.log.Warn("Failed to set socket timeout: " + ex.Message);
                }
            }
        }
