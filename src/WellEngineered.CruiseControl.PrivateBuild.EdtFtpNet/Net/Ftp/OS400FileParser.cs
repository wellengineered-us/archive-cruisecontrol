...
{    
    ...
    {            
        ...
        private DateTime GetLastModified(string lastModifiedStr)
        {
            ...
            for (int i = this.formatIndex; i < this.formats.Length; i++, this.formatIndex++)
            {
                try
                {
                    lastModified = DateTime.ParseExact(lastModifiedStr, this.formats[this.formatIndex],
            ...
            if (this.formatIndex > prevIndex) // we've changed formatters so redo
            {
