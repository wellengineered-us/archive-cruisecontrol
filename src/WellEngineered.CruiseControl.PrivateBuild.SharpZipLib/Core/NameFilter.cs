...
namespace WellEngineered.CruiseControl.PrivateBuild.SharpZipLib.Core
{
	...
	public class NameFilter : IScanFilter
	{
		...
		void Compile()
		{
			...
			string[] items = SplitQuoted(this.filter_);
			for (int i = 0; i < items.Length; ++i) {
				if ((items[i] != null) && (items[i].Length > 0)) {
					...
					if (include) {
						this.inclusions_.Add(new Regex(toCompile, RegexOptions.IgnoreCase | RegexOptions.Compiled | RegexOptions.Singleline));
					} else {
						this.exclusions_.Add(new Regex(toCompile, RegexOptions.IgnoreCase | RegexOptions.Compiled | RegexOptions.Singleline));
					}
				}
			}
		}
