...
namespace WellEngineered.CruiseControl.PrivateBuild.SharpZipLib.Tar
{
	...
	public class TarInputStream : Stream
	{
		...
		public bool IsStreamOwner {
			get { return this.tarBuffer.IsStreamOwner; }
		...
		public int RecordSize {
			get { return this.tarBuffer.RecordSize; }
		}
		...
		public TarEntry GetNextEntry()
		{
			...
			if (headerBuf == null) {
				...
				this.hasHitEOF |= TarBuffer.IsEndOfArchiveBlock(headerBuf);
			...
			if (this.hasHitEOF) {
				...
				try {
					...
					if (this.entryFactory == null) {
						...
						if (longName != null) {
							this.currentEntry.Name = longName.ToString();
						}
					} else {
					...
					string errorText = string.Format("Bad header in record {0} block {1} {2}",
						this.tarBuffer.CurrentRecord, this.tarBuffer.CurrentBlock, ex.Message);
