...
namespace WellEngineered.CruiseControl.PrivateBuild.SharpZipLib.Tar
{
	...
	public class TarHeader
	{
		...
		public bool IsChecksumValid {
			get { return this.isChecksumValid; }
		}
		...
		public void ParseBuffer(byte[] header)
		{
			...
			this.Size = ParseBinaryOrOctal(header, offset, SIZELEN);
			offset += SIZELEN;
			...
			if (this.Magic == "ustar")
			{
				...
				if (!string.IsNullOrEmpty(prefix)) this.Name = prefix + '/' + this.Name;
			}
		...
		public void WriteHeader(byte[] outBuffer)
		{
			...
			offset = GetNameBytes(this.Name, outBuffer, offset, NAMELEN);
			offset = GetOctalBytes(this.mode, outBuffer, offset, MODELEN);
			offset = GetOctalBytes(this.UserId, outBuffer, offset, UIDLEN);
			offset = GetOctalBytes(this.GroupId, outBuffer, offset, GIDLEN);

			offset = GetBinaryOrOctalBytes(this.Size, outBuffer, offset, SIZELEN);
			offset = GetOctalBytes(GetCTime(this.ModTime), outBuffer, offset, MODTIMELEN);
			...
			GetCheckSumOctalBytes(this.checksum, outBuffer, csOffset, CHKSUMLEN);
		...
		public override bool Equals(object obj)
		{
			...
			bool result;
			if (localHeader != null) {
				result = (this.name == localHeader.name)
					&& (this.mode == localHeader.mode)
					&& (this.UserId == localHeader.UserId)
					&& (this.GroupId == localHeader.GroupId)
					&& (this.Size == localHeader.Size)
					&& (this.ModTime == localHeader.ModTime)
					&& (this.Checksum == localHeader.Checksum)
					&& (this.TypeFlag == localHeader.TypeFlag)
					&& (this.LinkName == localHeader.LinkName)
					&& (this.Magic == localHeader.Magic)
					&& (this.Version == localHeader.Version)
					&& (this.UserName == localHeader.UserName)
					&& (this.GroupName == localHeader.GroupName)
					&& (this.DevMajor == localHeader.DevMajor)
					&& (this.DevMinor == localHeader.DevMinor);
			} else {
				result = false;
			}
			return result;
