...
namespace WellEngineered.CruiseControl.PrivateBuild.SharpZipLib.GZip
{
	...
	public class GZipInputStream : InflaterInputStream
	{
		...
		bool ReadHeader()
		{
			...
			int magic = this.inputBuffer.ReadLeByte();
			...
			magic = this.inputBuffer.ReadLeByte();
			...
			int compressionType = this.inputBuffer.ReadLeByte();
			...
			// 3. Check the flags
			int flags = this.inputBuffer.ReadLeByte();
			if (flags < 0) {
				throw new EndOfStreamException("EOS reading GZIP header");
			}
			...
			*/
			...
			for (int i = 0; i < 6; i++) {
				int readByte = this.inputBuffer.ReadLeByte();
				if (readByte < 0) {
					throw new EndOfStreamException("EOS reading GZIP header");
				}
			...
			if ((flags & GZipConstants.FEXTRA) != 0) {
				...
				int len1, len2;
				len1 = this.inputBuffer.ReadLeByte();
				len2 = this.inputBuffer.ReadLeByte();
				if ((len1 < 0) || (len2 < 0)) {
					throw new EndOfStreamException("EOS reading GZIP header");
				}
				headCRC.Update(len1);
			...
			if ((flags & GZipConstants.FNAME) != 0) {
				int readByte;
				while ((readByte = this.inputBuffer.ReadLeByte()) > 0) {
					headCRC.Update(readByte);
				}
			...
			if ((flags & GZipConstants.FHCRC) != 0) {
				int tempByte;
				int crcval = this.inputBuffer.ReadLeByte();
				if (crcval < 0) {
					throw new EndOfStreamException("EOS reading GZIP header");
				}

				tempByte = this.inputBuffer.ReadLeByte();
				if (tempByte < 0) {
					throw new EndOfStreamException("EOS reading GZIP header");
				}
		...
		void ReadFooter()
		{
			...
			long bytesRead = this.inf.TotalOut & 0xffffffff;
			...
			int needed = 8;
			while (needed > 0) {
				int count = this.inputBuffer.ReadClearTextBuffer(footer, 8 - needed, needed);
				if (count <= 0) {
