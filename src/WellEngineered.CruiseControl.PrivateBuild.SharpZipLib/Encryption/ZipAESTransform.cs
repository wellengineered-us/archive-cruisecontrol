...
namespace WellEngineered.CruiseControl.PrivateBuild.SharpZipLib.Encryption
{
	...
	internal class ZipAESTransform : ICryptoTransform
	{
		...
		public ZipAESTransform(string key, byte[] saltBytes, int blockSize, bool writeMode)
		{
			...
			byte[] byteKey1 = pdb.GetBytes(this._blockSize);
			byte[] byteKey2 = pdb.GetBytes(this._blockSize);
			this._encryptor = rm.CreateEncryptor(byteKey1, byteKey2);
			this._pwdVerifier = pdb.GetBytes(PWD_VER_LENGTH);
			//
			this._hmacsha1 = IncrementalHash.CreateHMAC(HashAlgorithmName.SHA1, byteKey2);
		...
		public int TransformBlock(byte[] inputBuffer, int inputOffset, int inputCount, byte[] outputBuffer, int outputOffset)
		{
			...
			int ix = 0;
			while (ix < inputCount) {
				if (this._encrPos == ENCRYPT_BLOCK) {
					/* increment encryption nonce   */
					int j = 0;
					while (++this._counterNonce[j] == 0) {
						++j;
					}
				...
				outputBuffer[ix + outputOffset] = (byte)(inputBuffer[ix + inputOffset] ^ this._encryptBuffer[this._encrPos++]);
				//
				ix++;
			}
			if (this._writeMode) {
