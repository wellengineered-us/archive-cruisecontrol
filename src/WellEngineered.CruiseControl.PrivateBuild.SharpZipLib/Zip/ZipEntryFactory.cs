...
namespace WellEngineered.CruiseControl.PrivateBuild.SharpZipLib.Zip
{
	...
	public class ZipEntryFactory : IEntryFactory
	{
		...
		public INameTransform NameTransform {
			get { return this.nameTransform_; }
			set {
				if (value == null) {
					this.nameTransform_ = new ZipNameTransform();
				} else {
		...
		public int GetAttributes {
			get { return this.getAttributes_; }
		...
		public int SetAttributes {
			get { return this.setAttributes_; }
		...
		public bool IsUnicodeText {
			get { return this.isUnicodeText_; }
		...
		{
			var result = new ZipEntry(this.nameTransform_.TransformFile(!string.IsNullOrEmpty(entryName) ? entryName : fileName));
			result.IsUnicodeText = this.isUnicodeText_;

			int externalAttributes = 0;
			bool useAttributes = (this.setAttributes_ != 0);
			...
			if ((fi != null) && fi.Exists) {
				switch (this.timeSetting_) {
					...
					case TimeSetting.Fixed:
						result.DateTime = this.fixedDateTime_;
						break;
				...
				useAttributes = true;
				externalAttributes = ((int)fi.Attributes & this.getAttributes_);
			} else {
			...
			if (useAttributes) {
				externalAttributes |= this.setAttributes_;
		...
		public ZipEntry MakeDirectoryEntry(string directoryName, bool useFileSystem)
		{
			...
			var result = new ZipEntry(this.nameTransform_.TransformDirectory(directoryName));
			result.IsUnicodeText = this.isUnicodeText_;
			result.Size = 0;
			...
			if ((di != null) && di.Exists) {
				...
				externalAttributes = ((int)di.Attributes & this.getAttributes_);
			} else {
				if (this.timeSetting_ == TimeSetting.Fixed) {
					result.DateTime = this.fixedDateTime_;
				}
			...
			externalAttributes |= (this.setAttributes_ | 16);
			result.ExternalFileAttributes = externalAttributes;
